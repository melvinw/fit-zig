//! This file was generated from a FIT profile. Do not edit by hand.
//! Instead, run this command: 'tools/gen-profile-types.py global-profile/enums.csv global-profile/messages.csv'
const std = @import("std");
const decoder = @import("../decoder.zig");
const assert = std.debug.assert;

pub const File = enum(u8) {
    /// Read only, single file. Must be in root directory.
    device = 1,
    /// Read/write, single file. Directory=Settings
    settings = 2,
    /// Read/write, multiple files, file number = sport type. Directory=Sports
    sport = 3,
    /// Read/erase, multiple files. Directory=Activities
    activity = 4,
    /// Read/write/erase, multiple files. Directory=Workouts
    workout = 5,
    /// Read/write/erase, multiple files. Directory=Courses
    course = 6,
    /// Read/write, single file. Directory=Schedules
    schedules = 7,
    /// Read only, single file. Circular buffer. All message definitions at start of file. Directory=Weight
    weight = 9,
    /// Read only, single file. Directory=Totals
    totals = 10,
    /// Read/write, single file. Directory=Goals
    goals = 11,
    /// Read only. Directory=Blood Pressure
    blood_pressure = 14,
    /// Read only. Directory=Monitoring. File number=sub type.
    monitoring_a = 15,
    /// Read/erase, multiple files. Directory=Activities
    activity_summary = 20,
    monitoring_daily = 28,
    /// Read only. Directory=Monitoring. File number=identifier
    monitoring_b = 32,
    /// Read/write/erase. Multiple Files. Directory=Segments
    segment = 34,
    /// Read/write/erase. Single File. Directory=Segments
    segment_list = 35,
    /// Read/write/erase. Single File. Directory=Settings
    exd_configuration = 40,
    /// 0xF7 - 0xFE reserved for manufacturer specific file types
    mfg_range_min = 0xF7,
    /// 0xF7 - 0xFE reserved for manufacturer specific file types
    mfg_range_max = 0xFE,
};

pub const MesgNum = enum(u16) {
    file_id = 0,
    capabilities = 1,
    device_settings = 2,
    user_profile = 3,
    hrm_profile = 4,
    sdm_profile = 5,
    bike_profile = 6,
    zones_target = 7,
    hr_zone = 8,
    power_zone = 9,
    met_zone = 10,
    sport = 12,
    training_settings = 13,
    goal = 15,
    session = 18,
    lap = 19,
    record = 20,
    event = 21,
    device_info = 23,
    workout = 26,
    workout_step = 27,
    schedule = 28,
    weight_scale = 30,
    course = 31,
    course_point = 32,
    totals = 33,
    activity = 34,
    software = 35,
    file_capabilities = 37,
    mesg_capabilities = 38,
    field_capabilities = 39,
    file_creator = 49,
    blood_pressure = 51,
    speed_zone = 53,
    monitoring = 55,
    training_file = 72,
    hrv = 78,
    ant_rx = 80,
    ant_tx = 81,
    ant_channel_id = 82,
    length = 101,
    monitoring_info = 103,
    pad = 105,
    slave_device = 106,
    connectivity = 127,
    weather_conditions = 128,
    weather_alert = 129,
    cadence_zone = 131,
    hr = 132,
    segment_lap = 142,
    memo_glob = 145,
    segment_id = 148,
    segment_leaderboard_entry = 149,
    segment_point = 150,
    segment_file = 151,
    workout_session = 158,
    watchface_settings = 159,
    gps_metadata = 160,
    camera_event = 161,
    timestamp_correlation = 162,
    gyroscope_data = 164,
    accelerometer_data = 165,
    three_d_sensor_calibration = 167,
    video_frame = 169,
    obdii_data = 174,
    nmea_sentence = 177,
    aviation_attitude = 178,
    video = 184,
    video_title = 185,
    video_description = 186,
    video_clip = 187,
    ohr_settings = 188,
    exd_screen_configuration = 200,
    exd_data_field_configuration = 201,
    exd_data_concept_configuration = 202,
    field_description = 206,
    developer_data_id = 207,
    magnetometer_data = 208,
    barometer_data = 209,
    one_d_sensor_calibration = 210,
    monitoring_hr_data = 211,
    time_in_zone = 216,
    set = 225,
    stress_level = 227,
    max_met_data = 229,
    dive_settings = 258,
    dive_gas = 259,
    dive_alarm = 262,
    exercise_title = 264,
    dive_summary = 268,
    spo2_data = 269,
    sleep_level = 275,
    jump = 285,
    aad_accel_features = 289,
    beat_intervals = 290,
    respiration_rate = 297,
    hsa_accelerometer_data = 302,
    hsa_step_data = 304,
    hsa_spo2_data = 305,
    hsa_stress_data = 306,
    hsa_respiration_data = 307,
    hsa_heart_rate_data = 308,
    split = 312,
    split_summary = 313,
    hsa_body_battery_data = 314,
    hsa_event = 315,
    climb_pro = 317,
    tank_update = 319,
    tank_summary = 323,
    sleep_assessment = 346,
    hrv_status_summary = 370,
    hrv_value = 371,
    raw_bbi = 372,
    device_aux_battery_info = 375,
    hsa_gyroscope_data = 376,
    chrono_shot_session = 387,
    chrono_shot_data = 388,
    hsa_configuration_data = 389,
    dive_apnea_alarm = 393,
    skin_temp_overnight = 398,
    /// Message number for the HSA wrist temperature data message
    hsa_wrist_temperature_data = 409,
    nap_event = 412,
    sleep_disruption_severity_period = 470,
    sleep_disruption_overnight_severity = 471,
    /// 0xFF00 - 0xFFFE reserved for manufacturer specific messages
    mfg_range_min = 0xFF00,
    /// 0xFF00 - 0xFFFE reserved for manufacturer specific messages
    mfg_range_max = 0xFFFE,
};

pub const Checksum = enum(u8) {
    /// Allows clear of checksum for flash memory where can only write 1 to 0 without erasing sector.
    clear = 0,
    /// Set to mark checksum as valid if computes to invalid values 0 or 0xFF. Checksum can also be set to ok to save encoding computation time.
    ok = 1,
};

pub const FileFlags = enum(u8) {
    read = 0x02,
    write = 0x04,
    erase = 0x08,
};

pub const MesgCount = enum(u8) {
    num_per_file = 0,
    max_per_file = 1,
    max_per_file_type = 2,
};

/// seconds since UTC 00:00 Dec 31 1989
pub const DateTime = enum(u32) {
    /// if date_time is < 0x10000000 then it is system time (seconds from device power on)
    min = 0x10000000,
};

/// seconds since 00:00 Dec 31 1989 in local time zone
pub const LocalDateTime = enum(u32) {
    /// if date_time is < 0x10000000 then it is system time (seconds from device power on)
    min = 0x10000000,
};

pub const MessageIndex = enum(u16) {
    /// message is selected if set
    selected = 0x8000,
    /// reserved (default 0)
    reserved = 0x7000,
    /// index
    mask = 0x0FFF,
};

pub const DeviceIndex = enum(u8) {
    /// Creator of the file is always device index 0.
    creator = 0,
};

pub const Gender = enum(u8) {
    female = 0,
    male = 1,
};

pub const Language = enum(u8) {
    english = 0,
    french = 1,
    italian = 2,
    german = 3,
    spanish = 4,
    croatian = 5,
    czech = 6,
    danish = 7,
    dutch = 8,
    finnish = 9,
    greek = 10,
    hungarian = 11,
    norwegian = 12,
    polish = 13,
    portuguese = 14,
    slovakian = 15,
    slovenian = 16,
    swedish = 17,
    russian = 18,
    turkish = 19,
    latvian = 20,
    ukrainian = 21,
    arabic = 22,
    farsi = 23,
    bulgarian = 24,
    romanian = 25,
    chinese = 26,
    japanese = 27,
    korean = 28,
    taiwanese = 29,
    thai = 30,
    hebrew = 31,
    brazilian_portuguese = 32,
    indonesian = 33,
    malaysian = 34,
    vietnamese = 35,
    burmese = 36,
    mongolian = 37,
    custom = 254,
};

/// Bit field corresponding to language enum type (1 << language).
pub const LanguageBits0 = enum(u8) {
    english = 0x01,
    french = 0x02,
    italian = 0x04,
    german = 0x08,
    spanish = 0x10,
    croatian = 0x20,
    czech = 0x40,
    danish = 0x80,
};

pub const LanguageBits1 = enum(u8) {
    dutch = 0x01,
    finnish = 0x02,
    greek = 0x04,
    hungarian = 0x08,
    norwegian = 0x10,
    polish = 0x20,
    portuguese = 0x40,
    slovakian = 0x80,
};

pub const LanguageBits2 = enum(u8) {
    slovenian = 0x01,
    swedish = 0x02,
    russian = 0x04,
    turkish = 0x08,
    latvian = 0x10,
    ukrainian = 0x20,
    arabic = 0x40,
    farsi = 0x80,
};

pub const LanguageBits3 = enum(u8) {
    bulgarian = 0x01,
    romanian = 0x02,
    chinese = 0x04,
    japanese = 0x08,
    korean = 0x10,
    taiwanese = 0x20,
    thai = 0x40,
    hebrew = 0x80,
};

pub const LanguageBits4 = enum(u8) {
    brazilian_portuguese = 0x01,
    indonesian = 0x02,
    malaysian = 0x04,
    vietnamese = 0x08,
    burmese = 0x10,
    mongolian = 0x20,
};

pub const TimeZone = enum(u8) {
    almaty = 0,
    bangkok = 1,
    bombay = 2,
    brasilia = 3,
    cairo = 4,
    cape_verde_is = 5,
    darwin = 6,
    eniwetok = 7,
    fiji = 8,
    hong_kong = 9,
    islamabad = 10,
    kabul = 11,
    magadan = 12,
    mid_atlantic = 13,
    moscow = 14,
    muscat = 15,
    newfoundland = 16,
    samoa = 17,
    sydney = 18,
    tehran = 19,
    tokyo = 20,
    us_alaska = 21,
    us_atlantic = 22,
    us_central = 23,
    us_eastern = 24,
    us_hawaii = 25,
    us_mountain = 26,
    us_pacific = 27,
    other = 28,
    auckland = 29,
    kathmandu = 30,
    europe_western_wet = 31,
    europe_central_cet = 32,
    europe_eastern_eet = 33,
    jakarta = 34,
    perth = 35,
    adelaide = 36,
    brisbane = 37,
    tasmania = 38,
    iceland = 39,
    amsterdam = 40,
    athens = 41,
    barcelona = 42,
    berlin = 43,
    brussels = 44,
    budapest = 45,
    copenhagen = 46,
    dublin = 47,
    helsinki = 48,
    lisbon = 49,
    london = 50,
    madrid = 51,
    munich = 52,
    oslo = 53,
    paris = 54,
    prague = 55,
    reykjavik = 56,
    rome = 57,
    stockholm = 58,
    vienna = 59,
    warsaw = 60,
    zurich = 61,
    quebec = 62,
    ontario = 63,
    manitoba = 64,
    saskatchewan = 65,
    alberta = 66,
    british_columbia = 67,
    boise = 68,
    boston = 69,
    chicago = 70,
    dallas = 71,
    denver = 72,
    kansas_city = 73,
    las_vegas = 74,
    los_angeles = 75,
    miami = 76,
    minneapolis = 77,
    new_york = 78,
    new_orleans = 79,
    phoenix = 80,
    santa_fe = 81,
    seattle = 82,
    washington_dc = 83,
    us_arizona = 84,
    chita = 85,
    ekaterinburg = 86,
    irkutsk = 87,
    kaliningrad = 88,
    krasnoyarsk = 89,
    novosibirsk = 90,
    petropavlovsk_kamchatskiy = 91,
    samara = 92,
    vladivostok = 93,
    mexico_central = 94,
    mexico_mountain = 95,
    mexico_pacific = 96,
    cape_town = 97,
    winkhoek = 98,
    lagos = 99,
    riyahd = 100,
    venezuela = 101,
    australia_lh = 102,
    santiago = 103,
    manual = 253,
    automatic = 254,
};

pub const DisplayMeasure = enum(u8) {
    metric = 0,
    statute = 1,
    nautical = 2,
};

pub const DisplayHeart = enum(u8) {
    bpm = 0,
    max = 1,
    reserve = 2,
};

pub const DisplayPower = enum(u8) {
    watts = 0,
    percent_ftp = 1,
};

pub const DisplayPosition = enum(u8) {
    /// dd.dddddd
    degree = 0,
    /// dddmm.mmm
    degree_minute = 1,
    /// dddmmss
    degree_minute_second = 2,
    /// Austrian Grid (BMN)
    austrian_grid = 3,
    /// British National Grid
    british_grid = 4,
    /// Dutch grid system
    dutch_grid = 5,
    /// Hungarian grid system
    hungarian_grid = 6,
    /// Finnish grid system Zone3 KKJ27
    finnish_grid = 7,
    /// Gausss Krueger (German)
    german_grid = 8,
    /// Icelandic Grid
    icelandic_grid = 9,
    /// Indonesian Equatorial LCO
    indonesian_equatorial = 10,
    /// Indonesian Irian LCO
    indonesian_irian = 11,
    /// Indonesian Southern LCO
    indonesian_southern = 12,
    /// India zone 0
    india_zone_0 = 13,
    /// India zone IA
    india_zone_IA = 14,
    /// India zone IB
    india_zone_IB = 15,
    /// India zone IIA
    india_zone_IIA = 16,
    /// India zone IIB
    india_zone_IIB = 17,
    /// India zone IIIA
    india_zone_IIIA = 18,
    /// India zone IIIB
    india_zone_IIIB = 19,
    /// India zone IVA
    india_zone_IVA = 20,
    /// India zone IVB
    india_zone_IVB = 21,
    /// Irish Transverse Mercator
    irish_transverse = 22,
    /// Irish Grid
    irish_grid = 23,
    /// Loran TD
    loran = 24,
    /// Maidenhead grid system
    maidenhead_grid = 25,
    /// MGRS grid system
    mgrs_grid = 26,
    /// New Zealand grid system
    new_zealand_grid = 27,
    /// New Zealand Transverse Mercator
    new_zealand_transverse = 28,
    /// Qatar National Grid
    qatar_grid = 29,
    /// Modified RT-90 (Sweden)
    modified_swedish_grid = 30,
    /// RT-90 (Sweden)
    swedish_grid = 31,
    /// South African Grid
    south_african_grid = 32,
    /// Swiss CH-1903 grid
    swiss_grid = 33,
    /// Taiwan Grid
    taiwan_grid = 34,
    /// United States National Grid
    united_states_grid = 35,
    /// UTM/UPS grid system
    utm_ups_grid = 36,
    /// West Malayan RSO
    west_malayan = 37,
    /// Borneo RSO
    borneo_rso = 38,
    /// Estonian grid system
    estonian_grid = 39,
    /// Latvian Transverse Mercator
    latvian_grid = 40,
    /// Reference Grid 99 TM (Swedish)
    swedish_ref_99_grid = 41,
};

pub const Switch = enum(u8) {
    off = 0,
    on = 1,
    auto = 2,
};

pub const Sport = enum(u8) {
    generic = 0,
    running = 1,
    cycling = 2,
    /// Mulitsport transition
    transition = 3,
    fitness_equipment = 4,
    swimming = 5,
    basketball = 6,
    soccer = 7,
    tennis = 8,
    american_football = 9,
    training = 10,
    walking = 11,
    cross_country_skiing = 12,
    alpine_skiing = 13,
    snowboarding = 14,
    rowing = 15,
    mountaineering = 16,
    hiking = 17,
    multisport = 18,
    paddling = 19,
    flying = 20,
    e_biking = 21,
    motorcycling = 22,
    boating = 23,
    driving = 24,
    golf = 25,
    hang_gliding = 26,
    horseback_riding = 27,
    hunting = 28,
    fishing = 29,
    inline_skating = 30,
    rock_climbing = 31,
    sailing = 32,
    ice_skating = 33,
    sky_diving = 34,
    snowshoeing = 35,
    snowmobiling = 36,
    stand_up_paddleboarding = 37,
    surfing = 38,
    wakeboarding = 39,
    water_skiing = 40,
    kayaking = 41,
    rafting = 42,
    windsurfing = 43,
    kitesurfing = 44,
    tactical = 45,
    jumpmaster = 46,
    boxing = 47,
    floor_climbing = 48,
    baseball = 49,
    diving = 53,
    /// Sport Shooting bits, set here for sport_bits alignment
    shooting = 56,
    winter_sport = 58,
    /// Sailing position, operating manual winches to power boat controls
    grinding = 59,
    hiit = 62,
    video_gaming = 63,
    racket = 64,
    wheelchair_push_walk = 65,
    wheelchair_push_run = 66,
    meditation = 67,
    para_sport = 68,
    disc_golf = 69,
    team_sport = 70,
    cricket = 71,
    rugby = 72,
    hockey = 73,
    lacrosse = 74,
    volleyball = 75,
    water_tubing = 76,
    wakesurfing = 77,
    water_sport = 78,
    archery = 79,
    mixed_martial_arts = 80,
    motor_sports = 81,
    snorkeling = 82,
    dance = 83,
    jump_rope = 84,
    pool_apnea = 85,
    mobility = 86,
    geocaching = 87,
    canoeing = 88,
    /// All is for goals only to include all sports.
    all = 254,
};

/// Bit field corresponding to sport enum type (1 << sport).
pub const SportBits0 = enum(u8) {
    generic = 0x01,
    running = 0x02,
    cycling = 0x04,
    /// Mulitsport transition
    transition = 0x08,
    fitness_equipment = 0x10,
    swimming = 0x20,
    basketball = 0x40,
    soccer = 0x80,
};

/// Bit field corresponding to sport enum type (1 << (sport-8)).
pub const SportBits1 = enum(u8) {
    tennis = 0x01,
    american_football = 0x02,
    training = 0x04,
    walking = 0x08,
    cross_country_skiing = 0x10,
    alpine_skiing = 0x20,
    snowboarding = 0x40,
    rowing = 0x80,
};

/// Bit field corresponding to sport enum type (1 << (sport-16)).
pub const SportBits2 = enum(u8) {
    mountaineering = 0x01,
    hiking = 0x02,
    multisport = 0x04,
    paddling = 0x08,
    flying = 0x10,
    e_biking = 0x20,
    motorcycling = 0x40,
    boating = 0x80,
};

/// Bit field corresponding to sport enum type (1 << (sport-24)).
pub const SportBits3 = enum(u8) {
    driving = 0x01,
    golf = 0x02,
    hang_gliding = 0x04,
    horseback_riding = 0x08,
    hunting = 0x10,
    fishing = 0x20,
    inline_skating = 0x40,
    rock_climbing = 0x80,
};

/// Bit field corresponding to sport enum type (1 << (sport-32)).
pub const SportBits4 = enum(u8) {
    sailing = 0x01,
    ice_skating = 0x02,
    sky_diving = 0x04,
    snowshoeing = 0x08,
    snowmobiling = 0x10,
    stand_up_paddleboarding = 0x20,
    surfing = 0x40,
    wakeboarding = 0x80,
};

/// Bit field corresponding to sport enum type (1 << (sport-40)).
pub const SportBits5 = enum(u8) {
    water_skiing = 0x01,
    kayaking = 0x02,
    rafting = 0x04,
    windsurfing = 0x08,
    kitesurfing = 0x10,
    tactical = 0x20,
    jumpmaster = 0x40,
    boxing = 0x80,
};

/// Bit field corresponding to sport enum type (1 << (sport-48)).
pub const SportBits6 = enum(u8) {
    floor_climbing = 0x01,
};

pub const SubSport = enum(u8) {
    generic = 0,
    /// Run/Fitness Equipment
    treadmill = 1,
    /// Run
    street = 2,
    /// Run
    trail = 3,
    /// Run
    track = 4,
    /// Cycling
    spin = 5,
    /// Cycling/Fitness Equipment
    indoor_cycling = 6,
    /// Cycling
    road = 7,
    /// Cycling
    mountain = 8,
    /// Cycling
    downhill = 9,
    /// Cycling
    recumbent = 10,
    /// Cycling
    cyclocross = 11,
    /// Cycling
    hand_cycling = 12,
    /// Cycling
    track_cycling = 13,
    /// Fitness Equipment
    indoor_rowing = 14,
    /// Fitness Equipment
    elliptical = 15,
    /// Fitness Equipment
    stair_climbing = 16,
    /// Swimming
    lap_swimming = 17,
    /// Swimming
    open_water = 18,
    /// Training
    flexibility_training = 19,
    /// Training
    strength_training = 20,
    /// Tennis
    warm_up = 21,
    /// Tennis
    match = 22,
    /// Tennis
    exercise = 23,
    challenge = 24,
    /// Fitness Equipment
    indoor_skiing = 25,
    /// Training
    cardio_training = 26,
    /// Walking/Fitness Equipment
    indoor_walking = 27,
    /// E-Biking
    e_bike_fitness = 28,
    /// Cycling
    bmx = 29,
    /// Walking
    casual_walking = 30,
    /// Walking
    speed_walking = 31,
    /// Transition
    bike_to_run_transition = 32,
    /// Transition
    run_to_bike_transition = 33,
    /// Transition
    swim_to_bike_transition = 34,
    /// Motorcycling
    atv = 35,
    /// Motorcycling
    motocross = 36,
    /// Alpine Skiing/Snowboarding
    backcountry = 37,
    /// Alpine Skiing/Snowboarding
    resort = 38,
    /// Flying
    rc_drone = 39,
    /// Flying
    wingsuit = 40,
    /// Kayaking/Rafting
    whitewater = 41,
    /// Cross Country Skiing
    skate_skiing = 42,
    /// Training
    yoga = 43,
    /// Fitness Equipment
    pilates = 44,
    /// Run
    indoor_running = 45,
    /// Cycling
    gravel_cycling = 46,
    /// Cycling
    e_bike_mountain = 47,
    /// Cycling
    commuting = 48,
    /// Cycling
    mixed_surface = 49,
    navigate = 50,
    track_me = 51,
    map = 52,
    /// Diving
    single_gas_diving = 53,
    /// Diving
    multi_gas_diving = 54,
    /// Diving
    gauge_diving = 55,
    /// Diving
    apnea_diving = 56,
    /// Diving
    apnea_hunting = 57,
    virtual_activity = 58,
    /// Used for events where participants run, crawl through mud, climb over walls, etc.
    obstacle = 59,
    breathing = 62,
    /// Diving w/ closed circuit rebreather
    ccr_diving = 63,
    /// Sailing
    sail_race = 65,
    /// Generic
    expedition = 66,
    /// Ultramarathon
    ultra = 67,
    /// Climbing
    indoor_climbing = 68,
    /// Climbing
    bouldering = 69,
    /// High Intensity Interval Training
    hiit = 70,
    /// Sailing position, operating manual winches to power boat controls
    indoor_grinding = 71,
    /// Hunting
    hunting_with_dogs = 72,
    /// HIIT
    amrap = 73,
    /// HIIT
    emom = 74,
    /// HIIT
    tabata = 75,
    /// Video Gaming, Cycling, etc.
    esport = 77,
    /// Multisport
    triathlon = 78,
    /// Multisport
    duathlon = 79,
    /// Multisport
    brick = 80,
    /// Multisport
    swim_run = 81,
    /// Multisport
    adventure_race = 82,
    /// DEZL trucker workout training sport
    trucker_workout = 83,
    /// Racket
    pickleball = 84,
    /// Racket
    padel = 85,
    indoor_wheelchair_walk = 86,
    indoor_wheelchair_run = 87,
    indoor_hand_cycling = 88,
    /// Hockey
    field = 90,
    /// Hockey
    ice = 91,
    /// Disc
    ultimate = 92,
    /// Racket
    platform = 93,
    /// Racket
    squash = 94,
    /// Racket
    badminton = 95,
    /// Racket
    racquetball = 96,
    /// Racket
    table_tennis = 97,
    overland = 98,
    /// Generic
    trolling_motor = 99,
    /// Flying
    fly_canopy = 110,
    /// Flying
    fly_paraglide = 111,
    /// Flying
    fly_paramotor = 112,
    /// Flying
    fly_pressurized = 113,
    /// Flying
    fly_navigate = 114,
    /// Flying
    fly_timer = 115,
    /// Flying
    fly_altimeter = 116,
    /// Flying
    fly_wx = 117,
    /// Flying
    fly_vfr = 118,
    /// Flying
    fly_ifr = 119,
    dynamic_apnea = 121,
    /// Cycling
    enduro = 123,
    /// Hiking
    rucking = 124,
    /// Motor sports
    rally = 125,
    /// Multisport
    pool_triathlon = 126,
    /// Cycling
    e_bike_enduro = 127,
    all = 254,
};

pub const SportEvent = enum(u8) {
    uncategorized = 0,
    geocaching = 1,
    fitness = 2,
    recreation = 3,
    race = 4,
    special_event = 5,
    training = 6,
    transportation = 7,
    touring = 8,
};

pub const Activity = enum(u8) {
    manual = 0,
    auto_multi_sport = 1,
};

pub const Intensity = enum(u8) {
    active = 0,
    rest = 1,
    warmup = 2,
    cooldown = 3,
    recovery = 4,
    interval = 5,
    other = 6,
};

pub const SessionTrigger = enum(u8) {
    activity_end = 0,
    /// User changed sport.
    manual = 1,
    /// Auto multi-sport feature is enabled and user pressed lap button to advance session.
    auto_multi_sport = 2,
    /// Auto sport change caused by user linking to fitness equipment.
    fitness_equipment = 3,
};

pub const AutolapTrigger = enum(u8) {
    time = 0,
    distance = 1,
    position_start = 2,
    position_lap = 3,
    position_waypoint = 4,
    position_marked = 5,
    off = 6,
    auto_select = 13,
};

pub const LapTrigger = enum(u8) {
    manual = 0,
    time = 1,
    distance = 2,
    position_start = 3,
    position_lap = 4,
    position_waypoint = 5,
    position_marked = 6,
    session_end = 7,
    fitness_equipment = 8,
};

pub const TimeMode = enum(u8) {
    hour12 = 0,
    /// Does not use a leading zero and has a colon
    hour24 = 1,
    /// Uses a leading zero and does not have a colon
    military = 2,
    hour_12_with_seconds = 3,
    hour_24_with_seconds = 4,
    utc = 5,
};

pub const BacklightMode = enum(u8) {
    off = 0,
    manual = 1,
    key_and_messages = 2,
    auto_brightness = 3,
    smart_notifications = 4,
    key_and_messages_night = 5,
    key_and_messages_and_smart_notifications = 6,
};

pub const DateMode = enum(u8) {
    day_month = 0,
    month_day = 1,
};

/// Timeout in seconds.
pub const BacklightTimeout = enum(u8) {
    /// Backlight stays on forever.
    infinite = 0,
};

pub const Event = enum(u8) {
    /// Group 0. Start / stop_all
    timer = 0,
    /// start / stop
    workout = 3,
    /// Start at beginning of workout. Stop at end of each step.
    workout_step = 4,
    /// stop_all group 0
    power_down = 5,
    /// stop_all group 0
    power_up = 6,
    /// start / stop group 0
    off_course = 7,
    /// Stop at end of each session.
    session = 8,
    /// Stop at end of each lap.
    lap = 9,
    /// marker
    course_point = 10,
    /// marker
    battery = 11,
    /// Group 1. Start at beginning of activity if VP enabled, when VP pace is changed during activity or VP enabled mid activity. stop_disable when VP disabled.
    virtual_partner_pace = 12,
    /// Group 0. Start / stop when in alert condition.
    hr_high_alert = 13,
    /// Group 0. Start / stop when in alert condition.
    hr_low_alert = 14,
    /// Group 0. Start / stop when in alert condition.
    speed_high_alert = 15,
    /// Group 0. Start / stop when in alert condition.
    speed_low_alert = 16,
    /// Group 0. Start / stop when in alert condition.
    cad_high_alert = 17,
    /// Group 0. Start / stop when in alert condition.
    cad_low_alert = 18,
    /// Group 0. Start / stop when in alert condition.
    power_high_alert = 19,
    /// Group 0. Start / stop when in alert condition.
    power_low_alert = 20,
    /// marker
    recovery_hr = 21,
    /// marker
    battery_low = 22,
    /// Group 1. Start if enabled mid activity (not required at start of activity). Stop when duration is reached. stop_disable if disabled.
    time_duration_alert = 23,
    /// Group 1. Start if enabled mid activity (not required at start of activity). Stop when duration is reached. stop_disable if disabled.
    distance_duration_alert = 24,
    /// Group 1. Start if enabled mid activity (not required at start of activity). Stop when duration is reached. stop_disable if disabled.
    calorie_duration_alert = 25,
    /// Group 1.. Stop at end of activity.
    activity = 26,
    /// marker
    fitness_equipment = 27,
    /// Stop at end of each length.
    length = 28,
    /// marker
    user_marker = 32,
    /// marker
    sport_point = 33,
    /// start/stop/marker
    calibration = 36,
    /// marker
    front_gear_change = 42,
    /// marker
    rear_gear_change = 43,
    /// marker
    rider_position_change = 44,
    /// Group 0. Start / stop when in alert condition.
    elev_high_alert = 45,
    /// Group 0. Start / stop when in alert condition.
    elev_low_alert = 46,
    /// marker
    comm_timeout = 47,
    /// marker
    auto_activity_detect = 54,
    /// marker
    dive_alert = 56,
    /// marker
    dive_gas_switched = 57,
    /// marker
    tank_pressure_reserve = 71,
    /// marker
    tank_pressure_critical = 72,
    /// marker
    tank_lost = 73,
    /// start/stop/marker
    radar_threat_alert = 75,
    /// marker
    tank_battery_low = 76,
    /// marker - tank pod has connected
    tank_pod_connected = 81,
    /// marker - tank pod has lost connection
    tank_pod_disconnected = 82,
};

pub const EventType = enum(u8) {
    start = 0,
    stop = 1,
    consecutive_depreciated = 2,
    marker = 3,
    stop_all = 4,
    begin_depreciated = 5,
    end_depreciated = 6,
    end_all_depreciated = 7,
    stop_disable = 8,
    stop_disable_all = 9,
};

/// timer event data
pub const TimerTrigger = enum(u8) {
    manual = 0,
    auto = 1,
    fitness_equipment = 2,
};

/// fitness equipment event data
pub const FitnessEquipmentState = enum(u8) {
    ready = 0,
    in_use = 1,
    paused = 2,
    /// lost connection to fitness equipment
    unknown = 3,
};

pub const Tone = enum(u8) {
    off = 0,
    tone = 1,
    vibrate = 2,
    tone_and_vibrate = 3,
};

pub const Autoscroll = enum(u8) {
    none = 0,
    slow = 1,
    medium = 2,
    fast = 3,
};

pub const ActivityClass = enum(u8) {
    /// 0 to 100
    level = 0x7F,
    level_max = 100,
    athlete = 0x80,
};

pub const HrZoneCalc = enum(u8) {
    custom = 0,
    percent_max_hr = 1,
    percent_hrr = 2,
    percent_lthr = 3,
};

pub const PwrZoneCalc = enum(u8) {
    custom = 0,
    percent_ftp = 1,
};

pub const WktStepDuration = enum(u8) {
    time = 0,
    distance = 1,
    hr_less_than = 2,
    hr_greater_than = 3,
    calories = 4,
    open = 5,
    repeat_until_steps_cmplt = 6,
    repeat_until_time = 7,
    repeat_until_distance = 8,
    repeat_until_calories = 9,
    repeat_until_hr_less_than = 10,
    repeat_until_hr_greater_than = 11,
    repeat_until_power_less_than = 12,
    repeat_until_power_greater_than = 13,
    power_less_than = 14,
    power_greater_than = 15,
    training_peaks_tss = 16,
    repeat_until_power_last_lap_less_than = 17,
    repeat_until_max_power_last_lap_less_than = 18,
    power_3s_less_than = 19,
    power_10s_less_than = 20,
    power_30s_less_than = 21,
    power_3s_greater_than = 22,
    power_10s_greater_than = 23,
    power_30s_greater_than = 24,
    power_lap_less_than = 25,
    power_lap_greater_than = 26,
    repeat_until_training_peaks_tss = 27,
    repetition_time = 28,
    reps = 29,
    time_only = 31,
};

pub const WktStepTarget = enum(u8) {
    speed = 0,
    heart_rate = 1,
    open = 2,
    cadence = 3,
    power = 4,
    grade = 5,
    resistance = 6,
    power_3s = 7,
    power_10s = 8,
    power_30s = 9,
    power_lap = 10,
    swim_stroke = 11,
    speed_lap = 12,
    heart_rate_lap = 13,
};

pub const Goal = enum(u8) {
    time = 0,
    distance = 1,
    calories = 2,
    frequency = 3,
    steps = 4,
    ascent = 5,
    active_minutes = 6,
};

pub const GoalRecurrence = enum(u8) {
    off = 0,
    daily = 1,
    weekly = 2,
    monthly = 3,
    yearly = 4,
    custom = 5,
};

pub const GoalSource = enum(u8) {
    /// Device generated
    auto = 0,
    /// Social network sourced goal
    community = 1,
    /// Manually generated
    user = 2,
};

pub const Schedule = enum(u8) {
    workout = 0,
    course = 1,
};

pub const CoursePoint = enum(u8) {
    generic = 0,
    summit = 1,
    valley = 2,
    water = 3,
    food = 4,
    danger = 5,
    left = 6,
    right = 7,
    straight = 8,
    first_aid = 9,
    fourth_category = 10,
    third_category = 11,
    second_category = 12,
    first_category = 13,
    hors_category = 14,
    sprint = 15,
    left_fork = 16,
    right_fork = 17,
    middle_fork = 18,
    slight_left = 19,
    sharp_left = 20,
    slight_right = 21,
    sharp_right = 22,
    u_turn = 23,
    segment_start = 24,
    segment_end = 25,
    campsite = 27,
    aid_station = 28,
    rest_area = 29,
    /// Used with UpAhead
    general_distance = 30,
    service = 31,
    energy_gel = 32,
    sports_drink = 33,
    mile_marker = 34,
    checkpoint = 35,
    shelter = 36,
    meeting_spot = 37,
    overlook = 38,
    toilet = 39,
    shower = 40,
    gear = 41,
    sharp_curve = 42,
    steep_incline = 43,
    tunnel = 44,
    bridge = 45,
    obstacle = 46,
    crossing = 47,
    store = 48,
    transition = 49,
    navaid = 50,
    transport = 51,
    alert = 52,
    info = 53,
};

pub const Manufacturer = enum(u16) {
    garmin = 1,
    /// Do not use. Used by FR405 for ANTFS man id.
    garmin_fr405_antfs = 2,
    zephyr = 3,
    dayton = 4,
    idt = 5,
    srm = 6,
    quarq = 7,
    ibike = 8,
    saris = 9,
    spark_hk = 10,
    tanita = 11,
    echowell = 12,
    dynastream_oem = 13,
    nautilus = 14,
    dynastream = 15,
    timex = 16,
    metrigear = 17,
    xelic = 18,
    beurer = 19,
    cardiosport = 20,
    a_and_d = 21,
    hmm = 22,
    suunto = 23,
    thita_elektronik = 24,
    gpulse = 25,
    clean_mobile = 26,
    pedal_brain = 27,
    peaksware = 28,
    saxonar = 29,
    lemond_fitness = 30,
    dexcom = 31,
    wahoo_fitness = 32,
    octane_fitness = 33,
    archinoetics = 34,
    the_hurt_box = 35,
    citizen_systems = 36,
    magellan = 37,
    osynce = 38,
    holux = 39,
    concept2 = 40,
    shimano = 41,
    one_giant_leap = 42,
    ace_sensor = 43,
    brim_brothers = 44,
    xplova = 45,
    perception_digital = 46,
    bf1systems = 47,
    pioneer = 48,
    spantec = 49,
    metalogics = 50,
    _4iiiis = 51,
    seiko_epson = 52,
    seiko_epson_oem = 53,
    ifor_powell = 54,
    maxwell_guider = 55,
    star_trac = 56,
    breakaway = 57,
    alatech_technology_ltd = 58,
    mio_technology_europe = 59,
    rotor = 60,
    geonaute = 61,
    id_bike = 62,
    specialized = 63,
    wtek = 64,
    physical_enterprises = 65,
    north_pole_engineering = 66,
    bkool = 67,
    cateye = 68,
    stages_cycling = 69,
    sigmasport = 70,
    tomtom = 71,
    peripedal = 72,
    wattbike = 73,
    moxy = 76,
    ciclosport = 77,
    powerbahn = 78,
    acorn_projects_aps = 79,
    lifebeam = 80,
    bontrager = 81,
    wellgo = 82,
    scosche = 83,
    magura = 84,
    woodway = 85,
    elite = 86,
    nielsen_kellerman = 87,
    dk_city = 88,
    tacx = 89,
    direction_technology = 90,
    magtonic = 91,
    _1partcarbon = 92,
    inside_ride_technologies = 93,
    sound_of_motion = 94,
    stryd = 95,
    /// Indoorcycling Group
    icg = 96,
    MiPulse = 97,
    bsx_athletics = 98,
    look = 99,
    campagnolo_srl = 100,
    body_bike_smart = 101,
    praxisworks = 102,
    /// Limits Technology Ltd.
    limits_technology = 103,
    /// TopAction Technology Inc.
    topaction_technology = 104,
    cosinuss = 105,
    fitcare = 106,
    magene = 107,
    giant_manufacturing_co = 108,
    /// Tigrasport
    tigrasport = 109,
    salutron = 110,
    technogym = 111,
    bryton_sensors = 112,
    latitude_limited = 113,
    soaring_technology = 114,
    igpsport = 115,
    thinkrider = 116,
    gopher_sport = 117,
    waterrower = 118,
    orangetheory = 119,
    inpeak = 120,
    kinetic = 121,
    johnson_health_tech = 122,
    polar_electro = 123,
    seesense = 124,
    nci_technology = 125,
    iqsquare = 126,
    leomo = 127,
    ifit_com = 128,
    coros_byte = 129,
    versa_design = 130,
    chileaf = 131,
    cycplus = 132,
    gravaa_byte = 133,
    sigeyi = 134,
    coospo = 135,
    geoid = 136,
    bosch = 137,
    kyto = 138,
    kinetic_sports = 139,
    decathlon_byte = 140,
    tq_systems = 141,
    tag_heuer = 142,
    keiser_fitness = 143,
    zwift_byte = 144,
    porsche_ep = 145,
    blackbird = 146,
    meilan_byte = 147,
    ezon = 148,
    laisi = 149,
    myzone = 150,
    abawo = 151,
    bafang = 152,
    luhong_technology = 153,
    development = 255,
    healthandlife = 257,
    lezyne = 258,
    scribe_labs = 259,
    zwift = 260,
    watteam = 261,
    recon = 262,
    favero_electronics = 263,
    dynovelo = 264,
    strava = 265,
    /// Amer Sports
    precor = 266,
    bryton = 267,
    sram = 268,
    /// MiTAC Global Corporation (Mio Technology)
    navman = 269,
    /// COBI GmbH
    cobi = 270,
    spivi = 271,
    mio_magellan = 272,
    evesports = 273,
    sensitivus_gauge = 274,
    podoon = 275,
    life_time_fitness = 276,
    /// Falco eMotors Inc.
    falco_e_motors = 277,
    minoura = 278,
    cycliq = 279,
    luxottica = 280,
    trainer_road = 281,
    the_sufferfest = 282,
    fullspeedahead = 283,
    virtualtraining = 284,
    feedbacksports = 285,
    omata = 286,
    vdo = 287,
    magneticdays = 288,
    hammerhead = 289,
    kinetic_by_kurt = 290,
    shapelog = 291,
    dabuziduo = 292,
    jetblack = 293,
    coros = 294,
    virtugo = 295,
    velosense = 296,
    cycligentinc = 297,
    trailforks = 298,
    mahle_ebikemotion = 299,
    nurvv = 300,
    microprogram = 301,
    zone5cloud = 302,
    greenteg = 303,
    yamaha_motors = 304,
    whoop = 305,
    gravaa = 306,
    onelap = 307,
    monark_exercise = 308,
    form = 309,
    decathlon = 310,
    syncros = 311,
    heatup = 312,
    cannondale = 313,
    true_fitness = 314,
    RGT_cycling = 315,
    vasa = 316,
    race_republic = 317,
    fazua = 318,
    oreka_training = 319,
    /// Lishun Electric & Communication
    lsec = 320,
    lululemon_studio = 321,
    shanyue = 322,
    spinning_mda = 323,
    hilldating = 324,
    aero_sensor = 325,
    nike = 326,
    magicshine = 327,
    ictrainer = 328,
    absolute_cycling = 329,
    eo_swimbetter = 330,
    mywhoosh = 331,
    ravemen = 332,
    tektro_racing_products = 333,
    darad_innovation_corporation = 334,
    cycloptim = 335,
    runna = 337,
    zepp = 339,
    peloton = 340,
    carv = 341,
    tissot = 342,
    real_velo = 345,
    wetech = 346,
    jespr = 347,
    huawei = 348,
    gotoes = 349,
    cadence_app = 350,
    actigraphcorp = 5759,
};

pub const GarminProduct = enum(u16) {
    hrm1 = 1,
    /// AXH01 HRM chipset
    axh01 = 2,
    axb01 = 3,
    axb02 = 4,
    hrm2ss = 5,
    dsi_alf02 = 6,
    hrm3ss = 7,
    /// hrm_run model for HRM ANT+ messaging
    hrm_run_single_byte_product_id = 8,
    /// BSM model for ANT+ messaging
    bsm = 9,
    /// BCM model for ANT+ messaging
    bcm = 10,
    /// AXS01 HRM Bike Chipset model for ANT+ messaging
    axs01 = 11,
    /// hrm_tri model for HRM ANT+ messaging
    hrm_tri_single_byte_product_id = 12,
    /// hrm4 run model for HRM ANT+ messaging
    hrm4_run_single_byte_product_id = 13,
    /// fr225 model for HRM ANT+ messaging
    fr225_single_byte_product_id = 14,
    /// gen3_bsm model for Bike Speed ANT+ messaging
    gen3_bsm_single_byte_product_id = 15,
    /// gen3_bcm model for Bike Cadence ANT+ messaging
    gen3_bcm_single_byte_product_id = 16,
    hrm_fit_single_byte_product_id = 22,
    /// Garmin Wearable Optical Heart Rate Sensor for ANT+ HR Profile Broadcasting
    OHR = 255,
    fr301_china = 473,
    fr301_japan = 474,
    fr301_korea = 475,
    fr301_taiwan = 494,
    /// Forerunner 405
    fr405 = 717,
    /// Forerunner 50
    fr50 = 782,
    fr405_japan = 987,
    /// Forerunner 60
    fr60 = 988,
    dsi_alf01 = 1011,
    /// Forerunner 310
    fr310xt = 1018,
    edge500 = 1036,
    /// Forerunner 110
    fr110 = 1124,
    edge800 = 1169,
    edge500_taiwan = 1199,
    edge500_japan = 1213,
    chirp = 1253,
    fr110_japan = 1274,
    edge200 = 1325,
    fr910xt = 1328,
    edge800_taiwan = 1333,
    edge800_japan = 1334,
    alf04 = 1341,
    fr610 = 1345,
    fr210_japan = 1360,
    vector_ss = 1380,
    vector_cp = 1381,
    edge800_china = 1386,
    edge500_china = 1387,
    approach_g10 = 1405,
    fr610_japan = 1410,
    edge500_korea = 1422,
    fr70 = 1436,
    fr310xt_4t = 1446,
    amx = 1461,
    fr10 = 1482,
    edge800_korea = 1497,
    swim = 1499,
    fr910xt_china = 1537,
    fenix = 1551,
    edge200_taiwan = 1555,
    edge510 = 1561,
    edge810 = 1567,
    tempe = 1570,
    fr910xt_japan = 1600,
    fr620 = 1623,
    fr220 = 1632,
    fr910xt_korea = 1664,
    fr10_japan = 1688,
    edge810_japan = 1721,
    virb_elite = 1735,
    /// Also Edge Touring Plus
    edge_touring = 1736,
    edge510_japan = 1742,
    /// Also HRM-Swim
    hrm_tri = 1743,
    hrm_run = 1752,
    fr920xt = 1765,
    edge510_asia = 1821,
    edge810_china = 1822,
    edge810_taiwan = 1823,
    edge1000 = 1836,
    vivo_fit = 1837,
    virb_remote = 1853,
    vivo_ki = 1885,
    fr15 = 1903,
    vivo_active = 1907,
    edge510_korea = 1918,
    fr620_japan = 1928,
    fr620_china = 1929,
    fr220_japan = 1930,
    fr220_china = 1931,
    approach_s6 = 1936,
    vivo_smart = 1956,
    fenix2 = 1967,
    epix = 1988,
    fenix3 = 2050,
    edge1000_taiwan = 2052,
    edge1000_japan = 2053,
    fr15_japan = 2061,
    edge520 = 2067,
    edge1000_china = 2070,
    fr620_russia = 2072,
    fr220_russia = 2073,
    vector_s = 2079,
    edge1000_korea = 2100,
    fr920xt_taiwan = 2130,
    fr920xt_china = 2131,
    fr920xt_japan = 2132,
    virbx = 2134,
    vivo_smart_apac = 2135,
    etrex_touch = 2140,
    edge25 = 2147,
    fr25 = 2148,
    vivo_fit2 = 2150,
    fr225 = 2153,
    fr630 = 2156,
    fr230 = 2157,
    fr735xt = 2158,
    vivo_active_apac = 2160,
    vector_2 = 2161,
    vector_2s = 2162,
    virbxe = 2172,
    fr620_taiwan = 2173,
    fr220_taiwan = 2174,
    truswing = 2175,
    d2airvenu = 2187,
    fenix3_china = 2188,
    fenix3_twn = 2189,
    varia_headlight = 2192,
    varia_taillight_old = 2193,
    edge_explore_1000 = 2204,
    fr225_asia = 2219,
    varia_radar_taillight = 2225,
    varia_radar_display = 2226,
    edge20 = 2238,
    edge520_asia = 2260,
    edge520_japan = 2261,
    d2_bravo = 2262,
    approach_s20 = 2266,
    vivo_smart2 = 2271,
    edge1000_thai = 2274,
    varia_remote = 2276,
    edge25_asia = 2288,
    edge25_jpn = 2289,
    edge20_asia = 2290,
    approach_x40 = 2292,
    fenix3_japan = 2293,
    vivo_smart_emea = 2294,
    fr630_asia = 2310,
    fr630_jpn = 2311,
    fr230_jpn = 2313,
    hrm4_run = 2327,
    epix_japan = 2332,
    vivo_active_hr = 2337,
    vivo_smart_gps_hr = 2347,
    vivo_smart_hr = 2348,
    vivo_smart_hr_asia = 2361,
    vivo_smart_gps_hr_asia = 2362,
    vivo_move = 2368,
    varia_taillight = 2379,
    fr235_asia = 2396,
    fr235_japan = 2397,
    varia_vision = 2398,
    vivo_fit3 = 2406,
    fenix3_korea = 2407,
    fenix3_sea = 2408,
    fenix3_hr = 2413,
    virb_ultra_30 = 2417,
    index_smart_scale = 2429,
    fr235 = 2431,
    fenix3_chronos = 2432,
    oregon7xx = 2441,
    rino7xx = 2444,
    epix_korea = 2457,
    fenix3_hr_chn = 2473,
    fenix3_hr_twn = 2474,
    fenix3_hr_jpn = 2475,
    fenix3_hr_sea = 2476,
    fenix3_hr_kor = 2477,
    nautix = 2496,
    vivo_active_hr_apac = 2497,
    fr35 = 2503,
    oregon7xx_ww = 2512,
    edge_820 = 2530,
    edge_explore_820 = 2531,
    fr735xt_apac = 2533,
    fr735xt_japan = 2534,
    fenix5s = 2544,
    d2_bravo_titanium = 2547,
    /// Varia UT 800 SW
    varia_ut800 = 2567,
    running_dynamics_pod = 2593,
    edge_820_china = 2599,
    edge_820_japan = 2600,
    fenix5x = 2604,
    vivo_fit_jr = 2606,
    vivo_smart3 = 2622,
    vivo_sport = 2623,
    edge_820_taiwan = 2628,
    edge_820_korea = 2629,
    edge_820_sea = 2630,
    fr35_hebrew = 2650,
    approach_s60 = 2656,
    fr35_apac = 2667,
    fr35_japan = 2668,
    fenix3_chronos_asia = 2675,
    virb_360 = 2687,
    fr935 = 2691,
    fenix5 = 2697,
    vivoactive3 = 2700,
    fr235_china_nfc = 2733,
    foretrex_601_701 = 2769,
    vivo_move_hr = 2772,
    edge_1030 = 2713,
    fr35_sea = 2727,
    vector_3 = 2787,
    fenix5_asia = 2796,
    fenix5s_asia = 2797,
    fenix5x_asia = 2798,
    approach_z80 = 2806,
    fr35_korea = 2814,
    d2charlie = 2819,
    vivo_smart3_apac = 2831,
    vivo_sport_apac = 2832,
    fr935_asia = 2833,
    descent = 2859,
    vivo_fit4 = 2878,
    fr645 = 2886,
    fr645m = 2888,
    fr30 = 2891,
    fenix5s_plus = 2900,
    Edge_130 = 2909,
    edge_1030_asia = 2924,
    vivosmart_4 = 2927,
    vivo_move_hr_asia = 2945,
    approach_x10 = 2962,
    fr30_asia = 2977,
    vivoactive3m_w = 2988,
    fr645_asia = 3003,
    fr645m_asia = 3004,
    edge_explore = 3011,
    gpsmap66 = 3028,
    approach_s10 = 3049,
    vivoactive3m_l = 3066,
    fr245 = 3076,
    fr245_music = 3077,
    approach_g80 = 3085,
    edge_130_asia = 3092,
    edge_1030_bontrager = 3095,
    fenix5_plus = 3110,
    fenix5x_plus = 3111,
    edge_520_plus = 3112,
    fr945 = 3113,
    edge_530 = 3121,
    edge_830 = 3122,
    instinct_esports = 3126,
    fenix5s_plus_apac = 3134,
    fenix5x_plus_apac = 3135,
    edge_520_plus_apac = 3142,
    descent_t1 = 3143,
    fr235l_asia = 3144,
    fr245_asia = 3145,
    vivo_active3m_apac = 3163,
    /// gen3 bike speed sensor
    gen3_bsm = 3192,
    /// gen3 bike cadence sensor
    gen3_bcm = 3193,
    vivo_smart4_asia = 3218,
    vivoactive4_small = 3224,
    vivoactive4_large = 3225,
    venu = 3226,
    marq_driver = 3246,
    marq_aviator = 3247,
    marq_captain = 3248,
    marq_commander = 3249,
    marq_expedition = 3250,
    marq_athlete = 3251,
    descent_mk2 = 3258,
    fr45 = 3282,
    gpsmap66i = 3284,
    fenix6S_sport = 3287,
    fenix6S = 3288,
    fenix6_sport = 3289,
    fenix6 = 3290,
    fenix6x = 3291,
    /// HRM-Dual
    hrm_dual = 3299,
    /// HRM-Pro
    hrm_pro = 3300,
    vivo_move3_premium = 3308,
    approach_s40 = 3314,
    fr245m_asia = 3321,
    edge_530_apac = 3349,
    edge_830_apac = 3350,
    vivo_move3 = 3378,
    vivo_active4_small_asia = 3387,
    vivo_active4_large_asia = 3388,
    vivo_active4_oled_asia = 3389,
    swim2 = 3405,
    marq_driver_asia = 3420,
    marq_aviator_asia = 3421,
    vivo_move3_asia = 3422,
    fr945_asia = 3441,
    vivo_active3t_chn = 3446,
    marq_captain_asia = 3448,
    marq_commander_asia = 3449,
    marq_expedition_asia = 3450,
    marq_athlete_asia = 3451,
    index_smart_scale_2 = 3461,
    instinct_solar = 3466,
    fr45_asia = 3469,
    vivoactive3_daimler = 3473,
    legacy_rey = 3498,
    legacy_darth_vader = 3499,
    legacy_captain_marvel = 3500,
    legacy_first_avenger = 3501,
    fenix6s_sport_asia = 3512,
    fenix6s_asia = 3513,
    fenix6_sport_asia = 3514,
    fenix6_asia = 3515,
    fenix6x_asia = 3516,
    legacy_captain_marvel_asia = 3535,
    legacy_first_avenger_asia = 3536,
    legacy_rey_asia = 3537,
    legacy_darth_vader_asia = 3538,
    descent_mk2s = 3542,
    edge_130_plus = 3558,
    edge_1030_plus = 3570,
    /// Rally 100/200 Power Meter Series
    rally_200 = 3578,
    fr745 = 3589,
    venusq_music = 3596,
    venusq_music_v2 = 3599,
    venusq = 3600,
    lily = 3615,
    marq_adventurer = 3624,
    enduro = 3638,
    swim2_apac = 3639,
    marq_adventurer_asia = 3648,
    fr945_lte = 3652,
    /// Mk2 and Mk2i
    descent_mk2_asia = 3702,
    venu2 = 3703,
    venu2s = 3704,
    venu_daimler_asia = 3737,
    marq_golfer = 3739,
    venu_daimler = 3740,
    fr745_asia = 3794,
    varia_rct715 = 3808,
    lily_asia = 3809,
    edge_1030_plus_asia = 3812,
    edge_130_plus_asia = 3813,
    approach_s12 = 3823,
    enduro_asia = 3872,
    venusq_asia = 3837,
    edge_1040 = 3843,
    marq_golfer_asia = 3850,
    venu2_plus = 3851,
    /// Airoha AG3335M Family
    gnss = 3865,
    fr55 = 3869,
    instinct_2 = 3888,
    instinct_2s = 3889,
    fenix7s = 3905,
    fenix7 = 3906,
    fenix7x = 3907,
    fenix7s_apac = 3908,
    fenix7_apac = 3909,
    fenix7x_apac = 3910,
    approach_g12 = 3927,
    descent_mk2s_asia = 3930,
    approach_s42 = 3934,
    epix_gen2 = 3943,
    epix_gen2_apac = 3944,
    venu2s_asia = 3949,
    venu2_asia = 3950,
    fr945_lte_asia = 3978,
    vivo_move_sport = 3982,
    vivomove_trend = 3983,
    approach_S12_asia = 3986,
    fr255_music = 3990,
    fr255_small_music = 3991,
    fr255 = 3992,
    fr255_small = 3993,
    approach_g12_asia = 4001,
    approach_s42_asia = 4002,
    descent_g1 = 4005,
    venu2_plus_asia = 4017,
    fr955 = 4024,
    fr55_asia = 4033,
    edge_540 = 4061,
    edge_840 = 4062,
    vivosmart_5 = 4063,
    instinct_2_asia = 4071,
    /// Adventurer, Athlete, Captain, Golfer
    marq_gen2 = 4105,
    venusq2 = 4115,
    venusq2music = 4116,
    marq_gen2_aviator = 4124,
    d2_air_x10 = 4125,
    hrm_pro_plus = 4130,
    descent_g1_asia = 4132,
    tactix7 = 4135,
    instinct_crossover = 4155,
    edge_explore2 = 4169,
    descent_mk3 = 4222,
    descent_mk3i = 4223,
    approach_s70 = 4233,
    fr265_large = 4257,
    fr265_small = 4258,
    venu3 = 4260,
    venu3s = 4261,
    /// Neo Smart, Tacx
    tacx_neo_smart = 4265,
    /// Neo 2 Smart, Tacx
    tacx_neo2_smart = 4266,
    /// Neo 2T Smart, Tacx
    tacx_neo2_t_smart = 4267,
    /// Neo Smart Bike, Tacx
    tacx_neo_smart_bike = 4268,
    /// Satori Smart, Tacx
    tacx_satori_smart = 4269,
    /// Flow Smart, Tacx
    tacx_flow_smart = 4270,
    /// Vortex Smart, Tacx
    tacx_vortex_smart = 4271,
    /// Bushido Smart, Tacx
    tacx_bushido_smart = 4272,
    /// Genius Smart, Tacx
    tacx_genius_smart = 4273,
    /// Flux/Flux S Smart, Tacx
    tacx_flux_flux_s_smart = 4274,
    /// Flux 2 Smart, Tacx
    tacx_flux2_smart = 4275,
    /// Magnum, Tacx
    tacx_magnum = 4276,
    edge_1040_asia = 4305,
    epix_gen2_pro_42 = 4312,
    epix_gen2_pro_47 = 4313,
    epix_gen2_pro_51 = 4314,
    fr965 = 4315,
    enduro2 = 4341,
    fenix7s_pro_solar = 4374,
    fenix7_pro_solar = 4375,
    fenix7x_pro_solar = 4376,
    lily2 = 4380,
    instinct_2x = 4394,
    vivoactive5 = 4426,
    fr165 = 4432,
    fr165_music = 4433,
    edge_1050 = 4440,
    descent_t2 = 4442,
    hrm_fit = 4446,
    marq_gen2_commander = 4472,
    /// aka the Lily 2 Active
    lily_athlete = 4477,
    /// Rally 110/210
    rally_x10 = 4525,
    fenix8_solar = 4532,
    fenix8_solar_large = 4533,
    fenix8_small = 4534,
    fenix8 = 4536,
    d2_mach1_pro = 4556,
    enduro3 = 4575,
    instinctE_40mm = 4583,
    instinctE_45mm = 4584,
    instinct3_solar_45mm = 4585,
    instinct3_amoled_45mm = 4586,
    instinct3_amoled_50mm = 4587,
    descent_g2 = 4588,
    venu_x1 = 4603,
    hrm_200 = 4606,
    vivoactive6 = 4625,
    fenix8_pro = 4631,
    edge_550 = 4633,
    edge_850 = 4634,
    venu4 = 4643,
    venu4s = 4644,
    approachS44 = 4647,
    edge_mtb = 4655,
    approachS50 = 4656,
    fenix_e = 4666,
    bounce2 = 4745,
    instinct3_solar_50mm = 4759,
    tactix8_amoled = 4775,
    tactix8_solar = 4776,
    fr170_music = 4814,
    fr170 = 4815,
    approach_j1 = 4825,
    d2_mach2 = 4879,
    fr70_2026 = 4916,
    instinct_crossover_amoled = 4678,
    d2_air_x15 = 4944,
    d2_mach2_pro = 5056,
    /// SDM4 footpod
    sdm4 = 10007,
    edge_remote = 10014,
    tacx_training_app_win = 20533,
    tacx_training_app_mac = 20534,
    tacx_training_app_mac_catalyst = 20565,
    training_center = 20119,
    tacx_training_app_android = 30045,
    tacx_training_app_ios = 30046,
    tacx_training_app_legacy = 30047,
    connectiq_simulator = 65531,
    android_antplus_plugin = 65532,
    /// Garmin Connect website
    connect = 65534,
};

pub const AntplusDeviceType = enum(u8) {
    antfs = 1,
    bike_power = 11,
    environment_sensor_legacy = 12,
    multi_sport_speed_distance = 15,
    control = 16,
    fitness_equipment = 17,
    blood_pressure = 18,
    geocache_node = 19,
    light_electric_vehicle = 20,
    env_sensor = 25,
    racquet = 26,
    control_hub = 27,
    muscle_oxygen = 31,
    shifting = 34,
    bike_light_main = 35,
    bike_light_shared = 36,
    exd = 38,
    bike_radar = 40,
    bike_aero = 46,
    weight_scale = 119,
    heart_rate = 120,
    bike_speed_cadence = 121,
    bike_cadence = 122,
    bike_speed = 123,
    stride_speed_distance = 124,
};

pub const AntNetwork = enum(u8) {
    public = 0,
    antplus = 1,
    antfs = 2,
    private = 3,
};

pub const WorkoutCapabilities = enum(u32) {
    interval = 0x00000001,
    custom = 0x00000002,
    fitness_equipment = 0x00000004,
    firstbeat = 0x00000008,
    new_leaf = 0x00000010,
    /// For backwards compatibility. Watch should add missing id fields then clear flag.
    tcx = 0x00000020,
    /// Speed source required for workout step.
    speed = 0x00000080,
    /// Heart rate source required for workout step.
    heart_rate = 0x00000100,
    /// Distance source required for workout step.
    distance = 0x00000200,
    /// Cadence source required for workout step.
    cadence = 0x00000400,
    /// Power source required for workout step.
    power = 0x00000800,
    /// Grade source required for workout step.
    grade = 0x00001000,
    /// Resistance source required for workout step.
    resistance = 0x00002000,
    protected = 0x00004000,
};

pub const BatteryStatus = enum(u8) {
    new = 1,
    good = 2,
    ok = 3,
    low = 4,
    critical = 5,
    charging = 6,
    unknown = 7,
};

pub const HrType = enum(u8) {
    normal = 0,
    irregular = 1,
};

pub const CourseCapabilities = enum(u32) {
    processed = 0x00000001,
    valid = 0x00000002,
    time = 0x00000004,
    distance = 0x00000008,
    position = 0x00000010,
    heart_rate = 0x00000020,
    power = 0x00000040,
    cadence = 0x00000080,
    training = 0x00000100,
    navigation = 0x00000200,
    bikeway = 0x00000400,
    /// Denote course files to be used as flight plans
    aviation = 0x00001000,
};

pub const Weight = enum(u16) {
    calculating = 0xFFFE,
};

/// 0 - 100 indicates% of max hr; >100 indicates bpm (255 max) plus 100
pub const WorkoutHr = enum(u32) {
    bpm_offset = 100,
};

/// 0 - 1000 indicates % of functional threshold power; >1000 indicates watts plus 1000.
pub const WorkoutPower = enum(u32) {
    watts_offset = 1000,
};

pub const BpStatus = enum(u8) {
    no_error = 0,
    error_incomplete_data = 1,
    error_no_measurement = 2,
    error_data_out_of_range = 3,
    error_irregular_heart_rate = 4,
};

pub const UserLocalId = enum(u16) {
    local_min = 0x0000,
    local_max = 0x000F,
    stationary_min = 0x0010,
    stationary_max = 0x00FF,
    portable_min = 0x0100,
    portable_max = 0xFFFE,
};

pub const SwimStroke = enum(u8) {
    freestyle = 0,
    backstroke = 1,
    breaststroke = 2,
    butterfly = 3,
    drill = 4,
    mixed = 5,
    /// IM is a mixed interval containing the same number of lengths for each of: Butterfly, Backstroke, Breaststroke, Freestyle, swam in that order.
    im = 6,
    /// For repeated workout steps, a new individual medly stroke is used for each round.
    im_by_round = 7,
    /// Reverse IM Order
    rimo = 8,
};

pub const ActivityType = enum(u8) {
    generic = 0,
    running = 1,
    cycling = 2,
    /// Mulitsport transition
    transition = 3,
    fitness_equipment = 4,
    swimming = 5,
    walking = 6,
    sedentary = 8,
    /// All is for goals only to include all sports.
    all = 254,
};

pub const ActivitySubtype = enum(u8) {
    generic = 0,
    /// Run
    treadmill = 1,
    /// Run
    street = 2,
    /// Run
    trail = 3,
    /// Run
    track = 4,
    /// Cycling
    spin = 5,
    /// Cycling
    indoor_cycling = 6,
    /// Cycling
    road = 7,
    /// Cycling
    mountain = 8,
    /// Cycling
    downhill = 9,
    /// Cycling
    recumbent = 10,
    /// Cycling
    cyclocross = 11,
    /// Cycling
    hand_cycling = 12,
    /// Cycling
    track_cycling = 13,
    /// Fitness Equipment
    indoor_rowing = 14,
    /// Fitness Equipment
    elliptical = 15,
    /// Fitness Equipment
    stair_climbing = 16,
    /// Swimming
    lap_swimming = 17,
    /// Swimming
    open_water = 18,
    all = 254,
};

pub const ActivityLevel = enum(u8) {
    low = 0,
    medium = 1,
    high = 2,
};

pub const Side = enum(u8) {
    right = 0,
    left = 1,
};

pub const LeftRightBalance = enum(u8) {
    /// % contribution
    mask = 0x7F,
    /// data corresponds to right if set, otherwise unknown
    right = 0x80,
};

pub const LeftRightBalance100 = enum(u16) {
    /// % contribution scaled by 100
    mask = 0x3FFF,
    /// data corresponds to right if set, otherwise unknown
    right = 0x8000,
};

pub const LengthType = enum(u8) {
    /// Rest period. Length with no strokes
    idle = 0,
    /// Length with strokes.
    active = 1,
};

pub const DayOfWeek = enum(u8) {
    sunday = 0,
    monday = 1,
    tuesday = 2,
    wednesday = 3,
    thursday = 4,
    friday = 5,
    saturday = 6,
};

pub const ConnectivityCapabilities = enum(u32) {
    bluetooth = 0x00000001,
    bluetooth_le = 0x00000002,
    ant = 0x00000004,
    activity_upload = 0x00000008,
    course_download = 0x00000010,
    workout_download = 0x00000020,
    live_track = 0x00000040,
    weather_conditions = 0x00000080,
    weather_alerts = 0x00000100,
    gps_ephemeris_download = 0x00000200,
    explicit_archive = 0x00000400,
    setup_incomplete = 0x00000800,
    continue_sync_after_software_update = 0x00001000,
    connect_iq_app_download = 0x00002000,
    golf_course_download = 0x00004000,
    /// Indicates device is in control of initiating all syncs
    device_initiates_sync = 0x00008000,
    connect_iq_watch_app_download = 0x00010000,
    connect_iq_widget_download = 0x00020000,
    connect_iq_watch_face_download = 0x00040000,
    connect_iq_data_field_download = 0x00080000,
    /// Device supports delete and reorder of apps via GCM
    connect_iq_app_managment = 0x00100000,
    swing_sensor = 0x00200000,
    swing_sensor_remote = 0x00400000,
    /// Device supports incident detection
    incident_detection = 0x00800000,
    audio_prompts = 0x01000000,
    /// Device supports reporting wifi verification via GCM
    wifi_verification = 0x02000000,
    /// Device supports True Up
    true_up = 0x04000000,
    /// Device supports Find My Watch
    find_my_watch = 0x08000000,
    remote_manual_sync = 0x10000000,
    /// Device supports LiveTrack auto start
    live_track_auto_start = 0x20000000,
    /// Device supports LiveTrack Messaging
    live_track_messaging = 0x40000000,
    /// Device supports instant input feature
    instant_input = 0x80000000,
};

pub const WeatherReport = enum(u8) {
    current = 0,
    /// Deprecated use hourly_forecast instead
    forecast = 1,
    hourly_forecast = 1,
    daily_forecast = 2,
};

pub const WeatherStatus = enum(u8) {
    clear = 0,
    partly_cloudy = 1,
    mostly_cloudy = 2,
    rain = 3,
    snow = 4,
    windy = 5,
    thunderstorms = 6,
    wintry_mix = 7,
    fog = 8,
    hazy = 11,
    hail = 12,
    scattered_showers = 13,
    scattered_thunderstorms = 14,
    unknown_precipitation = 15,
    light_rain = 16,
    heavy_rain = 17,
    light_snow = 18,
    heavy_snow = 19,
    light_rain_snow = 20,
    heavy_rain_snow = 21,
    cloudy = 22,
};

pub const WeatherSeverity = enum(u8) {
    unknown = 0,
    warning = 1,
    watch = 2,
    advisory = 3,
    statement = 4,
};

pub const WeatherSevereType = enum(u8) {
    unspecified = 0,
    tornado = 1,
    tsunami = 2,
    hurricane = 3,
    extreme_wind = 4,
    typhoon = 5,
    inland_hurricane = 6,
    hurricane_force_wind = 7,
    waterspout = 8,
    severe_thunderstorm = 9,
    wreckhouse_winds = 10,
    les_suetes_wind = 11,
    avalanche = 12,
    flash_flood = 13,
    tropical_storm = 14,
    inland_tropical_storm = 15,
    blizzard = 16,
    ice_storm = 17,
    freezing_rain = 18,
    debris_flow = 19,
    flash_freeze = 20,
    dust_storm = 21,
    high_wind = 22,
    winter_storm = 23,
    heavy_freezing_spray = 24,
    extreme_cold = 25,
    wind_chill = 26,
    cold_wave = 27,
    heavy_snow_alert = 28,
    lake_effect_blowing_snow = 29,
    snow_squall = 30,
    lake_effect_snow = 31,
    winter_weather = 32,
    sleet = 33,
    snowfall = 34,
    snow_and_blowing_snow = 35,
    blowing_snow = 36,
    snow_alert = 37,
    arctic_outflow = 38,
    freezing_drizzle = 39,
    storm = 40,
    storm_surge = 41,
    rainfall = 42,
    areal_flood = 43,
    coastal_flood = 44,
    lakeshore_flood = 45,
    excessive_heat = 46,
    heat = 47,
    weather = 48,
    high_heat_and_humidity = 49,
    humidex_and_health = 50,
    humidex = 51,
    gale = 52,
    freezing_spray = 53,
    special_marine = 54,
    squall = 55,
    strong_wind = 56,
    lake_wind = 57,
    marine_weather = 58,
    wind = 59,
    small_craft_hazardous_seas = 60,
    hazardous_seas = 61,
    small_craft = 62,
    small_craft_winds = 63,
    small_craft_rough_bar = 64,
    high_water_level = 65,
    ashfall = 66,
    freezing_fog = 67,
    dense_fog = 68,
    dense_smoke = 69,
    blowing_dust = 70,
    hard_freeze = 71,
    freeze = 72,
    frost = 73,
    fire_weather = 74,
    flood = 75,
    rip_tide = 76,
    high_surf = 77,
    smog = 78,
    air_quality = 79,
    brisk_wind = 80,
    air_stagnation = 81,
    low_water = 82,
    hydrological = 83,
    special_weather = 84,
};

/// number of seconds into the day since 00:00:00 UTC
pub const TimeIntoDay = enum(u32) {};

/// number of seconds into the day since local 00:00:00
pub const LocaltimeIntoDay = enum(u32) {};

pub const StrokeType = enum(u8) {
    no_event = 0,
    /// stroke was detected but cannot be identified
    other = 1,
    serve = 2,
    forehand = 3,
    backhand = 4,
    smash = 5,
};

pub const BodyLocation = enum(u8) {
    left_leg = 0,
    left_calf = 1,
    left_shin = 2,
    left_hamstring = 3,
    left_quad = 4,
    left_glute = 5,
    right_leg = 6,
    right_calf = 7,
    right_shin = 8,
    right_hamstring = 9,
    right_quad = 10,
    right_glute = 11,
    torso_back = 12,
    left_lower_back = 13,
    left_upper_back = 14,
    right_lower_back = 15,
    right_upper_back = 16,
    torso_front = 17,
    left_abdomen = 18,
    left_chest = 19,
    right_abdomen = 20,
    right_chest = 21,
    left_arm = 22,
    left_shoulder = 23,
    left_bicep = 24,
    left_tricep = 25,
    /// Left anterior forearm
    left_brachioradialis = 26,
    /// Left posterior forearm
    left_forearm_extensors = 27,
    right_arm = 28,
    right_shoulder = 29,
    right_bicep = 30,
    right_tricep = 31,
    /// Right anterior forearm
    right_brachioradialis = 32,
    /// Right posterior forearm
    right_forearm_extensors = 33,
    neck = 34,
    throat = 35,
    waist_mid_back = 36,
    waist_front = 37,
    waist_left = 38,
    waist_right = 39,
};

pub const SegmentLapStatus = enum(u8) {
    end = 0,
    fail = 1,
};

pub const SegmentLeaderboardType = enum(u8) {
    overall = 0,
    personal_best = 1,
    connections = 2,
    group = 3,
    challenger = 4,
    kom = 5,
    qom = 6,
    pr = 7,
    goal = 8,
    carrot = 9,
    club_leader = 10,
    rival = 11,
    last = 12,
    recent_best = 13,
    course_record = 14,
};

pub const SegmentDeleteStatus = enum(u8) {
    do_not_delete = 0,
    delete_one = 1,
    delete_all = 2,
};

pub const SegmentSelectionType = enum(u8) {
    starred = 0,
    suggested = 1,
};

pub const SourceType = enum(u8) {
    /// External device connected with ANT
    ant = 0,
    /// External device connected with ANT+
    antplus = 1,
    /// External device connected with BT
    bluetooth = 2,
    /// External device connected with BLE
    bluetooth_low_energy = 3,
    /// External device connected with Wifi
    wifi = 4,
    /// Onboard device
    local = 5,
};

pub const LocalDeviceType = enum(u8) {
    /// Onboard gps receiver
    gps = 0,
    /// Onboard glonass receiver
    glonass = 1,
    /// Onboard gps glonass receiver
    gps_glonass = 2,
    /// Onboard sensor
    accelerometer = 3,
    /// Onboard sensor
    barometer = 4,
    /// Onboard sensor
    temperature = 5,
    /// Onboard wrist HR sensor
    whr = 10,
    /// Onboard software package
    sensor_hub = 12,
};

pub const BleDeviceType = enum(u8) {
    /// GPS that is provided over a proprietary bluetooth service
    connected_gps = 0,
    heart_rate = 1,
    bike_power = 2,
    bike_speed_cadence = 3,
    bike_speed = 4,
    bike_cadence = 5,
    footpod = 6,
    /// Indoor-Bike FTMS protocol
    bike_trainer = 7,
};

pub const AntChannelId = enum(u32) {
    ant_extended_device_number_upper_nibble = 0xF0000000,
    ant_transmission_type_lower_nibble = 0x0F000000,
    ant_device_type = 0x00FF0000,
    ant_device_number = 0x0000FFFF,
};

pub const DisplayOrientation = enum(u8) {
    /// automatic if the device supports it
    auto = 0,
    portrait = 1,
    landscape = 2,
    /// portrait mode but rotated 180 degrees
    portrait_flipped = 3,
    /// landscape mode but rotated 180 degrees
    landscape_flipped = 4,
};

pub const WorkoutEquipment = enum(u8) {
    none = 0,
    swim_fins = 1,
    swim_kickboard = 2,
    swim_paddles = 3,
    swim_pull_buoy = 4,
    swim_snorkel = 5,
};

pub const WatchfaceMode = enum(u8) {
    digital = 0,
    analog = 1,
    connect_iq = 2,
    disabled = 3,
};

pub const DigitalWatchfaceLayout = enum(u8) {
    traditional = 0,
    modern = 1,
    bold = 2,
};

pub const AnalogWatchfaceLayout = enum(u8) {
    minimal = 0,
    traditional = 1,
    modern = 2,
};

pub const RiderPositionType = enum(u8) {
    seated = 0,
    standing = 1,
    transition_to_seated = 2,
    transition_to_standing = 3,
};

pub const PowerPhaseType = enum(u8) {
    power_phase_start_angle = 0,
    power_phase_end_angle = 1,
    power_phase_arc_length = 2,
    power_phase_center = 3,
};

pub const CameraEventType = enum(u8) {
    /// Start of video recording
    video_start = 0,
    /// Mark of video file split (end of one file, beginning of the other)
    video_split = 1,
    /// End of video recording
    video_end = 2,
    /// Still photo taken
    photo_taken = 3,
    video_second_stream_start = 4,
    video_second_stream_split = 5,
    video_second_stream_end = 6,
    /// Mark of video file split start
    video_split_start = 7,
    video_second_stream_split_start = 8,
    /// Mark when a video recording has been paused
    video_pause = 11,
    video_second_stream_pause = 12,
    /// Mark when a video recording has been resumed
    video_resume = 13,
    video_second_stream_resume = 14,
};

pub const SensorType = enum(u8) {
    accelerometer = 0,
    gyroscope = 1,
    /// Magnetometer
    compass = 2,
    barometer = 3,
};

pub const BikeLightNetworkConfigType = enum(u8) {
    auto = 0,
    individual = 4,
    high_visibility = 5,
    trail = 6,
};

pub const CommTimeoutType = enum(u16) {
    /// Timeout pairing to any device
    wildcard_pairing_timeout = 0,
    /// Timeout pairing to previously paired device
    pairing_timeout = 1,
    /// Temporary loss of communications
    connection_lost = 2,
    /// Connection closed due to extended bad communications
    connection_timeout = 3,
};

pub const CameraOrientationType = enum(u8) {
    camera_orientation_0 = 0,
    camera_orientation_90 = 1,
    camera_orientation_180 = 2,
    camera_orientation_270 = 3,
};

pub const AttitudeStage = enum(u8) {
    failed = 0,
    aligning = 1,
    degraded = 2,
    valid = 3,
};

pub const AttitudeValidity = enum(u16) {
    track_angle_heading_valid = 0x0001,
    pitch_valid = 0x0002,
    roll_valid = 0x0004,
    lateral_body_accel_valid = 0x0008,
    normal_body_accel_valid = 0x0010,
    turn_rate_valid = 0x0020,
    hw_fail = 0x0040,
    mag_invalid = 0x0080,
    no_gps = 0x0100,
    gps_invalid = 0x0200,
    solution_coasting = 0x0400,
    true_track_angle = 0x0800,
    magnetic_heading = 0x1000,
};

pub const AutoSyncFrequency = enum(u8) {
    never = 0,
    occasionally = 1,
    frequent = 2,
    once_a_day = 3,
    remote = 4,
};

pub const ExdLayout = enum(u8) {
    full_screen = 0,
    half_vertical = 1,
    half_horizontal = 2,
    half_vertical_right_split = 3,
    half_horizontal_bottom_split = 4,
    full_quarter_split = 5,
    half_vertical_left_split = 6,
    half_horizontal_top_split = 7,
    /// The EXD may display the configured concepts in any layout it sees fit.
    dynamic = 8,
};

pub const ExdDisplayType = enum(u8) {
    numerical = 0,
    simple = 1,
    graph = 2,
    bar = 3,
    circle_graph = 4,
    virtual_partner = 5,
    balance = 6,
    string_list = 7,
    string = 8,
    simple_dynamic_icon = 9,
    gauge = 10,
};

pub const ExdDataUnits = enum(u8) {
    no_units = 0,
    laps = 1,
    miles_per_hour = 2,
    kilometers_per_hour = 3,
    feet_per_hour = 4,
    meters_per_hour = 5,
    degrees_celsius = 6,
    degrees_farenheit = 7,
    zone = 8,
    gear = 9,
    rpm = 10,
    bpm = 11,
    degrees = 12,
    millimeters = 13,
    meters = 14,
    kilometers = 15,
    feet = 16,
    yards = 17,
    kilofeet = 18,
    miles = 19,
    time = 20,
    enum_turn_type = 21,
    percent = 22,
    watts = 23,
    watts_per_kilogram = 24,
    enum_battery_status = 25,
    enum_bike_light_beam_angle_mode = 26,
    enum_bike_light_battery_status = 27,
    enum_bike_light_network_config_type = 28,
    lights = 29,
    seconds = 30,
    minutes = 31,
    hours = 32,
    calories = 33,
    kilojoules = 34,
    milliseconds = 35,
    second_per_mile = 36,
    second_per_kilometer = 37,
    centimeter = 38,
    enum_course_point = 39,
    bradians = 40,
    enum_sport = 41,
    inches_hg = 42,
    mm_hg = 43,
    mbars = 44,
    hecto_pascals = 45,
    feet_per_min = 46,
    meters_per_min = 47,
    meters_per_sec = 48,
    eight_cardinal = 49,
};

pub const ExdQualifiers = enum(u8) {
    no_qualifier = 0,
    instantaneous = 1,
    average = 2,
    lap = 3,
    maximum = 4,
    maximum_average = 5,
    maximum_lap = 6,
    last_lap = 7,
    average_lap = 8,
    to_destination = 9,
    to_go = 10,
    to_next = 11,
    next_course_point = 12,
    total = 13,
    three_second_average = 14,
    ten_second_average = 15,
    thirty_second_average = 16,
    percent_maximum = 17,
    percent_maximum_average = 18,
    lap_percent_maximum = 19,
    elapsed = 20,
    sunrise = 21,
    sunset = 22,
    compared_to_virtual_partner = 23,
    maximum_24h = 24,
    minimum_24h = 25,
    minimum = 26,
    first = 27,
    second = 28,
    third = 29,
    shifter = 30,
    last_sport = 31,
    moving = 32,
    stopped = 33,
    estimated_total = 34,
    zone_9 = 242,
    zone_8 = 243,
    zone_7 = 244,
    zone_6 = 245,
    zone_5 = 246,
    zone_4 = 247,
    zone_3 = 248,
    zone_2 = 249,
    zone_1 = 250,
};

pub const ExdDescriptors = enum(u8) {
    bike_light_battery_status = 0,
    beam_angle_status = 1,
    batery_level = 2,
    light_network_mode = 3,
    number_lights_connected = 4,
    cadence = 5,
    distance = 6,
    estimated_time_of_arrival = 7,
    heading = 8,
    time = 9,
    battery_level = 10,
    trainer_resistance = 11,
    trainer_target_power = 12,
    time_seated = 13,
    time_standing = 14,
    elevation = 15,
    grade = 16,
    ascent = 17,
    descent = 18,
    vertical_speed = 19,
    di2_battery_level = 20,
    front_gear = 21,
    rear_gear = 22,
    gear_ratio = 23,
    heart_rate = 24,
    heart_rate_zone = 25,
    time_in_heart_rate_zone = 26,
    heart_rate_reserve = 27,
    calories = 28,
    gps_accuracy = 29,
    gps_signal_strength = 30,
    temperature = 31,
    time_of_day = 32,
    balance = 33,
    pedal_smoothness = 34,
    power = 35,
    functional_threshold_power = 36,
    intensity_factor = 37,
    work = 38,
    power_ratio = 39,
    normalized_power = 40,
    training_stress_Score = 41,
    time_on_zone = 42,
    speed = 43,
    laps = 44,
    reps = 45,
    workout_step = 46,
    course_distance = 47,
    navigation_distance = 48,
    course_estimated_time_of_arrival = 49,
    navigation_estimated_time_of_arrival = 50,
    course_time = 51,
    navigation_time = 52,
    course_heading = 53,
    navigation_heading = 54,
    power_zone = 55,
    torque_effectiveness = 56,
    timer_time = 57,
    power_weight_ratio = 58,
    left_platform_center_offset = 59,
    right_platform_center_offset = 60,
    left_power_phase_start_angle = 61,
    right_power_phase_start_angle = 62,
    left_power_phase_finish_angle = 63,
    right_power_phase_finish_angle = 64,
    /// Combined gear information
    gears = 65,
    pace = 66,
    training_effect = 67,
    vertical_oscillation = 68,
    vertical_ratio = 69,
    ground_contact_time = 70,
    left_ground_contact_time_balance = 71,
    right_ground_contact_time_balance = 72,
    stride_length = 73,
    running_cadence = 74,
    performance_condition = 75,
    course_type = 76,
    time_in_power_zone = 77,
    navigation_turn = 78,
    course_location = 79,
    navigation_location = 80,
    compass = 81,
    gear_combo = 82,
    muscle_oxygen = 83,
    icon = 84,
    compass_heading = 85,
    gps_heading = 86,
    gps_elevation = 87,
    anaerobic_training_effect = 88,
    course = 89,
    off_course = 90,
    glide_ratio = 91,
    vertical_distance = 92,
    vmg = 93,
    ambient_pressure = 94,
    pressure = 95,
    vam = 96,
};

pub const AutoActivityDetect = enum(u32) {
    none = 0x00000000,
    running = 0x00000001,
    cycling = 0x00000002,
    swimming = 0x00000004,
    walking = 0x00000008,
    elliptical = 0x00000020,
    sedentary = 0x00000400,
};

pub const SupportedExdScreenLayouts = enum(u32) {
    full_screen = 0x00000001,
    half_vertical = 0x00000002,
    half_horizontal = 0x00000004,
    half_vertical_right_split = 0x00000008,
    half_horizontal_bottom_split = 0x00000010,
    full_quarter_split = 0x00000020,
    half_vertical_left_split = 0x00000040,
    half_horizontal_top_split = 0x00000080,
};

pub const FitBaseType = enum(u8) {
    _enum = 0,
    sint8 = 1,
    uint8 = 2,
    sint16 = 131,
    uint16 = 132,
    sint32 = 133,
    uint32 = 134,
    string = 7,
    float32 = 136,
    float64 = 137,
    uint8z = 10,
    uint16z = 139,
    uint32z = 140,
    byte = 13,
    sint64 = 142,
    uint64 = 143,
    uint64z = 144,
};

pub const TurnType = enum(u8) {
    arriving_idx = 0,
    arriving_left_idx = 1,
    arriving_right_idx = 2,
    arriving_via_idx = 3,
    arriving_via_left_idx = 4,
    arriving_via_right_idx = 5,
    bear_keep_left_idx = 6,
    bear_keep_right_idx = 7,
    continue_idx = 8,
    exit_left_idx = 9,
    exit_right_idx = 10,
    ferry_idx = 11,
    roundabout_45_idx = 12,
    roundabout_90_idx = 13,
    roundabout_135_idx = 14,
    roundabout_180_idx = 15,
    roundabout_225_idx = 16,
    roundabout_270_idx = 17,
    roundabout_315_idx = 18,
    roundabout_360_idx = 19,
    roundabout_neg_45_idx = 20,
    roundabout_neg_90_idx = 21,
    roundabout_neg_135_idx = 22,
    roundabout_neg_180_idx = 23,
    roundabout_neg_225_idx = 24,
    roundabout_neg_270_idx = 25,
    roundabout_neg_315_idx = 26,
    roundabout_neg_360_idx = 27,
    roundabout_generic_idx = 28,
    roundabout_neg_generic_idx = 29,
    sharp_turn_left_idx = 30,
    sharp_turn_right_idx = 31,
    turn_left_idx = 32,
    turn_right_idx = 33,
    uturn_left_idx = 34,
    uturn_right_idx = 35,
    icon_inv_idx = 36,
    icon_idx_cnt = 37,
};

pub const BikeLightBeamAngleMode = enum(u8) {
    manual = 0,
    auto = 1,
};

pub const FitBaseUnit = enum(u16) {
    other = 0,
    kilogram = 1,
    pound = 2,
};

pub const SetType = enum(u8) {
    rest = 0,
    active = 1,
};

pub const MaxMetCategory = enum(u8) {
    generic = 0,
    cycling = 1,
};

pub const ExerciseCategory = enum(u16) {
    bench_press = 0,
    calf_raise = 1,
    cardio = 2,
    carry = 3,
    chop = 4,
    core = 5,
    crunch = 6,
    curl = 7,
    deadlift = 8,
    flye = 9,
    hip_raise = 10,
    hip_stability = 11,
    hip_swing = 12,
    hyperextension = 13,
    lateral_raise = 14,
    leg_curl = 15,
    leg_raise = 16,
    lunge = 17,
    olympic_lift = 18,
    plank = 19,
    plyo = 20,
    pull_up = 21,
    push_up = 22,
    row = 23,
    shoulder_press = 24,
    shoulder_stability = 25,
    shrug = 26,
    sit_up = 27,
    squat = 28,
    total_body = 29,
    triceps_extension = 30,
    warm_up = 31,
    run = 32,
    bike = 33,
    /// Exercises within workouts that use GPS/sensors rather than rep counting
    cardio_sensors = 34,
    move = 35,
    pose = 36,
    banded_exercises = 37,
    battle_rope = 38,
    elliptical = 39,
    floor_climb = 40,
    indoor_bike = 41,
    indoor_row = 42,
    ladder = 43,
    sandbag = 44,
    sled = 45,
    sledge_hammer = 46,
    stair_stepper = 47,
    suspension = 49,
    tire = 50,
    run_indoor = 52,
    bike_outdoor = 53,
    unknown = 65534,
};

pub const BenchPressExerciseName = enum(u16) {
    alternating_dumbbell_chest_press_on_swiss_ball = 0,
    barbell_bench_press = 1,
    barbell_board_bench_press = 2,
    barbell_floor_press = 3,
    close_grip_barbell_bench_press = 4,
    decline_dumbbell_bench_press = 5,
    dumbbell_bench_press = 6,
    dumbbell_floor_press = 7,
    incline_barbell_bench_press = 8,
    incline_dumbbell_bench_press = 9,
    incline_smith_machine_bench_press = 10,
    isometric_barbell_bench_press = 11,
    kettlebell_chest_press = 12,
    neutral_grip_dumbbell_bench_press = 13,
    neutral_grip_dumbbell_incline_bench_press = 14,
    one_arm_floor_press = 15,
    weighted_one_arm_floor_press = 16,
    partial_lockout = 17,
    reverse_grip_barbell_bench_press = 18,
    reverse_grip_incline_bench_press = 19,
    single_arm_cable_chest_press = 20,
    single_arm_dumbbell_bench_press = 21,
    smith_machine_bench_press = 22,
    swiss_ball_dumbbell_chest_press = 23,
    triple_stop_barbell_bench_press = 24,
    wide_grip_barbell_bench_press = 25,
    alternating_dumbbell_chest_press = 26,
};

pub const CalfRaiseExerciseName = enum(u16) {
    _3_way_calf_raise = 0,
    _3_way_weighted_calf_raise = 1,
    _3_way_single_leg_calf_raise = 2,
    _3_way_weighted_single_leg_calf_raise = 3,
    donkey_calf_raise = 4,
    weighted_donkey_calf_raise = 5,
    seated_calf_raise = 6,
    weighted_seated_calf_raise = 7,
    seated_dumbbell_toe_raise = 8,
    single_leg_bent_knee_calf_raise = 9,
    weighted_single_leg_bent_knee_calf_raise = 10,
    single_leg_decline_push_up = 11,
    single_leg_donkey_calf_raise = 12,
    weighted_single_leg_donkey_calf_raise = 13,
    single_leg_hip_raise_with_knee_hold = 14,
    single_leg_standing_calf_raise = 15,
    single_leg_standing_dumbbell_calf_raise = 16,
    standing_barbell_calf_raise = 17,
    standing_calf_raise = 18,
    weighted_standing_calf_raise = 19,
    standing_dumbbell_calf_raise = 20,
};

pub const CardioExerciseName = enum(u16) {
    bob_and_weave_circle = 0,
    weighted_bob_and_weave_circle = 1,
    cardio_core_crawl = 2,
    weighted_cardio_core_crawl = 3,
    double_under = 4,
    weighted_double_under = 5,
    jump_rope = 6,
    weighted_jump_rope = 7,
    jump_rope_crossover = 8,
    weighted_jump_rope_crossover = 9,
    jump_rope_jog = 10,
    weighted_jump_rope_jog = 11,
    jumping_jacks = 12,
    weighted_jumping_jacks = 13,
    ski_moguls = 14,
    weighted_ski_moguls = 15,
    split_jacks = 16,
    weighted_split_jacks = 17,
    squat_jacks = 18,
    weighted_squat_jacks = 19,
    triple_under = 20,
    weighted_triple_under = 21,
    elliptical = 22,
    spinning = 23,
    pole_paddle_forward_wheelchair = 24,
    pole_paddle_backward_wheelchair = 25,
    pole_handcycle_forward_wheelchair = 26,
    pole_handcycle_backward_wheelchair = 27,
    pole_rainbow_wheelchair = 28,
    double_punch_forward_wheelchair = 29,
    double_punch_down_wheelchair = 30,
    double_punch_sideways_wheelchair = 31,
    double_punch_up_wheelchair = 32,
    sit_ski_wheelchair = 33,
    sitting_jacks_wheelchair = 34,
    punch_forward_wheelchair = 35,
    punch_down_wheelchair = 36,
    punch_sideways_wheelchair = 37,
    punch_up_wheelchair = 38,
    punch_bag_wheelchair = 39,
    pole_dd_ff_uu_wheelchair = 40,
    butterfly_arms_wheelchair = 41,
    punch = 42,
};

pub const CarryExerciseName = enum(u16) {
    bar_holds = 0,
    farmers_walk = 1,
    farmers_walk_on_toes = 2,
    hex_dumbbell_hold = 3,
    overhead_carry = 4,
    dumbbell_waiter_carry = 5,
    farmers_carry_walk_lunge = 6,
    farmers_carry = 7,
    farmers_carry_on_toes = 8,
};

pub const ChopExerciseName = enum(u16) {
    cable_pull_through = 0,
    cable_rotational_lift = 1,
    cable_woodchop = 2,
    cross_chop_to_knee = 3,
    weighted_cross_chop_to_knee = 4,
    dumbbell_chop = 5,
    half_kneeling_rotation = 6,
    weighted_half_kneeling_rotation = 7,
    half_kneeling_rotational_chop = 8,
    half_kneeling_rotational_reverse_chop = 9,
    half_kneeling_stability_chop = 10,
    half_kneeling_stability_reverse_chop = 11,
    kneeling_rotational_chop = 12,
    kneeling_rotational_reverse_chop = 13,
    kneeling_stability_chop = 14,
    kneeling_woodchopper = 15,
    medicine_ball_wood_chops = 16,
    power_squat_chops = 17,
    weighted_power_squat_chops = 18,
    standing_rotational_chop = 19,
    standing_split_rotational_chop = 20,
    standing_split_rotational_reverse_chop = 21,
    standing_stability_reverse_chop = 22,
};

pub const CoreExerciseName = enum(u16) {
    abs_jabs = 0,
    weighted_abs_jabs = 1,
    alternating_plate_reach = 2,
    barbell_rollout = 3,
    weighted_barbell_rollout = 4,
    body_bar_oblique_twist = 5,
    cable_core_press = 6,
    cable_side_bend = 7,
    side_bend = 8,
    weighted_side_bend = 9,
    crescent_circle = 10,
    weighted_crescent_circle = 11,
    cycling_russian_twist = 12,
    weighted_cycling_russian_twist = 13,
    elevated_feet_russian_twist = 14,
    weighted_elevated_feet_russian_twist = 15,
    half_turkish_get_up = 16,
    kettlebell_windmill = 17,
    kneeling_ab_wheel = 18,
    weighted_kneeling_ab_wheel = 19,
    modified_front_lever = 20,
    open_knee_tucks = 21,
    weighted_open_knee_tucks = 22,
    side_abs_leg_lift = 23,
    weighted_side_abs_leg_lift = 24,
    swiss_ball_jackknife = 25,
    weighted_swiss_ball_jackknife = 26,
    swiss_ball_pike = 27,
    weighted_swiss_ball_pike = 28,
    swiss_ball_rollout = 29,
    weighted_swiss_ball_rollout = 30,
    triangle_hip_press = 31,
    weighted_triangle_hip_press = 32,
    trx_suspended_jackknife = 33,
    weighted_trx_suspended_jackknife = 34,
    u_boat = 35,
    weighted_u_boat = 36,
    windmill_switches = 37,
    weighted_windmill_switches = 38,
    alternating_slide_out = 39,
    weighted_alternating_slide_out = 40,
    ghd_back_extensions = 41,
    weighted_ghd_back_extensions = 42,
    overhead_walk = 43,
    inchworm = 44,
    weighted_modified_front_lever = 45,
    russian_twist = 46,
    /// Deprecated do not use
    abdominal_leg_rotations = 47,
    arm_and_leg_extension_on_knees = 48,
    bicycle = 49,
    bicep_curl_with_leg_extension = 50,
    cat_cow = 51,
    corkscrew = 52,
    criss_cross = 53,
    /// Deprecated do not use
    criss_cross_with_ball = 54,
    double_leg_stretch = 55,
    knee_folds = 56,
    lower_lift = 57,
    neck_pull = 58,
    pelvic_clocks = 59,
    roll_over = 60,
    roll_up = 61,
    rolling = 62,
    rowing_1 = 63,
    rowing_2 = 64,
    scissors = 65,
    single_leg_circles = 66,
    single_leg_stretch = 67,
    /// Deprecated do not use
    snake_twist_1_and_2 = 68,
    swan = 69,
    swimming = 70,
    teaser = 71,
    the_hundred = 72,
    bicep_curl_with_leg_extension_with_weights = 73,
    hanging_l_sit = 75,
    lower_lift_with_weights = 77,
    ring_l_sit = 79,
    rowing_1_with_weights = 80,
    rowing_2_with_weights = 81,
    scissors_with_weights = 82,
    single_leg_stretch_with_weights = 83,
    toes_to_elbows = 84,
    weighted_criss_cross = 85,
    weighted_double_leg_stretch = 86,
    weighted_the_hundred = 87,
    l_sit = 88,
    turkish_get_up = 89,
    weighted_ring_l_sit = 90,
    weighted_hanging_l_sit = 91,
    weighted_l_sit = 92,
    side_bend_low_wheelchair = 93,
    side_bend_mid_wheelchair = 94,
    side_bend_high_wheelchair = 95,
    seated_side_bend = 96,
};

pub const CrunchExerciseName = enum(u16) {
    bicycle_crunch = 0,
    cable_crunch = 1,
    circular_arm_crunch = 2,
    crossed_arms_crunch = 3,
    weighted_crossed_arms_crunch = 4,
    cross_leg_reverse_crunch = 5,
    weighted_cross_leg_reverse_crunch = 6,
    crunch_chop = 7,
    weighted_crunch_chop = 8,
    double_crunch = 9,
    weighted_double_crunch = 10,
    elbow_to_knee_crunch = 11,
    weighted_elbow_to_knee_crunch = 12,
    flutter_kicks = 13,
    weighted_flutter_kicks = 14,
    foam_roller_reverse_crunch_on_bench = 15,
    weighted_foam_roller_reverse_crunch_on_bench = 16,
    foam_roller_reverse_crunch_with_dumbbell = 17,
    foam_roller_reverse_crunch_with_medicine_ball = 18,
    frog_press = 19,
    hanging_knee_raise_oblique_crunch = 20,
    weighted_hanging_knee_raise_oblique_crunch = 21,
    hip_crossover = 22,
    weighted_hip_crossover = 23,
    hollow_rock = 24,
    weighted_hollow_rock = 25,
    incline_reverse_crunch = 26,
    weighted_incline_reverse_crunch = 27,
    kneeling_cable_crunch = 28,
    kneeling_cross_crunch = 29,
    weighted_kneeling_cross_crunch = 30,
    kneeling_oblique_cable_crunch = 31,
    knees_to_elbow = 32,
    leg_extensions = 33,
    weighted_leg_extensions = 34,
    leg_levers = 35,
    mcgill_curl_up = 36,
    weighted_mcgill_curl_up = 37,
    modified_pilates_roll_up_with_ball = 38,
    weighted_modified_pilates_roll_up_with_ball = 39,
    pilates_crunch = 40,
    weighted_pilates_crunch = 41,
    pilates_roll_up_with_ball = 42,
    weighted_pilates_roll_up_with_ball = 43,
    raised_legs_crunch = 44,
    weighted_raised_legs_crunch = 45,
    reverse_crunch = 46,
    weighted_reverse_crunch = 47,
    reverse_crunch_on_a_bench = 48,
    weighted_reverse_crunch_on_a_bench = 49,
    reverse_curl_and_lift = 50,
    weighted_reverse_curl_and_lift = 51,
    rotational_lift = 52,
    weighted_rotational_lift = 53,
    seated_alternating_reverse_crunch = 54,
    weighted_seated_alternating_reverse_crunch = 55,
    seated_leg_u = 56,
    weighted_seated_leg_u = 57,
    side_to_side_crunch_and_weave = 58,
    weighted_side_to_side_crunch_and_weave = 59,
    single_leg_reverse_crunch = 60,
    weighted_single_leg_reverse_crunch = 61,
    skater_crunch_cross = 62,
    weighted_skater_crunch_cross = 63,
    standing_cable_crunch = 64,
    standing_side_crunch = 65,
    step_climb = 66,
    weighted_step_climb = 67,
    swiss_ball_crunch = 68,
    swiss_ball_reverse_crunch = 69,
    weighted_swiss_ball_reverse_crunch = 70,
    swiss_ball_russian_twist = 71,
    weighted_swiss_ball_russian_twist = 72,
    swiss_ball_side_crunch = 73,
    weighted_swiss_ball_side_crunch = 74,
    thoracic_crunches_on_foam_roller = 75,
    weighted_thoracic_crunches_on_foam_roller = 76,
    triceps_crunch = 77,
    weighted_bicycle_crunch = 78,
    weighted_crunch = 79,
    weighted_swiss_ball_crunch = 80,
    toes_to_bar = 81,
    weighted_toes_to_bar = 82,
    crunch = 83,
    straight_leg_crunch_with_ball = 84,
    leg_climb_crunch = 86,
};

pub const CurlExerciseName = enum(u16) {
    alternating_dumbbell_biceps_curl = 0,
    alternating_dumbbell_biceps_curl_on_swiss_ball = 1,
    alternating_incline_dumbbell_biceps_curl = 2,
    barbell_biceps_curl = 3,
    barbell_reverse_wrist_curl = 4,
    barbell_wrist_curl = 5,
    behind_the_back_barbell_reverse_wrist_curl = 6,
    behind_the_back_one_arm_cable_curl = 7,
    cable_biceps_curl = 8,
    cable_hammer_curl = 9,
    cheating_barbell_biceps_curl = 10,
    close_grip_ez_bar_biceps_curl = 11,
    cross_body_dumbbell_hammer_curl = 12,
    dead_hang_biceps_curl = 13,
    decline_hammer_curl = 14,
    dumbbell_biceps_curl_with_static_hold = 15,
    dumbbell_hammer_curl = 16,
    dumbbell_reverse_wrist_curl = 17,
    dumbbell_wrist_curl = 18,
    ez_bar_preacher_curl = 19,
    forward_bend_biceps_curl = 20,
    hammer_curl_to_press = 21,
    incline_dumbbell_biceps_curl = 22,
    incline_offset_thumb_dumbbell_curl = 23,
    kettlebell_biceps_curl = 24,
    lying_concentration_cable_curl = 25,
    one_arm_preacher_curl = 26,
    plate_pinch_curl = 27,
    preacher_curl_with_cable = 28,
    reverse_ez_bar_curl = 29,
    reverse_grip_wrist_curl = 30,
    reverse_grip_barbell_biceps_curl = 31,
    seated_alternating_dumbbell_biceps_curl = 32,
    seated_dumbbell_biceps_curl = 33,
    seated_reverse_dumbbell_curl = 34,
    split_stance_offset_pinky_dumbbell_curl = 35,
    standing_alternating_dumbbell_curls = 36,
    standing_dumbbell_biceps_curl = 37,
    standing_ez_bar_biceps_curl = 38,
    static_curl = 39,
    swiss_ball_dumbbell_overhead_triceps_extension = 40,
    swiss_ball_ez_bar_preacher_curl = 41,
    twisting_standing_dumbbell_biceps_curl = 42,
    wide_grip_ez_bar_biceps_curl = 43,
    one_arm_concentration_curl = 44,
    standing_zottman_biceps_curl = 45,
    dumbbell_biceps_curl = 46,
    drag_curl_wheelchair = 47,
    dumbbell_biceps_curl_wheelchair = 48,
    bottle_curl = 49,
    seated_bottle_curl = 50,
};

pub const DeadliftExerciseName = enum(u16) {
    barbell_deadlift = 0,
    barbell_straight_leg_deadlift = 1,
    dumbbell_deadlift = 2,
    dumbbell_single_leg_deadlift_to_row = 3,
    dumbbell_straight_leg_deadlift = 4,
    kettlebell_floor_to_shelf = 5,
    one_arm_one_leg_deadlift = 6,
    rack_pull = 7,
    rotational_dumbbell_straight_leg_deadlift = 8,
    single_arm_deadlift = 9,
    single_leg_barbell_deadlift = 10,
    single_leg_barbell_straight_leg_deadlift = 11,
    single_leg_deadlift_with_barbell = 12,
    single_leg_rdl_circuit = 13,
    single_leg_romanian_deadlift_with_dumbbell = 14,
    sumo_deadlift = 15,
    sumo_deadlift_high_pull = 16,
    trap_bar_deadlift = 17,
    wide_grip_barbell_deadlift = 18,
    kettlebell_deadlift = 20,
    kettlebell_sumo_deadlift = 21,
    romanian_deadlift = 23,
    single_leg_romanian_deadlift_circuit = 24,
    straight_leg_deadlift = 25,
};

pub const FlyeExerciseName = enum(u16) {
    cable_crossover = 0,
    decline_dumbbell_flye = 1,
    dumbbell_flye = 2,
    incline_dumbbell_flye = 3,
    kettlebell_flye = 4,
    kneeling_rear_flye = 5,
    single_arm_standing_cable_reverse_flye = 6,
    swiss_ball_dumbbell_flye = 7,
    arm_rotations = 8,
    hug_a_tree = 9,
    face_down_incline_reverse_flye = 10,
    incline_reverse_flye = 11,
    rear_delt_fly_wheelchair = 12,
};

pub const HipRaiseExerciseName = enum(u16) {
    barbell_hip_thrust_on_floor = 0,
    barbell_hip_thrust_with_bench = 1,
    bent_knee_swiss_ball_reverse_hip_raise = 2,
    weighted_bent_knee_swiss_ball_reverse_hip_raise = 3,
    bridge_with_leg_extension = 4,
    weighted_bridge_with_leg_extension = 5,
    clam_bridge = 6,
    front_kick_tabletop = 7,
    weighted_front_kick_tabletop = 8,
    hip_extension_and_cross = 9,
    weighted_hip_extension_and_cross = 10,
    hip_raise = 11,
    weighted_hip_raise = 12,
    hip_raise_with_feet_on_swiss_ball = 13,
    weighted_hip_raise_with_feet_on_swiss_ball = 14,
    hip_raise_with_head_on_bosu_ball = 15,
    weighted_hip_raise_with_head_on_bosu_ball = 16,
    hip_raise_with_head_on_swiss_ball = 17,
    weighted_hip_raise_with_head_on_swiss_ball = 18,
    hip_raise_with_knee_squeeze = 19,
    weighted_hip_raise_with_knee_squeeze = 20,
    incline_rear_leg_extension = 21,
    weighted_incline_rear_leg_extension = 22,
    kettlebell_swing = 23,
    marching_hip_raise = 24,
    weighted_marching_hip_raise = 25,
    marching_hip_raise_with_feet_on_a_swiss_ball = 26,
    weighted_marching_hip_raise_with_feet_on_a_swiss_ball = 27,
    reverse_hip_raise = 28,
    weighted_reverse_hip_raise = 29,
    single_leg_hip_raise = 30,
    weighted_single_leg_hip_raise = 31,
    single_leg_hip_raise_with_foot_on_bench = 32,
    weighted_single_leg_hip_raise_with_foot_on_bench = 33,
    single_leg_hip_raise_with_foot_on_bosu_ball = 34,
    weighted_single_leg_hip_raise_with_foot_on_bosu_ball = 35,
    single_leg_hip_raise_with_foot_on_foam_roller = 36,
    weighted_single_leg_hip_raise_with_foot_on_foam_roller = 37,
    single_leg_hip_raise_with_foot_on_medicine_ball = 38,
    weighted_single_leg_hip_raise_with_foot_on_medicine_ball = 39,
    single_leg_hip_raise_with_head_on_bosu_ball = 40,
    weighted_single_leg_hip_raise_with_head_on_bosu_ball = 41,
    weighted_clam_bridge = 42,
    single_leg_swiss_ball_hip_raise_and_leg_curl = 43,
    clams = 44,
    /// Deprecated do not use
    inner_thigh_circles = 45,
    /// Deprecated do not use
    inner_thigh_side_lift = 46,
    leg_circles = 47,
    leg_lift = 48,
    leg_lift_in_external_rotation = 49,
};

pub const HipStabilityExerciseName = enum(u16) {
    band_side_lying_leg_raise = 0,
    dead_bug = 1,
    weighted_dead_bug = 2,
    external_hip_raise = 3,
    weighted_external_hip_raise = 4,
    fire_hydrant_kicks = 5,
    weighted_fire_hydrant_kicks = 6,
    hip_circles = 7,
    weighted_hip_circles = 8,
    inner_thigh_lift = 9,
    weighted_inner_thigh_lift = 10,
    lateral_walks_with_band_at_ankles = 11,
    pretzel_side_kick = 12,
    weighted_pretzel_side_kick = 13,
    prone_hip_internal_rotation = 14,
    weighted_prone_hip_internal_rotation = 15,
    quadruped = 16,
    quadruped_hip_extension = 17,
    weighted_quadruped_hip_extension = 18,
    quadruped_with_leg_lift = 19,
    weighted_quadruped_with_leg_lift = 20,
    side_lying_leg_raise = 21,
    weighted_side_lying_leg_raise = 22,
    sliding_hip_adduction = 23,
    weighted_sliding_hip_adduction = 24,
    standing_adduction = 25,
    weighted_standing_adduction = 26,
    standing_cable_hip_abduction = 27,
    standing_hip_abduction = 28,
    weighted_standing_hip_abduction = 29,
    standing_rear_leg_raise = 30,
    weighted_standing_rear_leg_raise = 31,
    supine_hip_internal_rotation = 32,
    weighted_supine_hip_internal_rotation = 33,
    lying_abduction_stretch = 34,
};

pub const HipSwingExerciseName = enum(u16) {
    single_arm_kettlebell_swing = 0,
    single_arm_dumbbell_swing = 1,
    step_out_swing = 2,
    one_arm_swing = 3,
};

pub const HyperextensionExerciseName = enum(u16) {
    back_extension_with_opposite_arm_and_leg_reach = 0,
    weighted_back_extension_with_opposite_arm_and_leg_reach = 1,
    base_rotations = 2,
    weighted_base_rotations = 3,
    bent_knee_reverse_hyperextension = 4,
    weighted_bent_knee_reverse_hyperextension = 5,
    hollow_hold_and_roll = 6,
    weighted_hollow_hold_and_roll = 7,
    kicks = 8,
    weighted_kicks = 9,
    knee_raises = 10,
    weighted_knee_raises = 11,
    kneeling_superman = 12,
    weighted_kneeling_superman = 13,
    lat_pull_down_with_row = 14,
    medicine_ball_deadlift_to_reach = 15,
    one_arm_one_leg_row = 16,
    one_arm_row_with_band = 17,
    overhead_lunge_with_medicine_ball = 18,
    plank_knee_tucks = 19,
    weighted_plank_knee_tucks = 20,
    side_step = 21,
    weighted_side_step = 22,
    single_leg_back_extension = 23,
    weighted_single_leg_back_extension = 24,
    spine_extension = 25,
    weighted_spine_extension = 26,
    static_back_extension = 27,
    weighted_static_back_extension = 28,
    superman_from_floor = 29,
    weighted_superman_from_floor = 30,
    swiss_ball_back_extension = 31,
    weighted_swiss_ball_back_extension = 32,
    swiss_ball_hyperextension = 33,
    weighted_swiss_ball_hyperextension = 34,
    swiss_ball_opposite_arm_and_leg_lift = 35,
    weighted_swiss_ball_opposite_arm_and_leg_lift = 36,
    superman_on_swiss_ball = 37,
    cobra = 38,
    /// Deprecated do not use
    supine_floor_barre = 39,
};

pub const LateralRaiseExerciseName = enum(u16) {
    _45_degree_cable_external_rotation = 0,
    alternating_lateral_raise_with_static_hold = 1,
    bar_muscle_up = 2,
    bent_over_lateral_raise = 3,
    cable_diagonal_raise = 4,
    cable_front_raise = 5,
    calorie_row = 6,
    combo_shoulder_raise = 7,
    dumbbell_diagonal_raise = 8,
    dumbbell_v_raise = 9,
    front_raise = 10,
    leaning_dumbbell_lateral_raise = 11,
    lying_dumbbell_raise = 12,
    muscle_up = 13,
    one_arm_cable_lateral_raise = 14,
    overhand_grip_rear_lateral_raise = 15,
    plate_raises = 16,
    ring_dip = 17,
    weighted_ring_dip = 18,
    ring_muscle_up = 19,
    weighted_ring_muscle_up = 20,
    rope_climb = 21,
    weighted_rope_climb = 22,
    scaption = 23,
    seated_lateral_raise = 24,
    seated_rear_lateral_raise = 25,
    side_lying_lateral_raise = 26,
    standing_lift = 27,
    suspended_row = 28,
    underhand_grip_rear_lateral_raise = 29,
    wall_slide = 30,
    weighted_wall_slide = 31,
    arm_circles = 32,
    shaving_the_head = 33,
    dumbbell_lateral_raise = 34,
    ring_dip_kipping = 36,
    wall_walk = 37,
    dumbbell_front_raise_wheelchair = 38,
    dumbbell_lateral_raise_wheelchair = 39,
    pole_double_arm_overhead_and_forward_wheelchair = 40,
    pole_straight_arm_overhead_wheelchair = 41,
};

pub const LegCurlExerciseName = enum(u16) {
    leg_curl = 0,
    weighted_leg_curl = 1,
    good_morning = 2,
    seated_barbell_good_morning = 3,
    single_leg_barbell_good_morning = 4,
    single_leg_sliding_leg_curl = 5,
    sliding_leg_curl = 6,
    split_barbell_good_morning = 7,
    split_stance_extension = 8,
    staggered_stance_good_morning = 9,
    swiss_ball_hip_raise_and_leg_curl = 10,
    zercher_good_morning = 11,
    band_good_morning = 12,
    bar_good_morning = 13,
};

pub const LegRaiseExerciseName = enum(u16) {
    hanging_knee_raise = 0,
    hanging_leg_raise = 1,
    weighted_hanging_leg_raise = 2,
    hanging_single_leg_raise = 3,
    weighted_hanging_single_leg_raise = 4,
    kettlebell_leg_raises = 5,
    leg_lowering_drill = 6,
    weighted_leg_lowering_drill = 7,
    lying_straight_leg_raise = 8,
    weighted_lying_straight_leg_raise = 9,
    medicine_ball_leg_drops = 10,
    quadruped_leg_raise = 11,
    weighted_quadruped_leg_raise = 12,
    reverse_leg_raise = 13,
    weighted_reverse_leg_raise = 14,
    reverse_leg_raise_on_swiss_ball = 15,
    weighted_reverse_leg_raise_on_swiss_ball = 16,
    single_leg_lowering_drill = 17,
    weighted_single_leg_lowering_drill = 18,
    weighted_hanging_knee_raise = 19,
    lateral_stepover = 20,
    weighted_lateral_stepover = 21,
};

pub const LungeExerciseName = enum(u16) {
    overhead_lunge = 0,
    lunge_matrix = 1,
    weighted_lunge_matrix = 2,
    alternating_barbell_forward_lunge = 3,
    alternating_dumbbell_lunge_with_reach = 4,
    back_foot_elevated_dumbbell_split_squat = 5,
    barbell_box_lunge = 6,
    barbell_bulgarian_split_squat = 7,
    barbell_crossover_lunge = 8,
    barbell_front_split_squat = 9,
    barbell_lunge = 10,
    barbell_reverse_lunge = 11,
    barbell_side_lunge = 12,
    barbell_split_squat = 13,
    core_control_rear_lunge = 14,
    diagonal_lunge = 15,
    drop_lunge = 16,
    dumbbell_box_lunge = 17,
    dumbbell_bulgarian_split_squat = 18,
    dumbbell_crossover_lunge = 19,
    dumbbell_diagonal_lunge = 20,
    dumbbell_lunge = 21,
    dumbbell_lunge_and_rotation = 22,
    dumbbell_overhead_bulgarian_split_squat = 23,
    dumbbell_reverse_lunge_to_high_knee_and_press = 24,
    dumbbell_side_lunge = 25,
    elevated_front_foot_barbell_split_squat = 26,
    front_foot_elevated_dumbbell_split_squat = 27,
    gunslinger_lunge = 28,
    lawnmower_lunge = 29,
    low_lunge_with_isometric_adduction = 30,
    low_side_to_side_lunge = 31,
    lunge = 32,
    weighted_lunge = 33,
    lunge_with_arm_reach = 34,
    lunge_with_diagonal_reach = 35,
    lunge_with_side_bend = 36,
    offset_dumbbell_lunge = 37,
    offset_dumbbell_reverse_lunge = 38,
    overhead_bulgarian_split_squat = 39,
    overhead_dumbbell_reverse_lunge = 40,
    overhead_dumbbell_split_squat = 41,
    overhead_lunge_with_rotation = 42,
    reverse_barbell_box_lunge = 43,
    reverse_box_lunge = 44,
    reverse_dumbbell_box_lunge = 45,
    reverse_dumbbell_crossover_lunge = 46,
    reverse_dumbbell_diagonal_lunge = 47,
    reverse_lunge_with_reach_back = 48,
    weighted_reverse_lunge_with_reach_back = 49,
    reverse_lunge_with_twist_and_overhead_reach = 50,
    weighted_reverse_lunge_with_twist_and_overhead_reach = 51,
    reverse_sliding_box_lunge = 52,
    weighted_reverse_sliding_box_lunge = 53,
    reverse_sliding_lunge = 54,
    weighted_reverse_sliding_lunge = 55,
    runners_lunge_to_balance = 56,
    weighted_runners_lunge_to_balance = 57,
    shifting_side_lunge = 58,
    side_and_crossover_lunge = 59,
    weighted_side_and_crossover_lunge = 60,
    side_lunge = 61,
    weighted_side_lunge = 62,
    side_lunge_and_press = 63,
    side_lunge_jump_off = 64,
    side_lunge_sweep = 65,
    weighted_side_lunge_sweep = 66,
    side_lunge_to_crossover_tap = 67,
    weighted_side_lunge_to_crossover_tap = 68,
    side_to_side_lunge_chops = 69,
    weighted_side_to_side_lunge_chops = 70,
    siff_jump_lunge = 71,
    weighted_siff_jump_lunge = 72,
    single_arm_reverse_lunge_and_press = 73,
    sliding_lateral_lunge = 74,
    weighted_sliding_lateral_lunge = 75,
    walking_barbell_lunge = 76,
    walking_dumbbell_lunge = 77,
    walking_lunge = 78,
    weighted_walking_lunge = 79,
    wide_grip_overhead_barbell_split_squat = 80,
    alternating_dumbbell_lunge = 81,
    dumbbell_reverse_lunge = 82,
    overhead_dumbbell_lunge = 83,
    scissor_power_switch = 84,
    dumbbell_overhead_walking_lunge = 85,
    curtsy_lunge = 86,
    weighted_curtsy_lunge = 87,
    weighted_shifting_side_lunge = 88,
    weighted_side_lunge_and_press = 89,
    weighted_side_lunge_jump_off = 90,
};

pub const OlympicLiftExerciseName = enum(u16) {
    barbell_hang_power_clean = 0,
    barbell_hang_squat_clean = 1,
    barbell_power_clean = 2,
    barbell_power_snatch = 3,
    barbell_squat_clean = 4,
    clean_and_jerk = 5,
    barbell_hang_power_snatch = 6,
    barbell_hang_pull = 7,
    barbell_high_pull = 8,
    barbell_snatch = 9,
    barbell_split_jerk = 10,
    clean = 11,
    dumbbell_clean = 12,
    dumbbell_hang_pull = 13,
    one_hand_dumbbell_split_snatch = 14,
    push_jerk = 15,
    single_arm_dumbbell_snatch = 16,
    single_arm_hang_snatch = 17,
    single_arm_kettlebell_snatch = 18,
    split_jerk = 19,
    squat_clean_and_jerk = 20,
    dumbbell_hang_snatch = 21,
    dumbbell_power_clean_and_jerk = 22,
    dumbbell_power_clean_and_push_press = 23,
    dumbbell_power_clean_and_strict_press = 24,
    dumbbell_snatch = 25,
    medicine_ball_clean = 26,
    clean_and_press = 27,
    snatch = 28,
};

pub const PlankExerciseName = enum(u16) {
    _45_degree_plank = 0,
    weighted_45_degree_plank = 1,
    _90_degree_static_hold = 2,
    weighted_90_degree_static_hold = 3,
    bear_crawl = 4,
    weighted_bear_crawl = 5,
    cross_body_mountain_climber = 6,
    weighted_cross_body_mountain_climber = 7,
    elbow_plank_pike_jacks = 8,
    weighted_elbow_plank_pike_jacks = 9,
    elevated_feet_plank = 10,
    weighted_elevated_feet_plank = 11,
    elevator_abs = 12,
    weighted_elevator_abs = 13,
    extended_plank = 14,
    weighted_extended_plank = 15,
    full_plank_passe_twist = 16,
    weighted_full_plank_passe_twist = 17,
    inching_elbow_plank = 18,
    weighted_inching_elbow_plank = 19,
    inchworm_to_side_plank = 20,
    weighted_inchworm_to_side_plank = 21,
    kneeling_plank = 22,
    weighted_kneeling_plank = 23,
    kneeling_side_plank_with_leg_lift = 24,
    weighted_kneeling_side_plank_with_leg_lift = 25,
    lateral_roll = 26,
    weighted_lateral_roll = 27,
    lying_reverse_plank = 28,
    weighted_lying_reverse_plank = 29,
    medicine_ball_mountain_climber = 30,
    weighted_medicine_ball_mountain_climber = 31,
    modified_mountain_climber_and_extension = 32,
    weighted_modified_mountain_climber_and_extension = 33,
    mountain_climber = 34,
    weighted_mountain_climber = 35,
    mountain_climber_on_sliding_discs = 36,
    weighted_mountain_climber_on_sliding_discs = 37,
    mountain_climber_with_feet_on_bosu_ball = 38,
    weighted_mountain_climber_with_feet_on_bosu_ball = 39,
    mountain_climber_with_hands_on_bench = 40,
    mountain_climber_with_hands_on_swiss_ball = 41,
    weighted_mountain_climber_with_hands_on_swiss_ball = 42,
    plank = 43,
    plank_jacks_with_feet_on_sliding_discs = 44,
    weighted_plank_jacks_with_feet_on_sliding_discs = 45,
    plank_knee_twist = 46,
    weighted_plank_knee_twist = 47,
    plank_pike_jumps = 48,
    weighted_plank_pike_jumps = 49,
    plank_pikes = 50,
    weighted_plank_pikes = 51,
    plank_to_stand_up = 52,
    weighted_plank_to_stand_up = 53,
    plank_with_arm_raise = 54,
    weighted_plank_with_arm_raise = 55,
    plank_with_knee_to_elbow = 56,
    weighted_plank_with_knee_to_elbow = 57,
    plank_with_oblique_crunch = 58,
    weighted_plank_with_oblique_crunch = 59,
    plyometric_side_plank = 60,
    weighted_plyometric_side_plank = 61,
    rolling_side_plank = 62,
    weighted_rolling_side_plank = 63,
    side_kick_plank = 64,
    weighted_side_kick_plank = 65,
    side_plank = 66,
    weighted_side_plank = 67,
    side_plank_and_row = 68,
    weighted_side_plank_and_row = 69,
    side_plank_lift = 70,
    weighted_side_plank_lift = 71,
    side_plank_with_elbow_on_bosu_ball = 72,
    weighted_side_plank_with_elbow_on_bosu_ball = 73,
    side_plank_with_feet_on_bench = 74,
    weighted_side_plank_with_feet_on_bench = 75,
    side_plank_with_knee_circle = 76,
    weighted_side_plank_with_knee_circle = 77,
    side_plank_with_knee_tuck = 78,
    weighted_side_plank_with_knee_tuck = 79,
    side_plank_with_leg_lift = 80,
    weighted_side_plank_with_leg_lift = 81,
    side_plank_with_reach_under = 82,
    weighted_side_plank_with_reach_under = 83,
    single_leg_elevated_feet_plank = 84,
    weighted_single_leg_elevated_feet_plank = 85,
    single_leg_flex_and_extend = 86,
    weighted_single_leg_flex_and_extend = 87,
    single_leg_side_plank = 88,
    weighted_single_leg_side_plank = 89,
    spiderman_plank = 90,
    weighted_spiderman_plank = 91,
    straight_arm_plank = 92,
    weighted_straight_arm_plank = 93,
    straight_arm_plank_with_shoulder_touch = 94,
    weighted_straight_arm_plank_with_shoulder_touch = 95,
    swiss_ball_plank = 96,
    weighted_swiss_ball_plank = 97,
    swiss_ball_plank_leg_lift = 98,
    weighted_swiss_ball_plank_leg_lift = 99,
    swiss_ball_plank_leg_lift_and_hold = 100,
    swiss_ball_plank_with_feet_on_bench = 101,
    weighted_swiss_ball_plank_with_feet_on_bench = 102,
    swiss_ball_prone_jackknife = 103,
    weighted_swiss_ball_prone_jackknife = 104,
    swiss_ball_side_plank = 105,
    weighted_swiss_ball_side_plank = 106,
    three_way_plank = 107,
    weighted_three_way_plank = 108,
    towel_plank_and_knee_in = 109,
    weighted_towel_plank_and_knee_in = 110,
    t_stabilization = 111,
    weighted_t_stabilization = 112,
    turkish_get_up_to_side_plank = 113,
    weighted_turkish_get_up_to_side_plank = 114,
    two_point_plank = 115,
    weighted_two_point_plank = 116,
    weighted_plank = 117,
    wide_stance_plank_with_diagonal_arm_lift = 118,
    weighted_wide_stance_plank_with_diagonal_arm_lift = 119,
    wide_stance_plank_with_diagonal_leg_lift = 120,
    weighted_wide_stance_plank_with_diagonal_leg_lift = 121,
    wide_stance_plank_with_leg_lift = 122,
    weighted_wide_stance_plank_with_leg_lift = 123,
    wide_stance_plank_with_opposite_arm_and_leg_lift = 124,
    weighted_mountain_climber_with_hands_on_bench = 125,
    weighted_swiss_ball_plank_leg_lift_and_hold = 126,
    weighted_wide_stance_plank_with_opposite_arm_and_leg_lift = 127,
    plank_with_feet_on_swiss_ball = 128,
    side_plank_to_plank_with_reach_under = 129,
    bridge_with_glute_lower_lift = 130,
    bridge_one_leg_bridge = 131,
    plank_with_arm_variations = 132,
    plank_with_leg_lift = 133,
    reverse_plank_with_leg_pull = 134,
    ring_plank_sprawls = 135,
};

pub const PlyoExerciseName = enum(u16) {
    alternating_jump_lunge = 0,
    weighted_alternating_jump_lunge = 1,
    barbell_jump_squat = 2,
    body_weight_jump_squat = 3,
    weighted_jump_squat = 4,
    cross_knee_strike = 5,
    weighted_cross_knee_strike = 6,
    depth_jump = 7,
    weighted_depth_jump = 8,
    dumbbell_jump_squat = 9,
    dumbbell_split_jump = 10,
    front_knee_strike = 11,
    weighted_front_knee_strike = 12,
    high_box_jump = 13,
    weighted_high_box_jump = 14,
    isometric_explosive_body_weight_jump_squat = 15,
    weighted_isometric_explosive_jump_squat = 16,
    lateral_leap_and_hop = 17,
    weighted_lateral_leap_and_hop = 18,
    lateral_plyo_squats = 19,
    weighted_lateral_plyo_squats = 20,
    lateral_slide = 21,
    weighted_lateral_slide = 22,
    medicine_ball_overhead_throws = 23,
    medicine_ball_side_throw = 24,
    medicine_ball_slam = 25,
    side_to_side_medicine_ball_throws = 26,
    side_to_side_shuffle_jump = 27,
    weighted_side_to_side_shuffle_jump = 28,
    squat_jump_onto_box = 29,
    weighted_squat_jump_onto_box = 30,
    squat_jumps_in_and_out = 31,
    weighted_squat_jumps_in_and_out = 32,
    box_jump = 33,
    box_jump_overs = 34,
    box_jump_overs_over_the_box = 35,
    star_jump_squats = 36,
    jump_squat = 37,
};

pub const PullUpExerciseName = enum(u16) {
    banded_pull_ups = 0,
    _30_degree_lat_pulldown = 1,
    band_assisted_chin_up = 2,
    close_grip_chin_up = 3,
    weighted_close_grip_chin_up = 4,
    close_grip_lat_pulldown = 5,
    crossover_chin_up = 6,
    weighted_crossover_chin_up = 7,
    ez_bar_pullover = 8,
    hanging_hurdle = 9,
    weighted_hanging_hurdle = 10,
    kneeling_lat_pulldown = 11,
    kneeling_underhand_grip_lat_pulldown = 12,
    lat_pulldown = 13,
    mixed_grip_chin_up = 14,
    weighted_mixed_grip_chin_up = 15,
    mixed_grip_pull_up = 16,
    weighted_mixed_grip_pull_up = 17,
    reverse_grip_pulldown = 18,
    standing_cable_pullover = 19,
    straight_arm_pulldown = 20,
    swiss_ball_ez_bar_pullover = 21,
    towel_pull_up = 22,
    weighted_towel_pull_up = 23,
    weighted_pull_up = 24,
    wide_grip_lat_pulldown = 25,
    wide_grip_pull_up = 26,
    weighted_wide_grip_pull_up = 27,
    burpee_pull_up = 28,
    weighted_burpee_pull_up = 29,
    jumping_pull_ups = 30,
    weighted_jumping_pull_ups = 31,
    kipping_pull_up = 32,
    weighted_kipping_pull_up = 33,
    l_pull_up = 34,
    weighted_l_pull_up = 35,
    suspended_chin_up = 36,
    weighted_suspended_chin_up = 37,
    pull_up = 38,
    chin_up = 39,
    neutral_grip_chin_up = 40,
    weighted_chin_up = 41,
    band_assisted_pull_up = 42,
    neutral_grip_pull_up = 43,
    weighted_neutral_grip_chin_up = 44,
    weighted_neutral_grip_pull_up = 45,
};

pub const PushUpExerciseName = enum(u16) {
    chest_press_with_band = 0,
    alternating_staggered_push_up = 1,
    weighted_alternating_staggered_push_up = 2,
    alternating_hands_medicine_ball_push_up = 3,
    weighted_alternating_hands_medicine_ball_push_up = 4,
    bosu_ball_push_up = 5,
    weighted_bosu_ball_push_up = 6,
    clapping_push_up = 7,
    weighted_clapping_push_up = 8,
    close_grip_medicine_ball_push_up = 9,
    weighted_close_grip_medicine_ball_push_up = 10,
    close_hands_push_up = 11,
    weighted_close_hands_push_up = 12,
    decline_push_up = 13,
    weighted_decline_push_up = 14,
    diamond_push_up = 15,
    weighted_diamond_push_up = 16,
    explosive_crossover_push_up = 17,
    weighted_explosive_crossover_push_up = 18,
    explosive_push_up = 19,
    weighted_explosive_push_up = 20,
    feet_elevated_side_to_side_push_up = 21,
    weighted_feet_elevated_side_to_side_push_up = 22,
    hand_release_push_up = 23,
    weighted_hand_release_push_up = 24,
    handstand_push_up = 25,
    weighted_handstand_push_up = 26,
    incline_push_up = 27,
    weighted_incline_push_up = 28,
    isometric_explosive_push_up = 29,
    weighted_isometric_explosive_push_up = 30,
    judo_push_up = 31,
    weighted_judo_push_up = 32,
    kneeling_push_up = 33,
    weighted_kneeling_push_up = 34,
    medicine_ball_chest_pass = 35,
    medicine_ball_push_up = 36,
    weighted_medicine_ball_push_up = 37,
    one_arm_push_up = 38,
    weighted_one_arm_push_up = 39,
    weighted_push_up = 40,
    push_up_and_row = 41,
    weighted_push_up_and_row = 42,
    push_up_plus = 43,
    weighted_push_up_plus = 44,
    push_up_with_feet_on_swiss_ball = 45,
    weighted_push_up_with_feet_on_swiss_ball = 46,
    push_up_with_one_hand_on_medicine_ball = 47,
    weighted_push_up_with_one_hand_on_medicine_ball = 48,
    shoulder_push_up = 49,
    weighted_shoulder_push_up = 50,
    single_arm_medicine_ball_push_up = 51,
    weighted_single_arm_medicine_ball_push_up = 52,
    spiderman_push_up = 53,
    weighted_spiderman_push_up = 54,
    stacked_feet_push_up = 55,
    weighted_stacked_feet_push_up = 56,
    staggered_hands_push_up = 57,
    weighted_staggered_hands_push_up = 58,
    suspended_push_up = 59,
    weighted_suspended_push_up = 60,
    swiss_ball_push_up = 61,
    weighted_swiss_ball_push_up = 62,
    swiss_ball_push_up_plus = 63,
    weighted_swiss_ball_push_up_plus = 64,
    t_push_up = 65,
    weighted_t_push_up = 66,
    triple_stop_push_up = 67,
    weighted_triple_stop_push_up = 68,
    wide_hands_push_up = 69,
    weighted_wide_hands_push_up = 70,
    parallette_handstand_push_up = 71,
    weighted_parallette_handstand_push_up = 72,
    ring_handstand_push_up = 73,
    weighted_ring_handstand_push_up = 74,
    ring_push_up = 75,
    weighted_ring_push_up = 76,
    push_up = 77,
    pilates_pushup = 78,
    dynamic_push_up = 79,
    kipping_handstand_push_up = 80,
    shoulder_tapping_push_up = 81,
    biceps_push_up = 82,
    hindu_push_up = 83,
    pike_push_up = 84,
    wide_grip_push_up = 85,
    weighted_biceps_push_up = 86,
    weighted_hindu_push_up = 87,
    weighted_pike_push_up = 88,
    kipping_parallette_handstand_push_up = 89,
    wall_push_up = 90,
};

pub const RowExerciseName = enum(u16) {
    barbell_straight_leg_deadlift_to_row = 0,
    cable_row_standing = 1,
    dumbbell_row = 2,
    elevated_feet_inverted_row = 3,
    weighted_elevated_feet_inverted_row = 4,
    face_pull = 5,
    face_pull_with_external_rotation = 6,
    inverted_row_with_feet_on_swiss_ball = 7,
    weighted_inverted_row_with_feet_on_swiss_ball = 8,
    kettlebell_row = 9,
    modified_inverted_row = 10,
    weighted_modified_inverted_row = 11,
    neutral_grip_alternating_dumbbell_row = 12,
    one_arm_bent_over_row = 13,
    one_legged_dumbbell_row = 14,
    renegade_row = 15,
    reverse_grip_barbell_row = 16,
    rope_handle_cable_row = 17,
    seated_cable_row = 18,
    seated_dumbbell_row = 19,
    single_arm_cable_row = 20,
    single_arm_cable_row_and_rotation = 21,
    single_arm_inverted_row = 22,
    weighted_single_arm_inverted_row = 23,
    single_arm_neutral_grip_dumbbell_row = 24,
    single_arm_neutral_grip_dumbbell_row_and_rotation = 25,
    suspended_inverted_row = 26,
    weighted_suspended_inverted_row = 27,
    t_bar_row = 28,
    towel_grip_inverted_row = 29,
    weighted_towel_grip_inverted_row = 30,
    underhand_grip_cable_row = 31,
    v_grip_cable_row = 32,
    wide_grip_seated_cable_row = 33,
    alternating_dumbbell_row = 34,
    inverted_row = 35,
    row = 36,
    weighted_row = 37,
    indoor_row = 38,
    banded_face_pulls = 39,
    chest_supported_dumbbell_row = 40,
    decline_ring_row = 41,
    elevated_ring_row = 42,
    rdl_bent_over_row_with_barbell_dumbbell = 43,
    ring_row = 44,
    barbell_row = 45,
    bent_over_row_with_barbell = 46,
    bent_over_row_with_dumbell = 47,
    seated_underhand_grip_cable_row = 48,
    trx_inverted_row = 49,
    weighted_inverted_row = 50,
    weighted_trx_inverted_row = 51,
    dumbbell_row_wheelchair = 52,
};

pub const ShoulderPressExerciseName = enum(u16) {
    alternating_dumbbell_shoulder_press = 0,
    arnold_press = 1,
    barbell_front_squat_to_push_press = 2,
    barbell_push_press = 3,
    barbell_shoulder_press = 4,
    dead_curl_press = 5,
    dumbbell_alternating_shoulder_press_and_twist = 6,
    dumbbell_hammer_curl_to_lunge_to_press = 7,
    dumbbell_push_press = 8,
    floor_inverted_shoulder_press = 9,
    weighted_floor_inverted_shoulder_press = 10,
    inverted_shoulder_press = 11,
    weighted_inverted_shoulder_press = 12,
    one_arm_push_press = 13,
    overhead_barbell_press = 14,
    overhead_dumbbell_press = 15,
    seated_barbell_shoulder_press = 16,
    seated_dumbbell_shoulder_press = 17,
    single_arm_dumbbell_shoulder_press = 18,
    single_arm_step_up_and_press = 19,
    smith_machine_overhead_press = 20,
    split_stance_hammer_curl_to_press = 21,
    swiss_ball_dumbbell_shoulder_press = 22,
    weight_plate_front_raise = 23,
    dumbbell_shoulder_press = 24,
    military_press = 25,
    strict_press = 27,
    dumbbell_front_raise = 28,
    dumbbell_curl_to_overhead_press_wheelchair = 29,
    arnold_press_wheelchair = 30,
    overhead_dumbbell_press_wheelchair = 31,
};

pub const ShoulderStabilityExerciseName = enum(u16) {
    _90_degree_cable_external_rotation = 0,
    band_external_rotation = 1,
    band_internal_rotation = 2,
    bent_arm_lateral_raise_and_external_rotation = 3,
    cable_external_rotation = 4,
    dumbbell_face_pull_with_external_rotation = 5,
    floor_i_raise = 6,
    weighted_floor_i_raise = 7,
    floor_t_raise = 8,
    weighted_floor_t_raise = 9,
    floor_y_raise = 10,
    weighted_floor_y_raise = 11,
    incline_i_raise = 12,
    weighted_incline_i_raise = 13,
    incline_l_raise = 14,
    weighted_incline_l_raise = 15,
    incline_t_raise = 16,
    weighted_incline_t_raise = 17,
    incline_w_raise = 18,
    weighted_incline_w_raise = 19,
    incline_y_raise = 20,
    weighted_incline_y_raise = 21,
    lying_external_rotation = 22,
    seated_dumbbell_external_rotation = 23,
    standing_l_raise = 24,
    swiss_ball_i_raise = 25,
    weighted_swiss_ball_i_raise = 26,
    swiss_ball_t_raise = 27,
    weighted_swiss_ball_t_raise = 28,
    swiss_ball_w_raise = 29,
    weighted_swiss_ball_w_raise = 30,
    swiss_ball_y_raise = 31,
    weighted_swiss_ball_y_raise = 32,
    cable_internal_rotation = 33,
    lying_internal_rotation = 34,
    seated_dumbbell_internal_rotation = 35,
};

pub const ShrugExerciseName = enum(u16) {
    barbell_jump_shrug = 0,
    barbell_shrug = 1,
    barbell_upright_row = 2,
    behind_the_back_smith_machine_shrug = 3,
    dumbbell_jump_shrug = 4,
    dumbbell_shrug = 5,
    dumbbell_upright_row = 6,
    incline_dumbbell_shrug = 7,
    overhead_barbell_shrug = 8,
    overhead_dumbbell_shrug = 9,
    scaption_and_shrug = 10,
    scapular_retraction = 11,
    serratus_chair_shrug = 12,
    weighted_serratus_chair_shrug = 13,
    serratus_shrug = 14,
    weighted_serratus_shrug = 15,
    wide_grip_jump_shrug = 16,
    wide_grip_barbell_shrug = 17,
    behind_the_back_shrug = 18,
    dumbbell_shrug_wheelchair = 19,
    shrug_wheelchair = 20,
    shrug_arm_down_wheelchair = 21,
    shrug_arm_mid_wheelchair = 22,
    shrug_arm_up_wheelchair = 23,
    upright_row = 24,
};

pub const SitUpExerciseName = enum(u16) {
    alternating_sit_up = 0,
    weighted_alternating_sit_up = 1,
    bent_knee_v_up = 2,
    weighted_bent_knee_v_up = 3,
    butterfly_sit_up = 4,
    weighted_butterfly_situp = 5,
    cross_punch_roll_up = 6,
    weighted_cross_punch_roll_up = 7,
    crossed_arms_sit_up = 8,
    weighted_crossed_arms_sit_up = 9,
    get_up_sit_up = 10,
    weighted_get_up_sit_up = 11,
    hovering_sit_up = 12,
    weighted_hovering_sit_up = 13,
    kettlebell_sit_up = 14,
    medicine_ball_alternating_v_up = 15,
    medicine_ball_sit_up = 16,
    medicine_ball_v_up = 17,
    modified_sit_up = 18,
    negative_sit_up = 19,
    one_arm_full_sit_up = 20,
    reclining_circle = 21,
    weighted_reclining_circle = 22,
    reverse_curl_up = 23,
    weighted_reverse_curl_up = 24,
    single_leg_swiss_ball_jackknife = 25,
    weighted_single_leg_swiss_ball_jackknife = 26,
    the_teaser = 27,
    the_teaser_weighted = 28,
    three_part_roll_down = 29,
    weighted_three_part_roll_down = 30,
    v_up = 31,
    weighted_v_up = 32,
    weighted_russian_twist_on_swiss_ball = 33,
    weighted_sit_up = 34,
    x_abs = 35,
    weighted_x_abs = 36,
    sit_up = 37,
    ghd_sit_ups = 38,
    sit_up_turkish_get_up = 39,
    russian_twist_on_swiss_ball = 40,
};

pub const SquatExerciseName = enum(u16) {
    leg_press = 0,
    back_squat_with_body_bar = 1,
    back_squats = 2,
    weighted_back_squats = 3,
    balancing_squat = 4,
    weighted_balancing_squat = 5,
    barbell_back_squat = 6,
    barbell_box_squat = 7,
    barbell_front_squat = 8,
    barbell_hack_squat = 9,
    barbell_hang_squat_snatch = 10,
    barbell_lateral_step_up = 11,
    barbell_quarter_squat = 12,
    barbell_siff_squat = 13,
    barbell_squat_snatch = 14,
    barbell_squat_with_heels_raised = 15,
    barbell_stepover = 16,
    barbell_step_up = 17,
    bench_squat_with_rotational_chop = 18,
    weighted_bench_squat_with_rotational_chop = 19,
    body_weight_wall_squat = 20,
    weighted_wall_squat = 21,
    box_step_squat = 22,
    weighted_box_step_squat = 23,
    braced_squat = 24,
    crossed_arm_barbell_front_squat = 25,
    crossover_dumbbell_step_up = 26,
    dumbbell_front_squat = 27,
    dumbbell_split_squat = 28,
    dumbbell_squat = 29,
    dumbbell_squat_clean = 30,
    dumbbell_stepover = 31,
    dumbbell_step_up = 32,
    elevated_single_leg_squat = 33,
    weighted_elevated_single_leg_squat = 34,
    figure_four_squats = 35,
    weighted_figure_four_squats = 36,
    goblet_squat = 37,
    kettlebell_squat = 38,
    kettlebell_swing_overhead = 39,
    kettlebell_swing_with_flip_to_squat = 40,
    lateral_dumbbell_step_up = 41,
    one_legged_squat = 42,
    overhead_dumbbell_squat = 43,
    overhead_squat = 44,
    partial_single_leg_squat = 45,
    weighted_partial_single_leg_squat = 46,
    pistol_squat = 47,
    weighted_pistol_squat = 48,
    plie_slides = 49,
    weighted_plie_slides = 50,
    plie_squat = 51,
    weighted_plie_squat = 52,
    prisoner_squat = 53,
    weighted_prisoner_squat = 54,
    single_leg_bench_get_up = 55,
    weighted_single_leg_bench_get_up = 56,
    single_leg_bench_squat = 57,
    weighted_single_leg_bench_squat = 58,
    single_leg_squat_on_swiss_ball = 59,
    weighted_single_leg_squat_on_swiss_ball = 60,
    squat = 61,
    weighted_squat = 62,
    squats_with_band = 63,
    staggered_squat = 64,
    weighted_staggered_squat = 65,
    step_up = 66,
    weighted_step_up = 67,
    suitcase_squats = 68,
    sumo_squat = 69,
    sumo_squat_slide_in = 70,
    weighted_sumo_squat_slide_in = 71,
    sumo_squat_to_high_pull = 72,
    sumo_squat_to_stand = 73,
    weighted_sumo_squat_to_stand = 74,
    sumo_squat_with_rotation = 75,
    weighted_sumo_squat_with_rotation = 76,
    swiss_ball_body_weight_wall_squat = 77,
    weighted_swiss_ball_wall_squat = 78,
    thrusters = 79,
    uneven_squat = 80,
    weighted_uneven_squat = 81,
    waist_slimming_squat = 82,
    wall_ball = 83,
    wide_stance_barbell_squat = 84,
    wide_stance_goblet_squat = 85,
    zercher_squat = 86,
    /// Deprecated do not use
    kbs_overhead = 87,
    squat_and_side_kick = 88,
    squat_jumps_in_n_out = 89,
    pilates_plie_squats_parallel_turned_out_flat_and_heels = 90,
    releve_straight_leg_and_knee_bent_with_one_leg_variation = 91,
    alternating_box_dumbbell_step_ups = 92,
    dumbbell_overhead_squat_single_arm = 93,
    dumbbell_squat_snatch = 94,
    medicine_ball_squat = 95,
    wall_ball_squat_and_press = 97,
    squat_american_swing = 98,
    air_squat = 100,
    dumbbell_thrusters = 101,
    overhead_barbell_squat = 102,
};

pub const TotalBodyExerciseName = enum(u16) {
    burpee = 0,
    weighted_burpee = 1,
    burpee_box_jump = 2,
    weighted_burpee_box_jump = 3,
    high_pull_burpee = 4,
    man_makers = 5,
    one_arm_burpee = 6,
    squat_thrusts = 7,
    weighted_squat_thrusts = 8,
    squat_plank_push_up = 9,
    weighted_squat_plank_push_up = 10,
    standing_t_rotation_balance = 11,
    weighted_standing_t_rotation_balance = 12,
    barbell_burpee = 13,
    burpee_box_jump_over_yes_literally_jumping_over_the_box = 15,
    burpee_box_jump_step_up_over = 16,
    lateral_barbell_burpee = 17,
    total_body_burpee_over_bar = 18,
    burpee_box_jump_over = 19,
    burpee_wheelchair = 20,
};

pub const MoveExerciseName = enum(u16) {
    arch_and_curl = 0,
    arm_circles_with_ball_band_and_weight = 1,
    arm_stretch = 2,
    back_massage = 3,
    belly_breathing = 4,
    bridge_with_ball = 5,
    diamond_leg_crunch = 6,
    diamond_leg_lift = 7,
    eight_point_shoulder_opener = 8,
    foot_rolling = 9,
    footwork = 10,
    footwork_on_disc = 11,
    forward_fold = 12,
    frog_with_band = 13,
    half_roll_up = 14,
    hamstring_curl = 15,
    hamstring_stretch = 16,
    hip_stretch = 17,
    hug_a_tree_with_ball_band_and_weight = 18,
    knee_circles = 19,
    knee_folds_on_disc = 20,
    lateral_flexion = 21,
    leg_stretch_with_band = 22,
    leg_stretch_with_leg_circles = 23,
    lower_lift_on_disc = 24,
    lunge_squat = 25,
    lunges_with_knee_lift = 26,
    mermaid_stretch = 27,
    neutral_pelvic_position = 28,
    pelvic_clocks_on_disc = 29,
    pilates_plie_squats_parallel_turned_out_flat_and_heels_with_chair = 30,
    piriformis_stretch = 31,
    plank_knee_crosses = 32,
    plank_knee_pulls = 33,
    plank_up_downs = 34,
    prayer_mudra = 35,
    psoas_lunge_stretch = 36,
    ribcage_breathing = 37,
    roll_down = 38,
    roll_up_with_weight_and_band = 39,
    saw = 40,
    scapular_stabilization = 41,
    scissors_on_disc = 42,
    seated_hip_stretchup = 43,
    seated_twist = 44,
    shaving_the_head_with_ball_band_and_weight = 45,
    spinal_twist = 46,
    spinal_twist_stretch = 47,
    spine_stretch_forward = 48,
    squat_open_arm_twist_pose = 49,
    squats_with_ball = 50,
    stand_and_hang = 51,
    standing_side_stretch = 52,
    standing_single_leg_forward_bend_with_it_band_opener = 53,
    straight_leg_crunch_with_leg_lift = 54,
    straight_leg_crunch_with_leg_lift_with_ball = 55,
    straight_leg_crunch_with_legs_crossed = 56,
    straight_leg_crunch_with_legs_crossed_with_ball = 57,
    straight_leg_diagonal_crunch = 58,
    straight_leg_diagonal_crunch_with_ball = 59,
    tailbone_curl = 60,
    throat_lock = 61,
    tick_tock_side_roll = 62,
    twist = 63,
    v_leg_crunches = 64,
    v_sit = 65,
    forward_fold_wheelchair = 66,
    forward_fold_plus_wheelchair = 67,
    arm_circles_low_forward_wheelchair = 68,
    arm_circles_mid_forward_wheelchair = 69,
    arm_circles_high_forward_wheelchair = 70,
    arm_circles_low_backward_wheelchair = 71,
    arm_circles_mid_backward_wheelchair = 72,
    arm_circles_high_backward_wheelchair = 73,
    core_twists_wheelchair = 74,
    arm_raise_wheelchair = 75,
    chest_expand_wheelchair = 76,
    arm_extend_wheelchair = 77,
    forward_bend_wheelchair = 78,
    toe_touch_wheelchair = 79,
    extended_toe_touch_wheelchair = 80,
    seated_arm_circles = 81,
    trunk_rotations = 82,
    seated_trunk_rotations = 83,
    toe_touch = 84,
};

pub const PoseExerciseName = enum(u16) {
    all_fours = 0,
    ankle_to_knee = 1,
    baby_cobra = 2,
    boat = 3,
    bound_angle = 4,
    bound_seated_single_leg_forward_bend = 5,
    bow = 6,
    bowed_half_moon = 7,
    bridge = 8,
    cat = 9,
    chair = 10,
    childs = 11,
    corpse = 12,
    cow_face = 13,
    cow = 14,
    devotional_warrior = 15,
    dolphin_plank = 16,
    dolphin = 17,
    down_dog_knee_to_nose = 18,
    down_dog_split = 19,
    down_dog_split_open_hip_bent_knee = 20,
    downward_facing_dog = 21,
    eagle = 22,
    easy_seated = 23,
    extended_puppy = 24,
    extended_side_angle = 25,
    fish = 26,
    four_limbed_staff = 27,
    full_split = 28,
    gate = 29,
    half_chair_half_ankle_to_knee = 30,
    half_moon = 31,
    head_to_knee = 32,
    heron = 33,
    heros = 34,
    high_lunge = 35,
    knees_chest_chin = 36,
    lizard = 37,
    locust = 38,
    low_lunge = 39,
    low_lunge_twist = 40,
    low_lunge_with_knee_down = 41,
    mermaid = 42,
    mountain = 43,
    one_legged_downward_facing_pose_open_hip_bent_knee = 44,
    one_legged_pigeon = 45,
    peaceful_warrior = 46,
    plank = 47,
    plow = 48,
    reclined_hand_to_foot = 49,
    revolved_half_moon = 50,
    revolved_head_to_knee = 51,
    revolved_triangle = 52,
    runners_lunge = 53,
    seated_easy_side_bend = 54,
    seated_easy_twist = 55,
    seated_long_leg_forward_bend = 56,
    seated_wide_leg_forward_bend = 57,
    shoulder_stand = 58,
    side_boat = 59,
    side_plank = 60,
    sphinx = 61,
    squat_open_arm_twist = 62,
    squat_palm_press = 63,
    staff = 64,
    standing_arms_up = 65,
    standing_forward_bend_halfway_up = 66,
    standing_forward_bend = 67,
    standing_side_opener = 68,
    standing_single_leg_forward_bend = 69,
    standing_split = 70,
    standing_wide_leg_forward_bend = 71,
    standing_wide_leg_forward_bend_with_twist = 72,
    supine_spinal_twist = 73,
    table_top = 74,
    thread_the_needle = 75,
    thunderbolt = 76,
    thunderbolt_pose_both_sides_arm_stretch = 77,
    tree = 78,
    triangle = 79,
    up_dog = 80,
    upward_facing_plank = 81,
    warrior_one = 82,
    warrior_three = 83,
    warrior_two = 84,
    wheel = 85,
    wide_side_lunge = 86,
    deep_breathing_wheelchair = 87,
    deep_breathing_low_wheelchair = 88,
    deep_breathing_mid_wheelchair = 89,
    deep_breathing_high_wheelchair = 90,
    prayer_wheelchair = 91,
    overhead_prayer_wheelchair = 92,
    cactus_wheelchair = 93,
    breathing_punches_wheelchair = 94,
    breathing_punches_extended_wheelchair = 95,
    breathing_punches_overhead_wheelchair = 96,
    breathing_punches_overhead_and_down_wheelchair = 97,
    breathing_punches_side_wheelchair = 98,
    breathing_punches_extended_side_wheelchair = 99,
    breathing_punches_overhead_side_wheelchair = 100,
    breathing_punches_overhead_and_down_side_wheelchair = 101,
    left_hand_back_wheelchair = 102,
    triangle_wheelchair = 103,
    thread_the_needle_wheelchair = 104,
    neck_flexion_and_extension_wheelchair = 105,
    neck_lateral_flexion_wheelchair = 106,
    spine_flexion_and_extension_wheelchair = 107,
    spine_rotation_wheelchair = 108,
    spine_lateral_flexion_wheelchair = 109,
    alternative_skiing_wheelchair = 110,
    reach_forward_wheelchair = 111,
    warrior_wheelchair = 112,
    reverse_warrior_wheelchair = 113,
    downward_facing_dog_to_cobra = 114,
    seated_cat_cow = 115,
};

pub const TricepsExtensionExerciseName = enum(u16) {
    bench_dip = 0,
    weighted_bench_dip = 1,
    body_weight_dip = 2,
    cable_kickback = 3,
    cable_lying_triceps_extension = 4,
    cable_overhead_triceps_extension = 5,
    dumbbell_kickback = 6,
    dumbbell_lying_triceps_extension = 7,
    ez_bar_overhead_triceps_extension = 8,
    incline_dip = 9,
    weighted_incline_dip = 10,
    incline_ez_bar_lying_triceps_extension = 11,
    lying_dumbbell_pullover_to_extension = 12,
    lying_ez_bar_triceps_extension = 13,
    lying_triceps_extension_to_close_grip_bench_press = 14,
    overhead_dumbbell_triceps_extension = 15,
    reclining_triceps_press = 16,
    reverse_grip_pressdown = 17,
    reverse_grip_triceps_pressdown = 18,
    rope_pressdown = 19,
    seated_barbell_overhead_triceps_extension = 20,
    seated_dumbbell_overhead_triceps_extension = 21,
    seated_ez_bar_overhead_triceps_extension = 22,
    seated_single_arm_overhead_dumbbell_extension = 23,
    single_arm_dumbbell_overhead_triceps_extension = 24,
    single_dumbbell_seated_overhead_triceps_extension = 25,
    single_leg_bench_dip_and_kick = 26,
    weighted_single_leg_bench_dip_and_kick = 27,
    single_leg_dip = 28,
    weighted_single_leg_dip = 29,
    static_lying_triceps_extension = 30,
    suspended_dip = 31,
    weighted_suspended_dip = 32,
    swiss_ball_dumbbell_lying_triceps_extension = 33,
    swiss_ball_ez_bar_lying_triceps_extension = 34,
    swiss_ball_ez_bar_overhead_triceps_extension = 35,
    tabletop_dip = 36,
    weighted_tabletop_dip = 37,
    triceps_extension_on_floor = 38,
    triceps_pressdown = 39,
    weighted_dip = 40,
    alternating_dumbbell_lying_triceps_extension = 41,
    triceps_press = 42,
    dumbbell_kickback_wheelchair = 43,
    overhead_dumbbell_triceps_extension_wheelchair = 44,
};

pub const WarmUpExerciseName = enum(u16) {
    quadruped_rocking = 0,
    neck_tilts = 1,
    ankle_circles = 2,
    ankle_dorsiflexion_with_band = 3,
    ankle_internal_rotation = 4,
    arm_circles = 5,
    bent_over_reach_to_sky = 6,
    cat_camel = 7,
    elbow_to_foot_lunge = 8,
    forward_and_backward_leg_swings = 9,
    groiners = 10,
    inverted_hamstring_stretch = 11,
    lateral_duck_under = 12,
    neck_rotations = 13,
    opposite_arm_and_leg_balance = 14,
    reach_roll_and_lift = 15,
    /// Deprecated do not use
    scorpion = 16,
    shoulder_circles = 17,
    side_to_side_leg_swings = 18,
    sleeper_stretch = 19,
    slide_out = 20,
    swiss_ball_hip_crossover = 21,
    swiss_ball_reach_roll_and_lift = 22,
    swiss_ball_windshield_wipers = 23,
    thoracic_rotation = 24,
    walking_high_kicks = 25,
    walking_high_knees = 26,
    walking_knee_hugs = 27,
    walking_leg_cradles = 28,
    walkout = 29,
    walkout_from_push_up_position = 30,
    biceps_stretch = 31,
    glutes_stretch = 32,
    standing_hamstring_stretch = 33,
    stretch_90_90 = 34,
    stretch_abs = 35,
    stretch_butterfly = 36,
    stretch_calf = 37,
    stretch_cat_cow = 38,
    stretch_childs_pose = 39,
    stretch_cobra = 40,
    stretch_forearms = 41,
    stretch_forward_glutes = 42,
    stretch_front_split = 43,
    stretch_hamstring = 44,
    stretch_hip_flexor_and_quad = 45,
    stretch_lat = 46,
    stretch_levator_scapulae = 47,
    stretch_lunge_with_spinal_twist = 48,
    stretch_lunging_hip_flexor = 49,
    stretch_lying_abduction = 50,
    stretch_lying_it_band = 51,
    stretch_lying_knee_to_chest = 52,
    stretch_lying_piriformis = 53,
    stretch_lying_spinal_twist = 54,
    stretch_neck = 55,
    stretch_obliques = 56,
    stretch_over_under_shoulder = 57,
    stretch_pectoral = 58,
    stretch_pigeon_pose = 59,
    stretch_piriformis = 60,
    stretch_quad = 61,
    stretch_scorpion = 62,
    stretch_shoulder = 63,
    stretch_side = 64,
    stretch_side_lunge = 65,
    stretch_side_split = 66,
    stretch_standing_it_band = 67,
    stretch_straddle = 68,
    stretch_triceps = 69,
    stretch_wall_chest_and_shoulder = 70,
    neck_rotations_wheelchair = 71,
    half_kneeling_arm_rotation = 72,
    three_way_ankle_mobilization = 73,
    /// 90_90_hip_switch
    ninety_ninety_hip_switch = 74,
    active_frog = 75,
    shoulder_sweeps = 76,
    ankle_lunges = 77,
    back_roll_foam_roller = 78,
    bear_crawl = 79,
    latissimus_dorsi_foam_roll = 80,
    reverse_t_hip_opener = 81,
    shoulder_rolls = 82,
    chest_openers = 83,
    triceps_stretch = 84,
    upper_back_stretch = 85,
    hip_circles = 86,
    ankle_stretch = 87,
    marching_in_place = 88,
    triceps_stretch_wheelchair = 89,
    upper_back_stretch_wheelchair = 90,
};

pub const RunExerciseName = enum(u16) {
    run = 0,
    walk = 1,
    jog = 2,
    sprint = 3,
    run_or_walk = 4,
    speed_walk = 5,
    warm_up = 6,
};

pub const BikeExerciseName = enum(u16) {
    bike = 0,
    ride = 1,
    sprint = 2,
};

pub const BandedExercisesExerciseName = enum(u16) {
    ab_twist = 1,
    back_extension = 2,
    bicycle_crunch = 3,
    calf_raises = 4,
    chest_press = 5,
    clam_shells = 6,
    curl = 7,
    deadbug = 8,
    deadlift = 9,
    donkey_kick = 10,
    external_rotation = 11,
    external_rotation_at_90_degree_abduction = 12,
    face_pull = 13,
    fire_hydrant = 14,
    fly = 15,
    front_raise = 16,
    glute_bridge = 17,
    hamstring_curls = 18,
    high_plank_leg_lifts = 19,
    hip_extension = 20,
    internal_rotation = 21,
    jumping_jack = 22,
    kneeling_crunch = 23,
    lateral_band_walks = 24,
    lateral_raise = 25,
    latpull = 26,
    leg_abduction = 27,
    leg_adduction = 28,
    leg_extension = 29,
    lunge = 30,
    plank = 31,
    pull_apart = 32,
    push_ups = 33,
    reverse_crunch = 34,
    row = 35,
    shoulder_abduction = 36,
    shoulder_extension = 37,
    shoulder_external_rotation = 38,
    shoulder_flexion_to_90_degrees = 39,
    side_plank_leg_lifts = 40,
    side_raise = 41,
    squat = 42,
    squat_to_press = 43,
    tricep_extension = 44,
    tricep_kickback = 45,
    upright_row = 46,
    wall_crawl_with_external_rotation = 47,
    lateral_raise_wheelchair = 49,
    triceps_extension_wheelchair = 50,
    chest_fly_incline_wheelchair = 51,
    chest_fly_decline_wheelchair = 52,
    pull_down_wheelchair = 53,
    straight_arm_pull_down_wheelchair = 54,
    curl_wheelchair = 55,
    overhead_curl_wheelchair = 56,
    face_pull_wheelchair = 57,
    around_the_world_wheelchair = 58,
    pull_apart_wheelchair = 59,
    side_curl_wheelchair = 60,
    overhead_press_wheelchair = 61,
};

pub const BattleRopeExerciseName = enum(u16) {
    alternating_figure_eight = 0,
    alternating_jump_wave = 1,
    alternating_kneeling_to_standing_wave = 2,
    alternating_lunge_wave = 3,
    alternating_squat_wave = 4,
    alternating_wave = 5,
    alternating_wave_with_lateral_shuffle = 6,
    clap_wave = 7,
    double_arm_figure_eight = 8,
    double_arm_side_to_side_snake = 9,
    double_arm_side_wave = 10,
    double_arm_slam = 11,
    double_arm_wave = 12,
    grappler_toss = 13,
    hip_toss = 14,
    in_and_out_wave = 15,
    inside_circle = 16,
    jumping_jacks = 17,
    outside_circle = 18,
    rainbow = 19,
    side_plank_wave = 20,
    sidewinder = 21,
    sitting_russian_twist = 22,
    snake_wave = 23,
    split_jack = 24,
    stage_coach = 25,
    ultimate_warrior = 26,
    upper_cuts = 27,
};

pub const EllipticalExerciseName = enum(u16) {
    elliptical = 0,
};

pub const FloorClimbExerciseName = enum(u16) {
    floor_climb = 0,
};

pub const IndoorBikeExerciseName = enum(u16) {
    air_bike = 0,
    assault_bike = 1,
    stationary_bike = 3,
};

pub const IndoorRowExerciseName = enum(u16) {
    rowing_machine = 0,
};

pub const LadderExerciseName = enum(u16) {
    agility = 0,
    speed = 1,
};

pub const SandbagExerciseName = enum(u16) {
    around_the_world = 0,
    back_squat = 1,
    bear_crawl_pull_through = 2,
    bear_hug_squat = 3,
    clean = 4,
    clean_and_press = 5,
    curl = 6,
    front_carry = 7,
    front_squat = 8,
    lunge = 9,
    overhead_press = 10,
    plank_pull_through = 11,
    rotational_lunge = 12,
    row = 13,
    russian_twist = 14,
    shouldering = 15,
    shoveling = 16,
    side_lunge = 17,
    sprint = 18,
    zercher_squat = 19,
};

pub const SledExerciseName = enum(u16) {
    backward_drag = 0,
    chest_press = 1,
    forward_drag = 2,
    low_push = 3,
    push = 4,
    row = 5,
};

pub const SledgeHammerExerciseName = enum(u16) {
    lateral_swing = 0,
    hammer_slam = 1,
};

pub const StairStepperExerciseName = enum(u16) {
    stair_stepper = 0,
};

pub const SuspensionExerciseName = enum(u16) {
    chest_fly = 0,
    chest_press = 1,
    crunch = 2,
    curl = 3,
    dip = 4,
    face_pull = 5,
    glute_bridge = 6,
    hamstring_curl = 7,
    hip_drop = 8,
    inverted_row = 9,
    knee_drive_jump = 10,
    knee_to_chest = 11,
    lat_pullover = 12,
    lunge = 13,
    mountain_climber = 14,
    pendulum = 15,
    pike = 16,
    plank = 17,
    power_pull = 18,
    pull_up = 19,
    push_up = 20,
    reverse_mountain_climber = 21,
    reverse_plank = 22,
    rollout = 23,
    row = 24,
    side_lunge = 25,
    side_plank = 26,
    single_leg_deadlift = 27,
    single_leg_squat = 28,
    sit_up = 29,
    split = 30,
    squat = 31,
    squat_jump = 32,
    tricep_press = 33,
    y_fly = 34,
};

pub const TireExerciseName = enum(u16) {
    flip = 0,
};

pub const BikeOutdoorExerciseName = enum(u16) {
    bike = 0,
};

pub const RunIndoorExerciseName = enum(u16) {
    indoor_track_run = 0,
    treadmill = 1,
};

pub const WaterType = enum(u8) {
    fresh = 0,
    salt = 1,
    en13319 = 2,
    custom = 3,
};

pub const TissueModelType = enum(u8) {
    /// Buhlmann's decompression algorithm, version C
    zhl_16c = 0,
};

pub const DiveGasStatus = enum(u8) {
    disabled = 0,
    enabled = 1,
    backup_only = 2,
};

pub const DiveAlert = enum(u8) {
    ndl_reached = 0,
    gas_switch_prompted = 1,
    near_surface = 2,
    approaching_ndl = 3,
    po2_warn = 4,
    po2_crit_high = 5,
    po2_crit_low = 6,
    time_alert = 7,
    depth_alert = 8,
    deco_ceiling_broken = 9,
    deco_complete = 10,
    safety_stop_broken = 11,
    safety_stop_complete = 12,
    cns_warning = 13,
    cns_critical = 14,
    otu_warning = 15,
    otu_critical = 16,
    ascent_critical = 17,
    alert_dismissed_by_key = 18,
    alert_dismissed_by_timeout = 19,
    battery_low = 20,
    battery_critical = 21,
    safety_stop_started = 22,
    approaching_first_deco_stop = 23,
    setpoint_switch_auto_low = 24,
    setpoint_switch_auto_high = 25,
    setpoint_switch_manual_low = 26,
    setpoint_switch_manual_high = 27,
    auto_setpoint_switch_ignored = 28,
    switched_to_open_circuit = 29,
    switched_to_closed_circuit = 30,
    tank_battery_low = 32,
    /// ccr diluent has low po2
    po2_ccr_dil_low = 33,
    /// a deco stop has been cleared
    deco_stop_cleared = 34,
    /// Target Depth Apnea Alarm triggered
    apnea_neutral_buoyancy = 35,
    /// Neutral Buoyance Apnea Alarm triggered
    apnea_target_depth = 36,
    /// Surface Apnea Alarm triggered
    apnea_surface = 37,
    /// High Speed Apnea Alarm triggered
    apnea_high_speed = 38,
    /// Low Speed Apnea Alarm triggered
    apnea_low_speed = 39,
};

pub const DiveAlarmType = enum(u8) {
    /// Alarm when a certain depth is crossed
    depth = 0,
    /// Alarm when a certain time has transpired
    time = 1,
    /// Alarm when a certain ascent or descent rate is exceeded
    speed = 2,
};

pub const DiveBacklightMode = enum(u8) {
    at_depth = 0,
    always_on = 1,
};

pub const SleepLevel = enum(u8) {
    unmeasurable = 0,
    awake = 1,
    light = 2,
    deep = 3,
    rem = 4,
};

pub const Spo2MeasurementType = enum(u8) {
    off_wrist = 0,
    spot_check = 1,
    continuous_check = 2,
    periodic = 3,
};

pub const CcrSetpointSwitchMode = enum(u8) {
    /// User switches setpoints manually
    manual = 0,
    /// Switch automatically based on depth
    automatic = 1,
};

pub const DiveGasMode = enum(u8) {
    open_circuit = 0,
    closed_circuit_diluent = 1,
};

pub const ProjectileType = enum(u8) {
    /// Arrow projectile type
    arrow = 0,
    /// Rifle cartridge projectile type
    rifle_cartridge = 1,
    /// Pistol cartridge projectile type
    pistol_cartridge = 2,
    /// Shotshell projectile type
    shotshell = 3,
    /// Air rifle pellet projectile type
    air_rifle_pellet = 4,
    /// Other projectile type
    other = 5,
};

pub const FaveroProduct = enum(u16) {
    assioma_uno = 10,
    assioma_duo = 12,
};

pub const SplitType = enum(u8) {
    ascent_split = 1,
    descent_split = 2,
    interval_active = 3,
    interval_rest = 4,
    interval_warmup = 5,
    interval_cooldown = 6,
    interval_recovery = 7,
    interval_other = 8,
    climb_active = 9,
    climb_rest = 10,
    surf_active = 11,
    run_active = 12,
    run_rest = 13,
    workout_round = 14,
    /// run/walk detection running
    rwd_run = 17,
    /// run/walk detection walking
    rwd_walk = 18,
    windsurf_active = 21,
    /// run/walk detection standing
    rwd_stand = 22,
    /// Marks the time going from ascent_split to descent_split/used in backcountry ski
    transition = 23,
    ski_lift_split = 28,
    ski_run_split = 29,
};

pub const ClimbProEvent = enum(u8) {
    approach = 0,
    start = 1,
    complete = 2,
};

pub const GasConsumptionRateType = enum(u8) {
    /// Pressure-based Surface Air Consumption
    pressure_sac = 0,
    /// Volumetric Surface Air Consumption
    volume_sac = 1,
    /// Respiratory Minute Volume
    rmv = 2,
};

pub const TapSensitivity = enum(u8) {
    high = 0,
    medium = 1,
    low = 2,
};

pub const RadarThreatLevelType = enum(u8) {
    threat_unknown = 0,
    threat_none = 1,
    threat_approaching = 2,
    threat_approaching_fast = 3,
};

pub const SleepDisruptionSeverity = enum(u8) {
    none = 0,
    low = 1,
    medium = 2,
    high = 3,
};

pub const NapPeriodFeedback = enum(u8) {
    none = 0,
    multiple_naps_during_day = 1,
    jetlag_ideal_timing_ideal_duration = 2,
    jetlag_ideal_timing_long_duration = 3,
    jetlag_late_timing_ideal_duration = 4,
    jetlag_late_timing_long_duration = 5,
    ideal_timing_ideal_duration_low_need = 6,
    ideal_timing_ideal_duration_high_need = 7,
    ideal_timing_long_duration_low_need = 8,
    ideal_timing_long_duration_high_need = 9,
    late_timing_ideal_duration_low_need = 10,
    late_timing_ideal_duration_high_need = 11,
    late_timing_long_duration_low_need = 12,
    late_timing_long_duration_high_need = 13,
    ideal_duration_low_need = 14,
    ideal_duration_high_need = 15,
    long_duration_low_need = 16,
    long_duration_high_need = 17,
};

pub const NapSource = enum(u8) {
    automatic = 0,
    manual_device = 1,
    manual_gc = 2,
};

pub const MaxMetSpeedSource = enum(u8) {
    onboard_gps = 0,
    connected_gps = 1,
    cadence = 2,
};

pub const MaxMetHeartRateSource = enum(u8) {
    /// Wrist Heart Rate Monitor
    whr = 0,
    /// Chest Strap Heart Rate Monitor
    hrm = 1,
};

pub const HrvStatus = enum(u8) {
    none = 0,
    poor = 1,
    low = 2,
    unbalanced = 3,
    balanced = 4,
};

pub const NoFlyTimeMode = enum(u8) {
    /// Standard Diver Alert Network no-fly guidance
    standard = 0,
    /// Flat 24 hour no-fly guidance
    flat_24_hours = 1,
};

/// Must be first message in file.
pub const FileIdMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getType(msg: FileIdMessage) ?File {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getManufacturer(msg: FileIdMessage) ?Manufacturer {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getSerialNumber(msg: FileIdMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32z);
        return rf.?.*.raw_value.scalar.uint32z;
    }

    /// Only set for files that are can be created/erased.
    pub fn getTimeCreated(msg: FileIdMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Only set for files that are not created/erased.
    pub fn getNumber(msg: FileIdMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Optional free form string to indicate the devices name or model
    pub fn getProductName(msg: FileIdMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) FileIdMessage {
        return FileIdMessage{ .raw_fields = raw_fields };
    }
};

pub const FileCreatorMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getSoftwareVersion(msg: FileCreatorMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getHardwareVersion(msg: FileCreatorMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) FileCreatorMessage {
        return FileCreatorMessage{ .raw_fields = raw_fields };
    }
};

pub const TimestampCorrelationMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Whole second part of UTC timestamp at the time the system timestamp was recorded.
    pub fn getTimestamp(msg: TimestampCorrelationMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Fractional part of the UTC timestamp at the time the system timestamp was recorded.
    pub fn getFractionalTimestamp(msg: TimestampCorrelationMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 32768) - 0;
    }

    /// Whole second part of the system timestamp
    pub fn getSystemTimestamp(msg: TimestampCorrelationMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Fractional part of the system timestamp
    pub fn getFractionalSystemTimestamp(msg: TimestampCorrelationMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 32768) - 0;
    }

    /// timestamp epoch expressed in local time used to convert timestamps to local time
    pub fn getLocalTimestamp(msg: TimestampCorrelationMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Millisecond part of the UTC timestamp at the time the system timestamp was recorded.
    pub fn getTimestampMs(msg: TimestampCorrelationMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Millisecond part of the system timestamp
    pub fn getSystemTimestampMs(msg: TimestampCorrelationMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) TimestampCorrelationMessage {
        return TimestampCorrelationMessage{ .raw_fields = raw_fields };
    }
};

pub const SoftwareMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: SoftwareMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getVersion(msg: SoftwareMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getPartNumber(msg: SoftwareMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SoftwareMessage {
        return SoftwareMessage{ .raw_fields = raw_fields };
    }
};

pub const SlaveDeviceMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getManufacturer(msg: SlaveDeviceMessage) ?Manufacturer {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SlaveDeviceMessage {
        return SlaveDeviceMessage{ .raw_fields = raw_fields };
    }
};

pub const CapabilitiesMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Use language_bits_x types where x is index of array.
    pub fn getLanguages(msg: CapabilitiesMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8z;
        }
        return ret;
    }

    /// Use sport_bits_x types where x is index of array.
    pub fn getSports(msg: CapabilitiesMessage, allocator: std.mem.Allocator) ?[]SportBits0 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        var ret = try allocator.alloc(SportBits0, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8z;
        }
        return ret;
    }

    pub fn getWorkoutsSupported(msg: CapabilitiesMessage) ?WorkoutCapabilities {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 21) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32z);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint32z);
    }

    pub fn getConnectivitySupported(msg: CapabilitiesMessage) ?ConnectivityCapabilities {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 23) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32z);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint32z);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) CapabilitiesMessage {
        return CapabilitiesMessage{ .raw_fields = raw_fields };
    }
};

pub const FileCapabilitiesMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: FileCapabilitiesMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getType(msg: FileCapabilitiesMessage) ?File {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getFlags(msg: FileCapabilitiesMessage) ?FileFlags {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint8z);
    }

    pub fn getDirectory(msg: FileCapabilitiesMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getMaxCount(msg: FileCapabilitiesMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getMaxSize(msg: FileCapabilitiesMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) FileCapabilitiesMessage {
        return FileCapabilitiesMessage{ .raw_fields = raw_fields };
    }
};

pub const MesgCapabilitiesMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: MesgCapabilitiesMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getFile(msg: MesgCapabilitiesMessage) ?File {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getMesgNum(msg: MesgCapabilitiesMessage) ?MesgNum {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getCountType(msg: MesgCapabilitiesMessage) ?MesgCount {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) MesgCapabilitiesMessage {
        return MesgCapabilitiesMessage{ .raw_fields = raw_fields };
    }
};

pub const FieldCapabilitiesMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: FieldCapabilitiesMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getFile(msg: FieldCapabilitiesMessage) ?File {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getMesgNum(msg: FieldCapabilitiesMessage) ?MesgNum {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getFieldNum(msg: FieldCapabilitiesMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getCount(msg: FieldCapabilitiesMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) FieldCapabilitiesMessage {
        return FieldCapabilitiesMessage{ .raw_fields = raw_fields };
    }
};

pub const DeviceSettingsMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Index into time zone arrays.
    pub fn getActiveTimeZone(msg: DeviceSettingsMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Offset from system time. Required to convert timestamp from system time to UTC.
    pub fn getUtcOffset(msg: DeviceSettingsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Offset from system time.
    pub fn getTimeOffset(msg: DeviceSettingsMessage, allocator: std.mem.Allocator) ?[]u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(u32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint32;
        }
        return ret;
    }

    /// Display mode for the time
    pub fn getTimeMode(msg: DeviceSettingsMessage, allocator: std.mem.Allocator) ?[]TimeMode {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        var ret = try allocator.alloc(TimeMode, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.enumeration;
        }
        return ret;
    }

    /// timezone offset in 1/4 hour increments
    pub fn getTimeZoneOffset(msg: DeviceSettingsMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint8);
            ret[i] = (f / 4) - 0;
        }
        return ret;
    }

    /// Mode for backlight
    pub fn getBacklightMode(msg: DeviceSettingsMessage) ?BacklightMode {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Enabled state of the activity tracker functionality
    pub fn getActivityTrackerEnabled(msg: DeviceSettingsMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 36) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// UTC timestamp used to set the devices clock and date
    pub fn getClockTime(msg: DeviceSettingsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 39) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Bitfield to configure enabled screens for each supported loop
    pub fn getPagesEnabled(msg: DeviceSettingsMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 40) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// Enabled state of the move alert
    pub fn getMoveAlertEnabled(msg: DeviceSettingsMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 46) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Display mode for the date
    pub fn getDateMode(msg: DeviceSettingsMessage) ?DateMode {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 47) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getDisplayOrientation(msg: DeviceSettingsMessage) ?DisplayOrientation {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 55) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getMountingSide(msg: DeviceSettingsMessage) ?Side {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 56) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Bitfield to indicate one page as default for each supported loop
    pub fn getDefaultPage(msg: DeviceSettingsMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 57) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// Minimum steps before an autosync can occur
    pub fn getAutosyncMinSteps(msg: DeviceSettingsMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 58) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Minimum minutes before an autosync can occur
    pub fn getAutosyncMinTime(msg: DeviceSettingsMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 59) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Enable auto-detect setting for the lactate threshold feature.
    pub fn getLactateThresholdAutodetectEnabled(msg: DeviceSettingsMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 80) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Automatically upload using BLE
    pub fn getBleAutoUploadEnabled(msg: DeviceSettingsMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 86) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Helps to conserve battery by changing modes
    pub fn getAutoSyncFrequency(msg: DeviceSettingsMessage) ?AutoSyncFrequency {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 89) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Allows setting specific activities auto-activity detect enabled/disabled settings
    pub fn getAutoActivityDetect(msg: DeviceSettingsMessage) ?AutoActivityDetect {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 90) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint32);
    }

    /// Number of screens configured to display
    pub fn getNumberOfScreens(msg: DeviceSettingsMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 94) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Smart Notification display orientation
    pub fn getSmartNotificationDisplayOrientation(msg: DeviceSettingsMessage) ?DisplayOrientation {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 95) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getTapInterface(msg: DeviceSettingsMessage) ?Switch {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 134) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Used to hold the tap threshold setting
    pub fn getTapSensitivity(msg: DeviceSettingsMessage) ?TapSensitivity {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 174) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) DeviceSettingsMessage {
        return DeviceSettingsMessage{ .raw_fields = raw_fields };
    }
};

pub const UserProfileMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: UserProfileMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    /// Used for Morning Report greeting
    pub fn getFriendlyName(msg: UserProfileMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getGender(msg: UserProfileMessage) ?Gender {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getAge(msg: UserProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getHeight(msg: UserProfileMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 100) - 0;
    }

    pub fn getWeight(msg: UserProfileMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    pub fn getLanguage(msg: UserProfileMessage) ?Language {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getElevSetting(msg: UserProfileMessage) ?DisplayMeasure {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getWeightSetting(msg: UserProfileMessage) ?DisplayMeasure {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getRestingHeartRate(msg: UserProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getDefaultMaxRunningHeartRate(msg: UserProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getDefaultMaxBikingHeartRate(msg: UserProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getDefaultMaxHeartRate(msg: UserProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getHrSetting(msg: UserProfileMessage) ?DisplayHeart {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getSpeedSetting(msg: UserProfileMessage) ?DisplayMeasure {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getDistSetting(msg: UserProfileMessage) ?DisplayMeasure {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 14) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getPowerSetting(msg: UserProfileMessage) ?DisplayPower {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 16) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getActivityClass(msg: UserProfileMessage) ?ActivityClass {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 17) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getPositionSetting(msg: UserProfileMessage) ?DisplayPosition {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 18) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getTemperatureSetting(msg: UserProfileMessage) ?DisplayMeasure {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 21) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getLocalId(msg: UserProfileMessage) ?UserLocalId {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 22) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getGlobalId(msg: UserProfileMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 23) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.byte;
        }
        return ret;
    }

    /// Typical wake time
    pub fn getWakeTime(msg: UserProfileMessage) ?LocaltimeIntoDay {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 28) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint32);
    }

    /// Typical bed time
    pub fn getSleepTime(msg: UserProfileMessage) ?LocaltimeIntoDay {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 29) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint32);
    }

    pub fn getHeightSetting(msg: UserProfileMessage) ?DisplayMeasure {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 30) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// User defined running step length set to 0 for auto length
    pub fn getUserRunningStepLength(msg: UserProfileMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 31) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    /// User defined walking step length set to 0 for auto length
    pub fn getUserWalkingStepLength(msg: UserProfileMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 32) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    pub fn getDepthSetting(msg: UserProfileMessage) ?DisplayMeasure {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 47) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getDiveCount(msg: UserProfileMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 49) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) UserProfileMessage {
        return UserProfileMessage{ .raw_fields = raw_fields };
    }
};

pub const HrmProfileMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: HrmProfileMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getEnabled(msg: HrmProfileMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getHrmAntId(msg: HrmProfileMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16z);
        return rf.?.*.raw_value.scalar.uint16z;
    }

    pub fn getLogHrv(msg: HrmProfileMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getHrmAntIdTransType(msg: HrmProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return rf.?.*.raw_value.scalar.uint8z;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HrmProfileMessage {
        return HrmProfileMessage{ .raw_fields = raw_fields };
    }
};

pub const SdmProfileMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: SdmProfileMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getEnabled(msg: SdmProfileMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getSdmAntId(msg: SdmProfileMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16z);
        return rf.?.*.raw_value.scalar.uint16z;
    }

    pub fn getSdmCalFactor(msg: SdmProfileMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    pub fn getOdometer(msg: SdmProfileMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 100) - 0;
    }

    /// Use footpod for speed source instead of GPS
    pub fn getSpeedSource(msg: SdmProfileMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getSdmAntIdTransType(msg: SdmProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return rf.?.*.raw_value.scalar.uint8z;
    }

    /// Rollover counter that can be used to extend the odometer
    pub fn getOdometerRollover(msg: SdmProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SdmProfileMessage {
        return SdmProfileMessage{ .raw_fields = raw_fields };
    }
};

pub const BikeProfileMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: BikeProfileMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getName(msg: BikeProfileMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getSport(msg: BikeProfileMessage) ?Sport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getSubSport(msg: BikeProfileMessage) ?SubSport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getOdometer(msg: BikeProfileMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 100) - 0;
    }

    pub fn getBikeSpdAntId(msg: BikeProfileMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16z);
        return rf.?.*.raw_value.scalar.uint16z;
    }

    pub fn getBikeCadAntId(msg: BikeProfileMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16z);
        return rf.?.*.raw_value.scalar.uint16z;
    }

    pub fn getBikeSpdcadAntId(msg: BikeProfileMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16z);
        return rf.?.*.raw_value.scalar.uint16z;
    }

    pub fn getBikePowerAntId(msg: BikeProfileMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16z);
        return rf.?.*.raw_value.scalar.uint16z;
    }

    pub fn getCustomWheelsize(msg: BikeProfileMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    pub fn getAutoWheelsize(msg: BikeProfileMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    pub fn getBikeWeight(msg: BikeProfileMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    pub fn getPowerCalFactor(msg: BikeProfileMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    pub fn getAutoWheelCal(msg: BikeProfileMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getAutoPowerZero(msg: BikeProfileMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getId(msg: BikeProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 14) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getSpdEnabled(msg: BikeProfileMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 15) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getCadEnabled(msg: BikeProfileMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 16) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getSpdcadEnabled(msg: BikeProfileMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 17) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getPowerEnabled(msg: BikeProfileMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 18) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getCrankLength(msg: BikeProfileMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 19) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - -110;
    }

    pub fn getEnabled(msg: BikeProfileMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 20) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getBikeSpdAntIdTransType(msg: BikeProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 21) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return rf.?.*.raw_value.scalar.uint8z;
    }

    pub fn getBikeCadAntIdTransType(msg: BikeProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 22) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return rf.?.*.raw_value.scalar.uint8z;
    }

    pub fn getBikeSpdcadAntIdTransType(msg: BikeProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 23) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return rf.?.*.raw_value.scalar.uint8z;
    }

    pub fn getBikePowerAntIdTransType(msg: BikeProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 24) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return rf.?.*.raw_value.scalar.uint8z;
    }

    /// Rollover counter that can be used to extend the odometer
    pub fn getOdometerRollover(msg: BikeProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 37) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Number of front gears
    pub fn getFrontGearNum(msg: BikeProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 38) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return rf.?.*.raw_value.scalar.uint8z;
    }

    /// Number of teeth on each gear 0 is innermost
    pub fn getFrontGear(msg: BikeProfileMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 39) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8z;
        }
        return ret;
    }

    /// Number of rear gears
    pub fn getRearGearNum(msg: BikeProfileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 40) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return rf.?.*.raw_value.scalar.uint8z;
    }

    /// Number of teeth on each gear 0 is innermost
    pub fn getRearGear(msg: BikeProfileMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 41) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8z;
        }
        return ret;
    }

    pub fn getShimanoDi2Enabled(msg: BikeProfileMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 44) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) BikeProfileMessage {
        return BikeProfileMessage{ .raw_fields = raw_fields };
    }
};

pub const ConnectivityMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Use Bluetooth for connectivity features
    pub fn getBluetoothEnabled(msg: ConnectivityMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Use Bluetooth Low Energy for connectivity features
    pub fn getBluetoothLeEnabled(msg: ConnectivityMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Use ANT for connectivity features
    pub fn getAntEnabled(msg: ConnectivityMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getName(msg: ConnectivityMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getLiveTrackingEnabled(msg: ConnectivityMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getWeatherConditionsEnabled(msg: ConnectivityMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getWeatherAlertsEnabled(msg: ConnectivityMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getAutoActivityUploadEnabled(msg: ConnectivityMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getCourseDownloadEnabled(msg: ConnectivityMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getWorkoutDownloadEnabled(msg: ConnectivityMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getGpsEphemerisDownloadEnabled(msg: ConnectivityMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getIncidentDetectionEnabled(msg: ConnectivityMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getGrouptrackEnabled(msg: ConnectivityMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) ConnectivityMessage {
        return ConnectivityMessage{ .raw_fields = raw_fields };
    }
};

pub const WatchfaceSettingsMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: WatchfaceSettingsMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getMode(msg: WatchfaceSettingsMessage) ?WatchfaceMode {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) WatchfaceSettingsMessage {
        return WatchfaceSettingsMessage{ .raw_fields = raw_fields };
    }
};

pub const OhrSettingsMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: OhrSettingsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getEnabled(msg: OhrSettingsMessage) ?Switch {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) OhrSettingsMessage {
        return OhrSettingsMessage{ .raw_fields = raw_fields };
    }
};

pub const TimeInZoneMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: TimeInZoneMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getReferenceMesg(msg: TimeInZoneMessage) ?MesgNum {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getReferenceIndex(msg: TimeInZoneMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getTimeInHrZone(msg: TimeInZoneMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getTimeInSpeedZone(msg: TimeInZoneMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getTimeInCadenceZone(msg: TimeInZoneMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getTimeInPowerZone(msg: TimeInZoneMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getHrZoneHighBoundary(msg: TimeInZoneMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8;
        }
        return ret;
    }

    pub fn getSpeedZoneHighBoundary(msg: TimeInZoneMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getCadenceZoneHighBondary(msg: TimeInZoneMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8;
        }
        return ret;
    }

    pub fn getPowerZoneHighBoundary(msg: TimeInZoneMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    pub fn getHrCalcType(msg: TimeInZoneMessage) ?HrZoneCalc {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getMaxHeartRate(msg: TimeInZoneMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getRestingHeartRate(msg: TimeInZoneMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getThresholdHeartRate(msg: TimeInZoneMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getPwrCalcType(msg: TimeInZoneMessage) ?PwrZoneCalc {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 14) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getFunctionalThresholdPower(msg: TimeInZoneMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 15) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) TimeInZoneMessage {
        return TimeInZoneMessage{ .raw_fields = raw_fields };
    }
};

pub const ZonesTargetMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMaxHeartRate(msg: ZonesTargetMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getThresholdHeartRate(msg: ZonesTargetMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getFunctionalThresholdPower(msg: ZonesTargetMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getHrCalcType(msg: ZonesTargetMessage) ?HrZoneCalc {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getPwrCalcType(msg: ZonesTargetMessage) ?PwrZoneCalc {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) ZonesTargetMessage {
        return ZonesTargetMessage{ .raw_fields = raw_fields };
    }
};

pub const SportMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getSport(msg: SportMessage) ?Sport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getSubSport(msg: SportMessage) ?SubSport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getName(msg: SportMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SportMessage {
        return SportMessage{ .raw_fields = raw_fields };
    }
};

pub const HrZoneMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: HrZoneMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getHighBpm(msg: HrZoneMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getName(msg: HrZoneMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HrZoneMessage {
        return HrZoneMessage{ .raw_fields = raw_fields };
    }
};

pub const SpeedZoneMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: SpeedZoneMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getHighValue(msg: SpeedZoneMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    pub fn getName(msg: SpeedZoneMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SpeedZoneMessage {
        return SpeedZoneMessage{ .raw_fields = raw_fields };
    }
};

pub const CadenceZoneMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: CadenceZoneMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getHighValue(msg: CadenceZoneMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getName(msg: CadenceZoneMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) CadenceZoneMessage {
        return CadenceZoneMessage{ .raw_fields = raw_fields };
    }
};

pub const PowerZoneMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: PowerZoneMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getHighValue(msg: PowerZoneMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getName(msg: PowerZoneMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) PowerZoneMessage {
        return PowerZoneMessage{ .raw_fields = raw_fields };
    }
};

pub const MetZoneMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: MetZoneMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getHighBpm(msg: MetZoneMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getCalories(msg: MetZoneMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    pub fn getFatCalories(msg: MetZoneMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 10) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) MetZoneMessage {
        return MetZoneMessage{ .raw_fields = raw_fields };
    }
};

pub const TrainingSettingsMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTargetDistance(msg: TrainingSettingsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 31) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 100) - 0;
    }

    pub fn getTargetSpeed(msg: TrainingSettingsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 32) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    pub fn getTargetTime(msg: TrainingSettingsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 33) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// A more precise target speed field
    pub fn getPreciseTargetSpeed(msg: TrainingSettingsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 153) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000000) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) TrainingSettingsMessage {
        return TrainingSettingsMessage{ .raw_fields = raw_fields };
    }
};

pub const DiveSettingsMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: DiveSettingsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getMessageIndex(msg: DiveSettingsMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getName(msg: DiveSettingsMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getModel(msg: DiveSettingsMessage) ?TissueModelType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getGfLow(msg: DiveSettingsMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getGfHigh(msg: DiveSettingsMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getWaterType(msg: DiveSettingsMessage) ?WaterType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Fresh water is usually 1000; salt water is usually 1025
    pub fn getWaterDensity(msg: DiveSettingsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// Typically 1.40
    pub fn getPo2Warn(msg: DiveSettingsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 100) - 0;
    }

    /// Typically 1.60
    pub fn getPo2Critical(msg: DiveSettingsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 100) - 0;
    }

    pub fn getPo2Deco(msg: DiveSettingsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 100) - 0;
    }

    pub fn getSafetyStopEnabled(msg: DiveSettingsMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getBottomDepth(msg: DiveSettingsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    pub fn getBottomTime(msg: DiveSettingsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getApneaCountdownEnabled(msg: DiveSettingsMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getApneaCountdownTime(msg: DiveSettingsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getBacklightMode(msg: DiveSettingsMessage) ?DiveBacklightMode {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 14) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getBacklightBrightness(msg: DiveSettingsMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 15) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getBacklightTimeout(msg: DiveSettingsMessage) ?BacklightTimeout {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 16) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint8);
    }

    /// Time between surfacing and ending the activity
    pub fn getRepeatDiveInterval(msg: DiveSettingsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 17) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1) - 0;
    }

    /// Time at safety stop (if enabled)
    pub fn getSafetyStopTime(msg: DiveSettingsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 18) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1) - 0;
    }

    pub fn getHeartRateSourceType(msg: DiveSettingsMessage) ?SourceType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 19) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Index of travel dive_gas message
    pub fn getTravelGas(msg: DiveSettingsMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 21) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    /// If low PO2 should be switched to automatically
    pub fn getCcrLowSetpointSwitchMode(msg: DiveSettingsMessage) ?CcrSetpointSwitchMode {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 22) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Target PO2 when using low setpoint
    pub fn getCcrLowSetpoint(msg: DiveSettingsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 23) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 100) - 0;
    }

    /// Depth to switch to low setpoint in automatic mode
    pub fn getCcrLowSetpointDepth(msg: DiveSettingsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 24) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// If high PO2 should be switched to automatically
    pub fn getCcrHighSetpointSwitchMode(msg: DiveSettingsMessage) ?CcrSetpointSwitchMode {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 25) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Target PO2 when using high setpoint
    pub fn getCcrHighSetpoint(msg: DiveSettingsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 26) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 100) - 0;
    }

    /// Depth to switch to high setpoint in automatic mode
    pub fn getCcrHighSetpointDepth(msg: DiveSettingsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 27) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Type of gas consumption rate to display. Some values are only valid if tank volume is known.
    pub fn getGasConsumptionDisplay(msg: DiveSettingsMessage) ?GasConsumptionRateType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 29) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Indicates whether the up key is enabled during dives
    pub fn getUpKeyEnabled(msg: DiveSettingsMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 30) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Sounds and vibration enabled or disabled in-dive
    pub fn getDiveSounds(msg: DiveSettingsMessage) ?Tone {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 35) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Usually 1.0/1.5/2.0 representing 3/4.5/6m or 10/15/20ft
    pub fn getLastStopMultiple(msg: DiveSettingsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 36) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 10) - 0;
    }

    /// Indicates which guidelines to use for no-fly surface interval.
    pub fn getNoFlyTimeMode(msg: DiveSettingsMessage) ?NoFlyTimeMode {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 37) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) DiveSettingsMessage {
        return DiveSettingsMessage{ .raw_fields = raw_fields };
    }
};

pub const DiveAlarmMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Index of the alarm
    pub fn getMessageIndex(msg: DiveAlarmMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    /// Depth setting (m) for depth type alarms
    pub fn getDepth(msg: DiveAlarmMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Time setting (s) for time type alarms
    pub fn getTime(msg: DiveAlarmMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint32);
        return (f / 1) - 0;
    }

    /// Enablement flag
    pub fn getEnabled(msg: DiveAlarmMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Alarm type setting
    pub fn getAlarmType(msg: DiveAlarmMessage) ?DiveAlarmType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Tone and Vibe setting for the alarm
    pub fn getSound(msg: DiveAlarmMessage) ?Tone {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Dive types the alarm will trigger on
    pub fn getDiveTypes(msg: DiveAlarmMessage, allocator: std.mem.Allocator) ?[]SubSport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        var ret = try allocator.alloc(SubSport, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.enumeration;
        }
        return ret;
    }

    /// Alarm ID
    pub fn getId(msg: DiveAlarmMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Show a visible pop-up for this alarm
    pub fn getPopupEnabled(msg: DiveAlarmMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Trigger the alarm on descent
    pub fn getTriggerOnDescent(msg: DiveAlarmMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Trigger the alarm on ascent
    pub fn getTriggerOnAscent(msg: DiveAlarmMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Repeat alarm each time threshold is crossed?
    pub fn getRepeating(msg: DiveAlarmMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Ascent/descent rate (mps) setting for speed type alarms
    pub fn getSpeed(msg: DiveAlarmMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint32);
        return (f / 1000) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) DiveAlarmMessage {
        return DiveAlarmMessage{ .raw_fields = raw_fields };
    }
};

pub const DiveApneaAlarmMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Index of the alarm
    pub fn getMessageIndex(msg: DiveApneaAlarmMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    /// Depth setting (m) for depth type alarms
    pub fn getDepth(msg: DiveApneaAlarmMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Time setting (s) for time type alarms
    pub fn getTime(msg: DiveApneaAlarmMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint32);
        return (f / 1) - 0;
    }

    /// Enablement flag
    pub fn getEnabled(msg: DiveApneaAlarmMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Alarm type setting
    pub fn getAlarmType(msg: DiveApneaAlarmMessage) ?DiveAlarmType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Tone and Vibe setting for the alarm.
    pub fn getSound(msg: DiveApneaAlarmMessage) ?Tone {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Dive types the alarm will trigger on
    pub fn getDiveTypes(msg: DiveApneaAlarmMessage, allocator: std.mem.Allocator) ?[]SubSport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        var ret = try allocator.alloc(SubSport, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.enumeration;
        }
        return ret;
    }

    /// Alarm ID
    pub fn getId(msg: DiveApneaAlarmMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Show a visible pop-up for this alarm
    pub fn getPopupEnabled(msg: DiveApneaAlarmMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Trigger the alarm on descent
    pub fn getTriggerOnDescent(msg: DiveApneaAlarmMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Trigger the alarm on ascent
    pub fn getTriggerOnAscent(msg: DiveApneaAlarmMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Repeat alarm each time threshold is crossed?
    pub fn getRepeating(msg: DiveApneaAlarmMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Ascent/descent rate (mps) setting for speed type alarms
    pub fn getSpeed(msg: DiveApneaAlarmMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint32);
        return (f / 1000) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) DiveApneaAlarmMessage {
        return DiveApneaAlarmMessage{ .raw_fields = raw_fields };
    }
};

pub const DiveGasMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: DiveGasMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getHeliumContent(msg: DiveGasMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getOxygenContent(msg: DiveGasMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getStatus(msg: DiveGasMessage) ?DiveGasStatus {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getMode(msg: DiveGasMessage) ?DiveGasMode {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) DiveGasMessage {
        return DiveGasMessage{ .raw_fields = raw_fields };
    }
};

pub const GoalMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: GoalMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getSport(msg: GoalMessage) ?Sport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getSubSport(msg: GoalMessage) ?SubSport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getStartDate(msg: GoalMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getEndDate(msg: GoalMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getType(msg: GoalMessage) ?Goal {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getValue(msg: GoalMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getRepeat(msg: GoalMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getTargetValue(msg: GoalMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getRecurrence(msg: GoalMessage) ?GoalRecurrence {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getRecurrenceValue(msg: GoalMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getEnabled(msg: GoalMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getSource(msg: GoalMessage) ?GoalSource {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) GoalMessage {
        return GoalMessage{ .raw_fields = raw_fields };
    }
};

pub const ActivityMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: ActivityMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Exclude pauses
    pub fn getTotalTimerTime(msg: ActivityMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getNumSessions(msg: ActivityMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getType(msg: ActivityMessage) ?Activity {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getEvent(msg: ActivityMessage) ?Event {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getEventType(msg: ActivityMessage) ?EventType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// timestamp epoch expressed in local time, used to convert activity timestamps to local time
    pub fn getLocalTimestamp(msg: ActivityMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getEventGroup(msg: ActivityMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) ActivityMessage {
        return ActivityMessage{ .raw_fields = raw_fields };
    }
};

pub const SessionMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Selected bit is set for the current session.
    pub fn getMessageIndex(msg: SessionMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getTimestamp(msg: SessionMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// session
    pub fn getEvent(msg: SessionMessage) ?Event {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// stop
    pub fn getEventType(msg: SessionMessage) ?EventType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getStartTime(msg: SessionMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getStartPositionLat(msg: SessionMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getStartPositionLong(msg: SessionMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getSport(msg: SessionMessage) ?Sport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getSubSport(msg: SessionMessage) ?SubSport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Time (includes pauses)
    pub fn getTotalElapsedTime(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Timer Time (excludes pauses)
    pub fn getTotalTimerTime(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getTotalDistance(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 100) - 0;
    }

    pub fn getTotalCalories(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getTotalFatCalories(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// total_distance / total_timer_time
    pub fn getAvgSpeed(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 14) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    pub fn getMaxSpeed(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 15) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    /// average heart rate (excludes pause time)
    pub fn getAvgHeartRate(msg: SessionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 16) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getMaxHeartRate(msg: SessionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 17) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// total_power / total_timer_time if non_zero_avg_power otherwise total_power / total_elapsed_time
    pub fn getAvgPower(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 20) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getMaxPower(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 21) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getTotalAscent(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 22) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getTotalDescent(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 23) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getTotalTrainingEffect(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 24) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 10) - 0;
    }

    pub fn getFirstLapIndex(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 25) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getNumLaps(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 26) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getEventGroup(msg: SessionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 27) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getTrigger(msg: SessionMessage) ?SessionTrigger {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 28) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// North east corner latitude
    pub fn getNecLat(msg: SessionMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 29) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    /// North east corner longitude
    pub fn getNecLong(msg: SessionMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 30) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    /// South west corner latitude
    pub fn getSwcLat(msg: SessionMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 31) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    /// South west corner longitude
    pub fn getSwcLong(msg: SessionMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 32) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    /// # of lengths of swim pool
    pub fn getNumLengths(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 33) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getNormalizedPower(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 34) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getTrainingStressScore(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 35) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    pub fn getIntensityFactor(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 36) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    pub fn getLeftRightBalance(msg: SessionMessage) ?LeftRightBalance100 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 37) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getEndPositionLat(msg: SessionMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 38) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getEndPositionLong(msg: SessionMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 39) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getAvgStrokeCount(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 41) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 10) - 0;
    }

    pub fn getAvgStrokeDistance(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 42) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getSwimStroke(msg: SessionMessage) ?SwimStroke {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 43) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getPoolLength(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 44) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getThresholdPower(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 45) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getPoolLengthUnit(msg: SessionMessage) ?DisplayMeasure {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 46) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// # of active lengths of swim pool
    pub fn getNumActiveLengths(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 47) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getTotalWork(msg: SessionMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 48) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getAvgAltitude(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 49) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 5) - 500;
    }

    pub fn getMaxAltitude(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 50) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 5) - 500;
    }

    pub fn getGpsAccuracy(msg: SessionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 51) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getAvgGrade(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 52) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn getAvgPosGrade(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 53) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn getAvgNegGrade(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 54) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn getMaxPosGrade(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 55) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn getMaxNegGrade(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 56) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn getAvgTemperature(msg: SessionMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 57) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    pub fn getMaxTemperature(msg: SessionMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 58) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    pub fn getTotalMovingTime(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 59) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getAvgPosVerticalSpeed(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 60) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 1000) - 0;
    }

    pub fn getAvgNegVerticalSpeed(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 61) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 1000) - 0;
    }

    pub fn getMaxPosVerticalSpeed(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 62) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 1000) - 0;
    }

    pub fn getMaxNegVerticalSpeed(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 63) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 1000) - 0;
    }

    pub fn getMinHeartRate(msg: SessionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 64) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getTimeInHrZone(msg: SessionMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 65) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getTimeInSpeedZone(msg: SessionMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 66) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getTimeInCadenceZone(msg: SessionMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 67) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getTimeInPowerZone(msg: SessionMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 68) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getAvgLapTime(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 69) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getBestLapIndex(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 70) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getMinAltitude(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 71) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 5) - 500;
    }

    pub fn getActiveTime(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 78) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getPlayerScore(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 82) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getOpponentScore(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 83) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getOpponentName(msg: SessionMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 84) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    /// stroke_type enum used as the index
    pub fn getStrokeCount(msg: SessionMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 85) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// zone number used as the index
    pub fn getZoneCount(msg: SessionMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 86) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    pub fn getMaxBallSpeed(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 87) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getAvgBallSpeed(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 88) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getAvgVerticalOscillation(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 89) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    pub fn getAvgStanceTimePercent(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 90) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getAvgStanceTime(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 91) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    /// fractional part of the avg_cadence
    pub fn getAvgFractionalCadence(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 92) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 128) - 0;
    }

    /// fractional part of the max_cadence
    pub fn getMaxFractionalCadence(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 93) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 128) - 0;
    }

    /// fractional part of the total_cycles
    pub fn getTotalFractionalCycles(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 94) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 128) - 0;
    }

    /// Avg saturated and unsaturated hemoglobin
    pub fn getAvgTotalHemoglobinConc(msg: SessionMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 95) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 100) - 0;
        }
        return ret;
    }

    /// Min saturated and unsaturated hemoglobin
    pub fn getMinTotalHemoglobinConc(msg: SessionMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 96) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 100) - 0;
        }
        return ret;
    }

    /// Max saturated and unsaturated hemoglobin
    pub fn getMaxTotalHemoglobinConc(msg: SessionMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 97) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 100) - 0;
        }
        return ret;
    }

    /// Avg percentage of hemoglobin saturated with oxygen
    pub fn getAvgSaturatedHemoglobinPercent(msg: SessionMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 98) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 10) - 0;
        }
        return ret;
    }

    /// Min percentage of hemoglobin saturated with oxygen
    pub fn getMinSaturatedHemoglobinPercent(msg: SessionMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 99) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 10) - 0;
        }
        return ret;
    }

    /// Max percentage of hemoglobin saturated with oxygen
    pub fn getMaxSaturatedHemoglobinPercent(msg: SessionMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 100) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 10) - 0;
        }
        return ret;
    }

    pub fn getAvgLeftTorqueEffectiveness(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 101) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getAvgRightTorqueEffectiveness(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 102) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getAvgLeftPedalSmoothness(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 103) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getAvgRightPedalSmoothness(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 104) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getAvgCombinedPedalSmoothness(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 105) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    /// Sport name from associated sport mesg
    pub fn getSportProfileName(msg: SessionMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 110) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getSportIndex(msg: SessionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 111) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Total time spend in the standing position
    pub fn getTimeStanding(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 112) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Number of transitions to the standing state
    pub fn getStandCount(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 113) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Average platform center offset Left
    pub fn getAvgLeftPco(msg: SessionMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 114) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    /// Average platform center offset Right
    pub fn getAvgRightPco(msg: SessionMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 115) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    /// Average left power phase angles. Indexes defined by power_phase_type.
    pub fn getAvgLeftPowerPhase(msg: SessionMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 116) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    /// Average left power phase peak angles. Data value indexes defined by power_phase_type.
    pub fn getAvgLeftPowerPhasePeak(msg: SessionMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 117) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    /// Average right power phase angles. Data value indexes defined by power_phase_type.
    pub fn getAvgRightPowerPhase(msg: SessionMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 118) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    /// Average right power phase peak angles data value indexes defined by power_phase_type.
    pub fn getAvgRightPowerPhasePeak(msg: SessionMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 119) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    /// Average power by position. Data value indexes defined by rider_position_type.
    pub fn getAvgPowerPosition(msg: SessionMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 120) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// Maximum power by position. Data value indexes defined by rider_position_type.
    pub fn getMaxPowerPosition(msg: SessionMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 121) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// Average cadence by position. Data value indexes defined by rider_position_type.
    pub fn getAvgCadencePosition(msg: SessionMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 122) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8;
        }
        return ret;
    }

    /// Maximum cadence by position. Data value indexes defined by rider_position_type.
    pub fn getMaxCadencePosition(msg: SessionMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 123) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8;
        }
        return ret;
    }

    /// total_distance / total_timer_time
    pub fn getEnhancedAvgSpeed(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 124) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getEnhancedMaxSpeed(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 125) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getEnhancedAvgAltitude(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 126) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 5) - 500;
    }

    pub fn getEnhancedMinAltitude(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 127) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 5) - 500;
    }

    pub fn getEnhancedMaxAltitude(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 128) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 5) - 500;
    }

    /// lev average motor power during session
    pub fn getAvgLevMotorPower(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 129) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// lev maximum motor power during session
    pub fn getMaxLevMotorPower(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 130) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// lev battery consumption during session
    pub fn getLevBatteryConsumption(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 131) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getAvgVerticalRatio(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 132) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getAvgStanceTimeBalance(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 133) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getAvgStepLength(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 134) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    pub fn getTotalAnaerobicTrainingEffect(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 137) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 10) - 0;
    }

    pub fn getAvgVam(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 139) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    /// 0 if above water
    pub fn getAvgDepth(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 140) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// 0 if above water
    pub fn getMaxDepth(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 141) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Time since end of last dive
    pub fn getSurfaceInterval(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 142) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1) - 0;
    }

    pub fn getStartCns(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 143) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 1) - 0;
    }

    pub fn getEndCns(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 144) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 1) - 0;
    }

    pub fn getStartN2(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 145) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1) - 0;
    }

    pub fn getEndN2(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 146) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1) - 0;
    }

    pub fn getAvgRespirationRate(msg: SessionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 147) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getMaxRespirationRate(msg: SessionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 148) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getMinRespirationRate(msg: SessionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 149) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getMinTemperature(msg: SessionMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 150) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    pub fn getO2Toxicity(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 155) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getDiveNumber(msg: SessionMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 156) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getTrainingLoadPeak(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 168) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint32);
        return (f / 65536) - 0;
    }

    pub fn getEnhancedAvgRespirationRate(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 169) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getEnhancedMaxRespirationRate(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 170) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getEnhancedMinRespirationRate(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 180) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    /// The grit score estimates how challenging a route could be for a cyclist in terms of time spent going over sharp turns or large grade slopes.
    pub fn getTotalGrit(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 181) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// The flow score estimates how long distance wise a cyclist deaccelerates over intervals where deacceleration is unnecessary such as smooth turns or small grade angle intervals.
    pub fn getTotalFlow(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 182) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    pub fn getJumpCount(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 183) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// The grit score estimates how challenging a route could be for a cyclist in terms of time spent going over sharp turns or large grade slopes.
    pub fn getAvgGrit(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 186) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// The flow score estimates how long distance wise a cyclist deaccelerates over intervals where deacceleration is unnecessary such as smooth turns or small grade angle intervals.
    pub fn getAvgFlow(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 187) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// A 0-100 scale representing how a user felt while performing a workout. Low values are considered feeling bad, while high values are good.
    pub fn getWorkoutFeel(msg: SessionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 192) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Common Borg CR10 / 0-10 RPE scale, multiplied 10x.. Aggregate score for all workouts in a single session.
    pub fn getWorkoutRpe(msg: SessionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 193) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Average SPO2 for the monitoring session
    pub fn getAvgSpo2(msg: SessionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 194) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Average stress for the monitoring session
    pub fn getAvgStress(msg: SessionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 195) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getMetabolicCalories(msg: SessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 196) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Standard deviation of R-R interval (SDRR) - Heart rate variability measure most useful for wellness users.
    pub fn getSdrrHrv(msg: SessionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 197) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Root mean square successive difference (RMSSD) - Heart rate variability measure most useful for athletes
    pub fn getRmssdHrv(msg: SessionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 198) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// fractional part of total_ascent
    pub fn getTotalFractionalAscent(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 199) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 100) - 0;
    }

    /// fractional part of total_descent
    pub fn getTotalFractionalDescent(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 200) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 100) - 0;
    }

    pub fn getAvgCoreTemperature(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 208) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getMinCoreTemperature(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 209) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getMaxCoreTemperature(msg: SessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 210) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SessionMessage {
        return SessionMessage{ .raw_fields = raw_fields };
    }
};

pub const LapMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: LapMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getTimestamp(msg: LapMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getEvent(msg: LapMessage) ?Event {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getEventType(msg: LapMessage) ?EventType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getStartTime(msg: LapMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getStartPositionLat(msg: LapMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getStartPositionLong(msg: LapMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getEndPositionLat(msg: LapMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getEndPositionLong(msg: LapMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    /// Time (includes pauses)
    pub fn getTotalElapsedTime(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Timer Time (excludes pauses)
    pub fn getTotalTimerTime(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getTotalDistance(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 100) - 0;
    }

    pub fn getTotalCalories(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// If New Leaf
    pub fn getTotalFatCalories(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getAvgSpeed(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    pub fn getMaxSpeed(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 14) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    pub fn getAvgHeartRate(msg: LapMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 15) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getMaxHeartRate(msg: LapMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 16) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// total_power / total_timer_time if non_zero_avg_power otherwise total_power / total_elapsed_time
    pub fn getAvgPower(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 19) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getMaxPower(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 20) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getTotalAscent(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 21) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getTotalDescent(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 22) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getIntensity(msg: LapMessage) ?Intensity {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 23) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getLapTrigger(msg: LapMessage) ?LapTrigger {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 24) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getSport(msg: LapMessage) ?Sport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 25) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getEventGroup(msg: LapMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 26) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// # of lengths of swim pool
    pub fn getNumLengths(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 32) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getNormalizedPower(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 33) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getLeftRightBalance(msg: LapMessage) ?LeftRightBalance100 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 34) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getFirstLengthIndex(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 35) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getAvgStrokeDistance(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 37) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getSwimStroke(msg: LapMessage) ?SwimStroke {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 38) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getSubSport(msg: LapMessage) ?SubSport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 39) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// # of active lengths of swim pool
    pub fn getNumActiveLengths(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 40) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getTotalWork(msg: LapMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 41) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getAvgAltitude(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 42) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 5) - 500;
    }

    pub fn getMaxAltitude(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 43) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 5) - 500;
    }

    pub fn getGpsAccuracy(msg: LapMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 44) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getAvgGrade(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 45) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn getAvgPosGrade(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 46) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn getAvgNegGrade(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 47) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn getMaxPosGrade(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 48) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn getMaxNegGrade(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 49) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn getAvgTemperature(msg: LapMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 50) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    pub fn getMaxTemperature(msg: LapMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 51) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    pub fn getTotalMovingTime(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 52) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getAvgPosVerticalSpeed(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 53) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 1000) - 0;
    }

    pub fn getAvgNegVerticalSpeed(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 54) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 1000) - 0;
    }

    pub fn getMaxPosVerticalSpeed(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 55) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 1000) - 0;
    }

    pub fn getMaxNegVerticalSpeed(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 56) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 1000) - 0;
    }

    pub fn getTimeInHrZone(msg: LapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 57) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getTimeInSpeedZone(msg: LapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 58) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getTimeInCadenceZone(msg: LapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 59) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getTimeInPowerZone(msg: LapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 60) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getRepetitionNum(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 61) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getMinAltitude(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 62) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 5) - 500;
    }

    pub fn getMinHeartRate(msg: LapMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 63) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getActiveTime(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 70) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getWktStepIndex(msg: LapMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 71) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getOpponentScore(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 74) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// stroke_type enum used as the index
    pub fn getStrokeCount(msg: LapMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 75) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// zone number used as the index
    pub fn getZoneCount(msg: LapMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 76) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    pub fn getAvgVerticalOscillation(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 77) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    pub fn getAvgStanceTimePercent(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 78) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getAvgStanceTime(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 79) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    /// fractional part of the avg_cadence
    pub fn getAvgFractionalCadence(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 80) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 128) - 0;
    }

    /// fractional part of the max_cadence
    pub fn getMaxFractionalCadence(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 81) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 128) - 0;
    }

    /// fractional part of the total_cycles
    pub fn getTotalFractionalCycles(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 82) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 128) - 0;
    }

    pub fn getPlayerScore(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 83) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Avg saturated and unsaturated hemoglobin
    pub fn getAvgTotalHemoglobinConc(msg: LapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 84) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 100) - 0;
        }
        return ret;
    }

    /// Min saturated and unsaturated hemoglobin
    pub fn getMinTotalHemoglobinConc(msg: LapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 85) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 100) - 0;
        }
        return ret;
    }

    /// Max saturated and unsaturated hemoglobin
    pub fn getMaxTotalHemoglobinConc(msg: LapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 86) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 100) - 0;
        }
        return ret;
    }

    /// Avg percentage of hemoglobin saturated with oxygen
    pub fn getAvgSaturatedHemoglobinPercent(msg: LapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 87) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 10) - 0;
        }
        return ret;
    }

    /// Min percentage of hemoglobin saturated with oxygen
    pub fn getMinSaturatedHemoglobinPercent(msg: LapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 88) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 10) - 0;
        }
        return ret;
    }

    /// Max percentage of hemoglobin saturated with oxygen
    pub fn getMaxSaturatedHemoglobinPercent(msg: LapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 89) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 10) - 0;
        }
        return ret;
    }

    pub fn getAvgLeftTorqueEffectiveness(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 91) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getAvgRightTorqueEffectiveness(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 92) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getAvgLeftPedalSmoothness(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 93) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getAvgRightPedalSmoothness(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 94) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getAvgCombinedPedalSmoothness(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 95) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    /// Total time spent in the standing position
    pub fn getTimeStanding(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 98) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Number of transitions to the standing state
    pub fn getStandCount(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 99) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Average left platform center offset
    pub fn getAvgLeftPco(msg: LapMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 100) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    /// Average right platform center offset
    pub fn getAvgRightPco(msg: LapMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 101) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    /// Average left power phase angles. Data value indexes defined by power_phase_type.
    pub fn getAvgLeftPowerPhase(msg: LapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 102) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    /// Average left power phase peak angles. Data value indexes defined by power_phase_type.
    pub fn getAvgLeftPowerPhasePeak(msg: LapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 103) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    /// Average right power phase angles. Data value indexes defined by power_phase_type.
    pub fn getAvgRightPowerPhase(msg: LapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 104) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    /// Average right power phase peak angles. Data value indexes defined by power_phase_type.
    pub fn getAvgRightPowerPhasePeak(msg: LapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 105) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    /// Average power by position. Data value indexes defined by rider_position_type.
    pub fn getAvgPowerPosition(msg: LapMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 106) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// Maximum power by position. Data value indexes defined by rider_position_type.
    pub fn getMaxPowerPosition(msg: LapMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 107) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// Average cadence by position. Data value indexes defined by rider_position_type.
    pub fn getAvgCadencePosition(msg: LapMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 108) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8;
        }
        return ret;
    }

    /// Maximum cadence by position. Data value indexes defined by rider_position_type.
    pub fn getMaxCadencePosition(msg: LapMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 109) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8;
        }
        return ret;
    }

    pub fn getEnhancedAvgSpeed(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 110) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getEnhancedMaxSpeed(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 111) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getEnhancedAvgAltitude(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 112) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 5) - 500;
    }

    pub fn getEnhancedMinAltitude(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 113) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 5) - 500;
    }

    pub fn getEnhancedMaxAltitude(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 114) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 5) - 500;
    }

    /// lev average motor power during lap
    pub fn getAvgLevMotorPower(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 115) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// lev maximum motor power during lap
    pub fn getMaxLevMotorPower(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 116) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// lev battery consumption during lap
    pub fn getLevBatteryConsumption(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 117) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getAvgVerticalRatio(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 118) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getAvgStanceTimeBalance(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 119) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getAvgStepLength(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 120) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    pub fn getAvgVam(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 121) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    /// 0 if above water
    pub fn getAvgDepth(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 122) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// 0 if above water
    pub fn getMaxDepth(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 123) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getMinTemperature(msg: LapMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 124) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    pub fn getEnhancedAvgRespirationRate(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 136) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getEnhancedMaxRespirationRate(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 137) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getAvgRespirationRate(msg: LapMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 147) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getMaxRespirationRate(msg: LapMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 148) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// The grit score estimates how challenging a route could be for a cyclist in terms of time spent going over sharp turns or large grade slopes.
    pub fn getTotalGrit(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 149) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// The flow score estimates how long distance wise a cyclist deaccelerates over intervals where deacceleration is unnecessary such as smooth turns or small grade angle intervals.
    pub fn getTotalFlow(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 150) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    pub fn getJumpCount(msg: LapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 151) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// The grit score estimates how challenging a route could be for a cyclist in terms of time spent going over sharp turns or large grade slopes.
    pub fn getAvgGrit(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 153) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// The flow score estimates how long distance wise a cyclist deaccelerates over intervals where deacceleration is unnecessary such as smooth turns or small grade angle intervals.
    pub fn getAvgFlow(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 154) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// fractional part of total_ascent
    pub fn getTotalFractionalAscent(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 156) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 100) - 0;
    }

    /// fractional part of total_descent
    pub fn getTotalFractionalDescent(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 157) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 100) - 0;
    }

    pub fn getAvgCoreTemperature(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 158) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getMinCoreTemperature(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 159) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getMaxCoreTemperature(msg: LapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 160) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) LapMessage {
        return LapMessage{ .raw_fields = raw_fields };
    }
};

pub const LengthMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: LengthMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getTimestamp(msg: LengthMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getEvent(msg: LengthMessage) ?Event {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getEventType(msg: LengthMessage) ?EventType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getStartTime(msg: LengthMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getTotalElapsedTime(msg: LengthMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getTotalTimerTime(msg: LengthMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getTotalStrokes(msg: LengthMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getAvgSpeed(msg: LengthMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    pub fn getSwimStroke(msg: LengthMessage) ?SwimStroke {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getAvgSwimmingCadence(msg: LengthMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getEventGroup(msg: LengthMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getTotalCalories(msg: LengthMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getLengthType(msg: LengthMessage) ?LengthType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getPlayerScore(msg: LengthMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 18) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getOpponentScore(msg: LengthMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 19) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// stroke_type enum used as the index
    pub fn getStrokeCount(msg: LengthMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 20) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// zone number used as the index
    pub fn getZoneCount(msg: LengthMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 21) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    pub fn getEnhancedAvgRespirationRate(msg: LengthMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 22) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getEnhancedMaxRespirationRate(msg: LengthMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 23) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getAvgRespirationRate(msg: LengthMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 24) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getMaxRespirationRate(msg: LengthMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 25) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) LengthMessage {
        return LengthMessage{ .raw_fields = raw_fields };
    }
};

pub const RecordMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: RecordMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getPositionLat(msg: RecordMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getPositionLong(msg: RecordMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getAltitude(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 5) - 500;
    }

    pub fn getHeartRate(msg: RecordMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getCadence(msg: RecordMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getDistance(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 100) - 0;
    }

    pub fn getSpeed(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    pub fn getPower(msg: RecordMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getGrade(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    /// Relative. 0 is none 254 is Max.
    pub fn getResistance(msg: RecordMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getTimeFromCourse(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint32);
        return (f / 1000) - 0;
    }

    pub fn getCycleLength(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 100) - 0;
    }

    pub fn getTemperature(msg: RecordMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    /// Speed at 1s intervals. Timestamp field indicates time of last array element.
    pub fn getSpeed1s(msg: RecordMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 17) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 16) - 0;
        }
        return ret;
    }

    pub fn getCycles(msg: RecordMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 18) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getTotalCycles(msg: RecordMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 19) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getCompressedAccumulatedPower(msg: RecordMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 28) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getAccumulatedPower(msg: RecordMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 29) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getLeftRightBalance(msg: RecordMessage) ?LeftRightBalance {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 30) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint8);
    }

    pub fn getGpsAccuracy(msg: RecordMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 31) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getVerticalSpeed(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 32) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 1000) - 0;
    }

    pub fn getCalories(msg: RecordMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 33) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getVerticalOscillation(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 39) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    pub fn getStanceTimePercent(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 40) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getStanceTime(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 41) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    pub fn getActivityType(msg: RecordMessage) ?ActivityType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 42) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getLeftTorqueEffectiveness(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 43) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getRightTorqueEffectiveness(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 44) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getLeftPedalSmoothness(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 45) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getRightPedalSmoothness(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 46) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getCombinedPedalSmoothness(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 47) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getTime128(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 48) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 128) - 0;
    }

    pub fn getStrokeType(msg: RecordMessage) ?StrokeType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 49) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getZone(msg: RecordMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 50) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getBallSpeed(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 51) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    /// Log cadence and fractional cadence for backwards compatability
    pub fn getCadence256(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 52) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 256) - 0;
    }

    pub fn getFractionalCadence(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 53) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 128) - 0;
    }

    /// Total saturated and unsaturated hemoglobin
    pub fn getTotalHemoglobinConc(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 54) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    /// Min saturated and unsaturated hemoglobin
    pub fn getTotalHemoglobinConcMin(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 55) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    /// Max saturated and unsaturated hemoglobin
    pub fn getTotalHemoglobinConcMax(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 56) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    /// Percentage of hemoglobin saturated with oxygen
    pub fn getSaturatedHemoglobinPercent(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 57) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    /// Min percentage of hemoglobin saturated with oxygen
    pub fn getSaturatedHemoglobinPercentMin(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 58) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    /// Max percentage of hemoglobin saturated with oxygen
    pub fn getSaturatedHemoglobinPercentMax(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 59) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    pub fn getDeviceIndex(msg: RecordMessage) ?DeviceIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 62) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint8);
    }

    /// Left platform center offset
    pub fn getLeftPco(msg: RecordMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 67) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    /// Right platform center offset
    pub fn getRightPco(msg: RecordMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 68) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    /// Left power phase angles. Data value indexes defined by power_phase_type.
    pub fn getLeftPowerPhase(msg: RecordMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 69) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    /// Left power phase peak angles. Data value indexes defined by power_phase_type.
    pub fn getLeftPowerPhasePeak(msg: RecordMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 70) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    /// Right power phase angles. Data value indexes defined by power_phase_type.
    pub fn getRightPowerPhase(msg: RecordMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 71) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    /// Right power phase peak angles. Data value indexes defined by power_phase_type.
    pub fn getRightPowerPhasePeak(msg: RecordMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 72) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    pub fn getEnhancedSpeed(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 73) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getEnhancedAltitude(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 78) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 5) - 500;
    }

    /// lev battery state of charge
    pub fn getBatterySoc(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 81) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    /// lev motor power
    pub fn getMotorPower(msg: RecordMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 82) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getVerticalRatio(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 83) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getStanceTimeBalance(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 84) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getStepLength(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 85) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    /// Supports larger cycle sizes needed for paddlesports. Max cycle size: 655.35
    pub fn getCycleLength16(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 87) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    /// Includes atmospheric pressure
    pub fn getAbsolutePressure(msg: RecordMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 91) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// 0 if above water
    pub fn getDepth(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 92) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// 0 if above water
    pub fn getNextStopDepth(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 93) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getNextStopTime(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 94) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1) - 0;
    }

    pub fn getTimeToSurface(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 95) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1) - 0;
    }

    pub fn getNdlTime(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 96) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1) - 0;
    }

    pub fn getCnsLoad(msg: RecordMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 97) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getN2Load(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 98) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1) - 0;
    }

    pub fn getRespirationRate(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 99) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 1) - 0;
    }

    pub fn getEnhancedRespirationRate(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 108) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    /// The grit score estimates how challenging a route could be for a cyclist in terms of time spent going over sharp turns or large grade slopes.
    pub fn getGrit(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 114) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// The flow score estimates how long distance wise a cyclist deaccelerates over intervals where deacceleration is unnecessary such as smooth turns or small grade angle intervals.
    pub fn getFlow(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 115) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// Current Stress value
    pub fn getCurrentStress(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 116) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getEbikeTravelRange(msg: RecordMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 117) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getEbikeBatteryLevel(msg: RecordMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 118) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getEbikeAssistMode(msg: RecordMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 119) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getEbikeAssistLevelPercent(msg: RecordMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 120) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getAirTimeRemaining(msg: RecordMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 123) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Pressure-based surface air consumption
    pub fn getPressureSac(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 124) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    /// Volumetric surface air consumption
    pub fn getVolumeSac(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 125) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    /// Respiratory minute volume
    pub fn getRmv(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 126) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getAscentRate(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 127) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint32);
        return (f / 1000) - 0;
    }

    /// Current partial pressure of oxygen
    pub fn getPo2(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 129) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 100) - 0;
    }

    pub fn getCoreTemperature(msg: RecordMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 139) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) RecordMessage {
        return RecordMessage{ .raw_fields = raw_fields };
    }
};

pub const EventMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: EventMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getEvent(msg: EventMessage) ?Event {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getEventType(msg: EventMessage) ?EventType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getData16(msg: EventMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getEventGroup(msg: EventMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Do not populate directly. Autogenerated by decoder for sport_point subfield components
    pub fn getScore(msg: EventMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Do not populate directly. Autogenerated by decoder for sport_point subfield components
    pub fn getOpponentScore(msg: EventMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Do not populate directly. Autogenerated by decoder for gear_change subfield components. Front gear number. 1 is innermost.
    pub fn getFrontGearNum(msg: EventMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return rf.?.*.raw_value.scalar.uint8z;
    }

    /// Do not populate directly. Autogenerated by decoder for gear_change subfield components. Number of front teeth.
    pub fn getFrontGear(msg: EventMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return rf.?.*.raw_value.scalar.uint8z;
    }

    /// Do not populate directly. Autogenerated by decoder for gear_change subfield components. Rear gear number. 1 is innermost.
    pub fn getRearGearNum(msg: EventMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return rf.?.*.raw_value.scalar.uint8z;
    }

    /// Do not populate directly. Autogenerated by decoder for gear_change subfield components. Number of rear teeth.
    pub fn getRearGear(msg: EventMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return rf.?.*.raw_value.scalar.uint8z;
    }

    pub fn getDeviceIndex(msg: EventMessage) ?DeviceIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint8);
    }

    /// Activity Type associated with an auto_activity_detect event
    pub fn getActivityType(msg: EventMessage) ?ActivityType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 14) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Do not populate directly. Autogenerated by decoder for threat_alert subfield components.
    pub fn getRadarThreatLevelMax(msg: EventMessage) ?RadarThreatLevelType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 21) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Do not populate directly. Autogenerated by decoder for threat_alert subfield components.
    pub fn getRadarThreatCount(msg: EventMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 22) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Do not populate directly. Autogenerated by decoder for radar_threat_alert subfield components
    pub fn getRadarThreatAvgApproachSpeed(msg: EventMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 23) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 10) - 0;
    }

    /// Do not populate directly. Autogenerated by decoder for radar_threat_alert subfield components
    pub fn getRadarThreatMaxApproachSpeed(msg: EventMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 24) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 10) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) EventMessage {
        return EventMessage{ .raw_fields = raw_fields };
    }
};

pub const DeviceInfoMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: DeviceInfoMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getDeviceIndex(msg: DeviceInfoMessage) ?DeviceIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint8);
    }

    pub fn getManufacturer(msg: DeviceInfoMessage) ?Manufacturer {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getSerialNumber(msg: DeviceInfoMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32z);
        return rf.?.*.raw_value.scalar.uint32z;
    }

    pub fn getSoftwareVersion(msg: DeviceInfoMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getHardwareVersion(msg: DeviceInfoMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Reset by new battery or charge.
    pub fn getCumOperatingTime(msg: DeviceInfoMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getBatteryVoltage(msg: DeviceInfoMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 256) - 0;
    }

    pub fn getBatteryStatus(msg: DeviceInfoMessage) ?BatteryStatus {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint8);
    }

    /// Indicates the location of the sensor
    pub fn getSensorPosition(msg: DeviceInfoMessage) ?BodyLocation {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 18) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Used to describe the sensor or location
    pub fn getDescriptor(msg: DeviceInfoMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 19) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getAntTransmissionType(msg: DeviceInfoMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 20) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return rf.?.*.raw_value.scalar.uint8z;
    }

    pub fn getAntDeviceNumber(msg: DeviceInfoMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 21) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16z);
        return rf.?.*.raw_value.scalar.uint16z;
    }

    pub fn getAntNetwork(msg: DeviceInfoMessage) ?AntNetwork {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 22) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getSourceType(msg: DeviceInfoMessage) ?SourceType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 25) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Optional free form string to indicate the devices name or model
    pub fn getProductName(msg: DeviceInfoMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 27) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getBatteryLevel(msg: DeviceInfoMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 32) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) DeviceInfoMessage {
        return DeviceInfoMessage{ .raw_fields = raw_fields };
    }
};

pub const DeviceAuxBatteryInfoMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: DeviceAuxBatteryInfoMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getDeviceIndex(msg: DeviceAuxBatteryInfoMessage) ?DeviceIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint8);
    }

    pub fn getBatteryVoltage(msg: DeviceAuxBatteryInfoMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 256) - 0;
    }

    pub fn getBatteryStatus(msg: DeviceAuxBatteryInfoMessage) ?BatteryStatus {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint8);
    }

    pub fn getBatteryIdentifier(msg: DeviceAuxBatteryInfoMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) DeviceAuxBatteryInfoMessage {
        return DeviceAuxBatteryInfoMessage{ .raw_fields = raw_fields };
    }
};

/// Corresponds to file_id of workout or course.
pub const TrainingFileMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: TrainingFileMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getType(msg: TrainingFileMessage) ?File {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getManufacturer(msg: TrainingFileMessage) ?Manufacturer {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getSerialNumber(msg: TrainingFileMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32z);
        return rf.?.*.raw_value.scalar.uint32z;
    }

    pub fn getTimeCreated(msg: TrainingFileMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) TrainingFileMessage {
        return TrainingFileMessage{ .raw_fields = raw_fields };
    }
};

pub const WeatherConditionsMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// time of update for current conditions, else forecast time
    pub fn getTimestamp(msg: WeatherConditionsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Current or forecast
    pub fn getWeatherReport(msg: WeatherConditionsMessage) ?WeatherReport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getTemperature(msg: WeatherConditionsMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    /// Corresponds to GSC Response weatherIcon field
    pub fn getCondition(msg: WeatherConditionsMessage) ?WeatherStatus {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getWindDirection(msg: WeatherConditionsMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getWindSpeed(msg: WeatherConditionsMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    /// range 0-100
    pub fn getPrecipitationProbability(msg: WeatherConditionsMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Heat Index if GCS heatIdx above or equal to 90F or wind chill if GCS windChill below or equal to 32F
    pub fn getTemperatureFeelsLike(msg: WeatherConditionsMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    pub fn getRelativeHumidity(msg: WeatherConditionsMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// string corresponding to GCS response location string
    pub fn getLocation(msg: WeatherConditionsMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getObservedAtTime(msg: WeatherConditionsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getObservedLocationLat(msg: WeatherConditionsMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getObservedLocationLong(msg: WeatherConditionsMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getDayOfWeek(msg: WeatherConditionsMessage) ?DayOfWeek {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getHighTemperature(msg: WeatherConditionsMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    pub fn getLowTemperature(msg: WeatherConditionsMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 14) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) WeatherConditionsMessage {
        return WeatherConditionsMessage{ .raw_fields = raw_fields };
    }
};

pub const WeatherAlertMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: WeatherAlertMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Unique identifier from GCS report ID string, length is 12
    pub fn getReportId(msg: WeatherAlertMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    /// Time alert was issued
    pub fn getIssueTime(msg: WeatherAlertMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Time alert expires
    pub fn getExpireTime(msg: WeatherAlertMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Warning, Watch, Advisory, Statement
    pub fn getSeverity(msg: WeatherAlertMessage) ?WeatherSeverity {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Tornado, Severe Thunderstorm, etc.
    pub fn getType(msg: WeatherAlertMessage) ?WeatherSevereType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) WeatherAlertMessage {
        return WeatherAlertMessage{ .raw_fields = raw_fields };
    }
};

pub const GpsMetadataMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Whole second part of the timestamp.
    pub fn getTimestamp(msg: GpsMetadataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Millisecond part of the timestamp.
    pub fn getTimestampMs(msg: GpsMetadataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getPositionLat(msg: GpsMetadataMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getPositionLong(msg: GpsMetadataMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getEnhancedAltitude(msg: GpsMetadataMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 5) - 500;
    }

    pub fn getEnhancedSpeed(msg: GpsMetadataMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getHeading(msg: GpsMetadataMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    /// Used to correlate UTC to system time if the timestamp of the message is in system time. This UTC time is derived from the GPS data.
    pub fn getUtcTimestamp(msg: GpsMetadataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// velocity[0] is lon velocity. Velocity[1] is lat velocity. Velocity[2] is altitude velocity.
    pub fn getVelocity(msg: GpsMetadataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint16);
            ret[i] = (f / 100) - 0;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) GpsMetadataMessage {
        return GpsMetadataMessage{ .raw_fields = raw_fields };
    }
};

pub const CameraEventMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Whole second part of the timestamp.
    pub fn getTimestamp(msg: CameraEventMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Millisecond part of the timestamp.
    pub fn getTimestampMs(msg: CameraEventMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getCameraEventType(msg: CameraEventMessage) ?CameraEventType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getCameraFileUuid(msg: CameraEventMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getCameraOrientation(msg: CameraEventMessage) ?CameraOrientationType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) CameraEventMessage {
        return CameraEventMessage{ .raw_fields = raw_fields };
    }
};

pub const GyroscopeDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Whole second part of the timestamp
    pub fn getTimestamp(msg: GyroscopeDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Millisecond part of the timestamp.
    pub fn getTimestampMs(msg: GyroscopeDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Each time in the array describes the time at which the gyro sample with the corrosponding index was taken. Limited to 30 samples in each message. The samples may span across seconds. Array size must match the number of samples in gyro_x and gyro_y and gyro_z
    pub fn getSampleTimeOffset(msg: GyroscopeDataMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// These are the raw ADC reading. Maximum number of samples is 30 in each message. The samples may span across seconds. A conversion will need to be done on this data once read.
    pub fn getGyroX(msg: GyroscopeDataMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// These are the raw ADC reading. Maximum number of samples is 30 in each message. The samples may span across seconds. A conversion will need to be done on this data once read.
    pub fn getGyroY(msg: GyroscopeDataMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// These are the raw ADC reading. Maximum number of samples is 30 in each message. The samples may span across seconds. A conversion will need to be done on this data once read.
    pub fn getGyroZ(msg: GyroscopeDataMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// Calibrated gyro reading
    pub fn getCalibratedGyroX(msg: GyroscopeDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.float32;
        }
        return ret;
    }

    /// Calibrated gyro reading
    pub fn getCalibratedGyroY(msg: GyroscopeDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.float32;
        }
        return ret;
    }

    /// Calibrated gyro reading
    pub fn getCalibratedGyroZ(msg: GyroscopeDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.float32;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) GyroscopeDataMessage {
        return GyroscopeDataMessage{ .raw_fields = raw_fields };
    }
};

pub const AccelerometerDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Whole second part of the timestamp
    pub fn getTimestamp(msg: AccelerometerDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Millisecond part of the timestamp.
    pub fn getTimestampMs(msg: AccelerometerDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Each time in the array describes the time at which the accelerometer sample with the corrosponding index was taken. Limited to 30 samples in each message. The samples may span across seconds. Array size must match the number of samples in accel_x and accel_y and accel_z
    pub fn getSampleTimeOffset(msg: AccelerometerDataMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// These are the raw ADC reading. Maximum number of samples is 30 in each message. The samples may span across seconds. A conversion will need to be done on this data once read.
    pub fn getAccelX(msg: AccelerometerDataMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// These are the raw ADC reading. Maximum number of samples is 30 in each message. The samples may span across seconds. A conversion will need to be done on this data once read.
    pub fn getAccelY(msg: AccelerometerDataMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// These are the raw ADC reading. Maximum number of samples is 30 in each message. The samples may span across seconds. A conversion will need to be done on this data once read.
    pub fn getAccelZ(msg: AccelerometerDataMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// Calibrated accel reading
    pub fn getCalibratedAccelX(msg: AccelerometerDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.float32;
        }
        return ret;
    }

    /// Calibrated accel reading
    pub fn getCalibratedAccelY(msg: AccelerometerDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.float32;
        }
        return ret;
    }

    /// Calibrated accel reading
    pub fn getCalibratedAccelZ(msg: AccelerometerDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.float32;
        }
        return ret;
    }

    /// Calibrated accel reading
    pub fn getCompressedCalibratedAccelX(msg: AccelerometerDataMessage, allocator: std.mem.Allocator) ?[]i16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(i16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.sint16;
        }
        return ret;
    }

    /// Calibrated accel reading
    pub fn getCompressedCalibratedAccelY(msg: AccelerometerDataMessage, allocator: std.mem.Allocator) ?[]i16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(i16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.sint16;
        }
        return ret;
    }

    /// Calibrated accel reading
    pub fn getCompressedCalibratedAccelZ(msg: AccelerometerDataMessage, allocator: std.mem.Allocator) ?[]i16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(i16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.sint16;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) AccelerometerDataMessage {
        return AccelerometerDataMessage{ .raw_fields = raw_fields };
    }
};

pub const MagnetometerDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Whole second part of the timestamp
    pub fn getTimestamp(msg: MagnetometerDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Millisecond part of the timestamp.
    pub fn getTimestampMs(msg: MagnetometerDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Each time in the array describes the time at which the compass sample with the corrosponding index was taken. Limited to 30 samples in each message. The samples may span across seconds. Array size must match the number of samples in cmps_x and cmps_y and cmps_z
    pub fn getSampleTimeOffset(msg: MagnetometerDataMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// These are the raw ADC reading. Maximum number of samples is 30 in each message. The samples may span across seconds. A conversion will need to be done on this data once read.
    pub fn getMagX(msg: MagnetometerDataMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// These are the raw ADC reading. Maximum number of samples is 30 in each message. The samples may span across seconds. A conversion will need to be done on this data once read.
    pub fn getMagY(msg: MagnetometerDataMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// These are the raw ADC reading. Maximum number of samples is 30 in each message. The samples may span across seconds. A conversion will need to be done on this data once read.
    pub fn getMagZ(msg: MagnetometerDataMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// Calibrated Magnetometer reading
    pub fn getCalibratedMagX(msg: MagnetometerDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.float32;
        }
        return ret;
    }

    /// Calibrated Magnetometer reading
    pub fn getCalibratedMagY(msg: MagnetometerDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.float32;
        }
        return ret;
    }

    /// Calibrated Magnetometer reading
    pub fn getCalibratedMagZ(msg: MagnetometerDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.float32;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) MagnetometerDataMessage {
        return MagnetometerDataMessage{ .raw_fields = raw_fields };
    }
};

pub const BarometerDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Whole second part of the timestamp
    pub fn getTimestamp(msg: BarometerDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Millisecond part of the timestamp.
    pub fn getTimestampMs(msg: BarometerDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Each time in the array describes the time at which the barometer sample with the corrosponding index was taken. The samples may span across seconds. Array size must match the number of samples in baro_cal
    pub fn getSampleTimeOffset(msg: BarometerDataMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// These are the raw ADC reading. The samples may span across seconds. A conversion will need to be done on this data once read.
    pub fn getBaroPres(msg: BarometerDataMessage, allocator: std.mem.Allocator) ?[]u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(u32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint32;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) BarometerDataMessage {
        return BarometerDataMessage{ .raw_fields = raw_fields };
    }
};

pub const ThreeDSensorCalibrationMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Whole second part of the timestamp
    pub fn getTimestamp(msg: ThreeDSensorCalibrationMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Indicates which sensor the calibration is for
    pub fn getSensorType(msg: ThreeDSensorCalibrationMessage) ?SensorType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Calibration factor divisor
    pub fn getCalibrationDivisor(msg: ThreeDSensorCalibrationMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Level shift value used to shift the ADC value back into range
    pub fn getLevelShift(msg: ThreeDSensorCalibrationMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Internal calibration factors, one for each: xy, yx, zx
    pub fn getOffsetCal(msg: ThreeDSensorCalibrationMessage, allocator: std.mem.Allocator) ?[]i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        var ret = try allocator.alloc(i32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.sint32;
        }
        return ret;
    }

    /// 3 x 3 rotation matrix (row major)
    pub fn getOrientationMatrix(msg: ThreeDSensorCalibrationMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint32);
            ret[i] = (f / 65535) - 0;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) ThreeDSensorCalibrationMessage {
        return ThreeDSensorCalibrationMessage{ .raw_fields = raw_fields };
    }
};

pub const OneDSensorCalibrationMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Whole second part of the timestamp
    pub fn getTimestamp(msg: OneDSensorCalibrationMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Indicates which sensor the calibration is for
    pub fn getSensorType(msg: OneDSensorCalibrationMessage) ?SensorType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Calibration factor divisor
    pub fn getCalibrationDivisor(msg: OneDSensorCalibrationMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Level shift value used to shift the ADC value back into range
    pub fn getLevelShift(msg: OneDSensorCalibrationMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Internal Calibration factor
    pub fn getOffsetCal(msg: OneDSensorCalibrationMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) OneDSensorCalibrationMessage {
        return OneDSensorCalibrationMessage{ .raw_fields = raw_fields };
    }
};

pub const VideoFrameMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Whole second part of the timestamp
    pub fn getTimestamp(msg: VideoFrameMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Millisecond part of the timestamp.
    pub fn getTimestampMs(msg: VideoFrameMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Number of the frame that the timestamp and timestamp_ms correlate to
    pub fn getFrameNumber(msg: VideoFrameMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) VideoFrameMessage {
        return VideoFrameMessage{ .raw_fields = raw_fields };
    }
};

pub const ObdiiDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Timestamp message was output
    pub fn getTimestamp(msg: ObdiiDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Fractional part of timestamp, added to timestamp
    pub fn getTimestampMs(msg: ObdiiDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Offset of PID reading [i] from start_timestamp+start_timestamp_ms. Readings may span accross seconds.
    pub fn getTimeOffset(msg: ObdiiDataMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// Parameter ID
    pub fn getPid(msg: ObdiiDataMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        return rf.?.*.raw_value.scalar.byte;
    }

    /// Raw parameter data
    pub fn getRawData(msg: ObdiiDataMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.byte;
        }
        return ret;
    }

    /// Optional, data size of PID[i]. If not specified refer to SAE J1979.
    pub fn getPidDataSize(msg: ObdiiDataMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8;
        }
        return ret;
    }

    /// System time associated with sample expressed in ms, can be used instead of time_offset. There will be a system_time value for each raw_data element. For multibyte pids the system_time is repeated.
    pub fn getSystemTime(msg: ObdiiDataMessage, allocator: std.mem.Allocator) ?[]u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(u32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint32;
        }
        return ret;
    }

    /// Timestamp of first sample recorded in the message. Used with time_offset to generate time of each sample
    pub fn getStartTimestamp(msg: ObdiiDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Fractional part of start_timestamp
    pub fn getStartTimestampMs(msg: ObdiiDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) ObdiiDataMessage {
        return ObdiiDataMessage{ .raw_fields = raw_fields };
    }
};

pub const NmeaSentenceMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Timestamp message was output
    pub fn getTimestamp(msg: NmeaSentenceMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Fractional part of timestamp, added to timestamp
    pub fn getTimestampMs(msg: NmeaSentenceMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// NMEA sentence
    pub fn getSentence(msg: NmeaSentenceMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) NmeaSentenceMessage {
        return NmeaSentenceMessage{ .raw_fields = raw_fields };
    }
};

pub const AviationAttitudeMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Timestamp message was output
    pub fn getTimestamp(msg: AviationAttitudeMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Fractional part of timestamp, added to timestamp
    pub fn getTimestampMs(msg: AviationAttitudeMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// System time associated with sample expressed in ms.
    pub fn getSystemTime(msg: AviationAttitudeMessage, allocator: std.mem.Allocator) ?[]u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(u32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint32;
        }
        return ret;
    }

    /// Range -PI/2 to +PI/2
    pub fn getPitch(msg: AviationAttitudeMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint16);
            ret[i] = (f / 10430.38) - 0;
        }
        return ret;
    }

    /// Range -PI to +PI
    pub fn getRoll(msg: AviationAttitudeMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint16);
            ret[i] = (f / 10430.38) - 0;
        }
        return ret;
    }

    /// Range -78.4 to +78.4 (-8 Gs to 8 Gs)
    pub fn getAccelLateral(msg: AviationAttitudeMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint16);
            ret[i] = (f / 100) - 0;
        }
        return ret;
    }

    /// Range -78.4 to +78.4 (-8 Gs to 8 Gs)
    pub fn getAccelNormal(msg: AviationAttitudeMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint16);
            ret[i] = (f / 100) - 0;
        }
        return ret;
    }

    /// Range -8.727 to +8.727 (-500 degs/sec to +500 degs/sec)
    pub fn getTurnRate(msg: AviationAttitudeMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint16);
            ret[i] = (f / 1024) - 0;
        }
        return ret;
    }

    pub fn getStage(msg: AviationAttitudeMessage, allocator: std.mem.Allocator) ?[]AttitudeStage {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        var ret = try allocator.alloc(AttitudeStage, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.enumeration;
        }
        return ret;
    }

    /// The percent complete of the current attitude stage. Set to 0 for attitude stages 0, 1 and 2 and to 100 for attitude stage 3 by AHRS modules that do not support it. Range - 100
    pub fn getAttitudeStageComplete(msg: AviationAttitudeMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8;
        }
        return ret;
    }

    /// Track Angle/Heading Range 0 - 2pi
    pub fn getTrack(msg: AviationAttitudeMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 10430.38) - 0;
        }
        return ret;
    }

    pub fn getValidity(msg: AviationAttitudeMessage, allocator: std.mem.Allocator) ?[]AttitudeValidity {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(AttitudeValidity, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) AviationAttitudeMessage {
        return AviationAttitudeMessage{ .raw_fields = raw_fields };
    }
};

pub const VideoMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getUrl(msg: VideoMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getHostingProvider(msg: VideoMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    /// Playback time of video
    pub fn getDuration(msg: VideoMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) VideoMessage {
        return VideoMessage{ .raw_fields = raw_fields };
    }
};

pub const VideoTitleMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Long titles will be split into multiple parts
    pub fn getMessageIndex(msg: VideoTitleMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    /// Total number of title parts
    pub fn getMessageCount(msg: VideoTitleMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getText(msg: VideoTitleMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) VideoTitleMessage {
        return VideoTitleMessage{ .raw_fields = raw_fields };
    }
};

pub const VideoDescriptionMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Long descriptions will be split into multiple parts
    pub fn getMessageIndex(msg: VideoDescriptionMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    /// Total number of description parts
    pub fn getMessageCount(msg: VideoDescriptionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getText(msg: VideoDescriptionMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) VideoDescriptionMessage {
        return VideoDescriptionMessage{ .raw_fields = raw_fields };
    }
};

pub const VideoClipMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getClipNumber(msg: VideoClipMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getStartTimestamp(msg: VideoClipMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getStartTimestampMs(msg: VideoClipMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getEndTimestamp(msg: VideoClipMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getEndTimestampMs(msg: VideoClipMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Start of clip in video time
    pub fn getClipStart(msg: VideoClipMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// End of clip in video time
    pub fn getClipEnd(msg: VideoClipMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) VideoClipMessage {
        return VideoClipMessage{ .raw_fields = raw_fields };
    }
};

pub const SetMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Timestamp of the set
    pub fn getTimestamp(msg: SetMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getDuration(msg: SetMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// # of repitions of the movement
    pub fn getRepetitions(msg: SetMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Amount of weight applied for the set
    pub fn getWeight(msg: SetMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 16) - 0;
    }

    pub fn getSetType(msg: SetMessage) ?SetType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint8);
    }

    /// Start time of the set
    pub fn getStartTime(msg: SetMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getCategory(msg: SetMessage, allocator: std.mem.Allocator) ?[]ExerciseCategory {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(ExerciseCategory, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// Based on the associated category, see [category]_exercise_names
    pub fn getCategorySubtype(msg: SetMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    pub fn getWeightDisplayUnit(msg: SetMessage) ?FitBaseUnit {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getMessageIndex(msg: SetMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getWktStepIndex(msg: SetMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SetMessage {
        return SetMessage{ .raw_fields = raw_fields };
    }
};

pub const JumpMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: JumpMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getDistance(msg: JumpMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    pub fn getHeight(msg: JumpMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    pub fn getRotations(msg: JumpMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getHangTime(msg: JumpMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// A score for a jump calculated based on hang time, rotations, and distance.
    pub fn getScore(msg: JumpMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    pub fn getPositionLat(msg: JumpMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getPositionLong(msg: JumpMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getSpeed(msg: JumpMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    pub fn getEnhancedSpeed(msg: JumpMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) JumpMessage {
        return JumpMessage{ .raw_fields = raw_fields };
    }
};

pub const SplitMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: SplitMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getSplitType(msg: SplitMessage) ?SplitType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getTotalElapsedTime(msg: SplitMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getTotalTimerTime(msg: SplitMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getTotalDistance(msg: SplitMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 100) - 0;
    }

    pub fn getAvgSpeed(msg: SplitMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getStartTime(msg: SplitMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getTotalAscent(msg: SplitMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getTotalDescent(msg: SplitMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 14) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getStartPositionLat(msg: SplitMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 21) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getStartPositionLong(msg: SplitMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 22) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getEndPositionLat(msg: SplitMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 23) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getEndPositionLong(msg: SplitMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 24) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getMaxSpeed(msg: SplitMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 25) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getAvgVertSpeed(msg: SplitMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 26) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint32);
        return (f / 1000) - 0;
    }

    pub fn getEndTime(msg: SplitMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 27) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getTotalCalories(msg: SplitMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 28) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getStartElevation(msg: SplitMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 74) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 5) - 500;
    }

    /// Active time of split rounds
    pub fn getActiveTime(msg: SplitMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 78) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getTotalMovingTime(msg: SplitMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 110) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SplitMessage {
        return SplitMessage{ .raw_fields = raw_fields };
    }
};

pub const SplitSummaryMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: SplitSummaryMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getSplitType(msg: SplitSummaryMessage) ?SplitType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getNumSplits(msg: SplitSummaryMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getTotalTimerTime(msg: SplitSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getTotalDistance(msg: SplitSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 100) - 0;
    }

    pub fn getAvgSpeed(msg: SplitSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getMaxSpeed(msg: SplitSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getTotalAscent(msg: SplitSummaryMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getTotalDescent(msg: SplitSummaryMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getAvgHeartRate(msg: SplitSummaryMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getMaxHeartRate(msg: SplitSummaryMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getAvgVertSpeed(msg: SplitSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint32);
        return (f / 1000) - 0;
    }

    pub fn getTotalCalories(msg: SplitSummaryMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// total active time in all split rounds
    pub fn getActiveTime(msg: SplitSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 65) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getTotalMovingTime(msg: SplitSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 77) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SplitSummaryMessage {
        return SplitSummaryMessage{ .raw_fields = raw_fields };
    }
};

pub const ClimbProMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: ClimbProMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getPositionLat(msg: ClimbProMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getPositionLong(msg: ClimbProMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getClimbProEvent(msg: ClimbProMessage) ?ClimbProEvent {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getClimbNumber(msg: ClimbProMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getClimbCategory(msg: ClimbProMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getCurrentDist(msg: ClimbProMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) ClimbProMessage {
        return ClimbProMessage{ .raw_fields = raw_fields };
    }
};

/// Must be logged before developer field is used
pub const FieldDescriptionMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getDeveloperDataIndex(msg: FieldDescriptionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getFieldDefinitionNumber(msg: FieldDescriptionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getFitBaseTypeId(msg: FieldDescriptionMessage) ?FitBaseType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint8);
    }

    pub fn getFieldName(msg: FieldDescriptionMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        var ret = try allocator.alloc([:0]u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.string;
        }
        return ret;
    }

    pub fn getArray(msg: FieldDescriptionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getComponents(msg: FieldDescriptionMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getScale(msg: FieldDescriptionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getOffset(msg: FieldDescriptionMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    pub fn getUnits(msg: FieldDescriptionMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        var ret = try allocator.alloc([:0]u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.string;
        }
        return ret;
    }

    pub fn getBits(msg: FieldDescriptionMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getAccumulate(msg: FieldDescriptionMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getFitBaseUnitId(msg: FieldDescriptionMessage) ?FitBaseUnit {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getNativeMesgNum(msg: FieldDescriptionMessage) ?MesgNum {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 14) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getNativeFieldNum(msg: FieldDescriptionMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 15) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) FieldDescriptionMessage {
        return FieldDescriptionMessage{ .raw_fields = raw_fields };
    }
};

/// Must be logged before field description
pub const DeveloperDataIdMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getDeveloperId(msg: DeveloperDataIdMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.byte;
        }
        return ret;
    }

    pub fn getApplicationId(msg: DeveloperDataIdMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.byte;
        }
        return ret;
    }

    pub fn getManufacturerId(msg: DeveloperDataIdMessage) ?Manufacturer {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getDeveloperDataIndex(msg: DeveloperDataIdMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getApplicationVersion(msg: DeveloperDataIdMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) DeveloperDataIdMessage {
        return DeveloperDataIdMessage{ .raw_fields = raw_fields };
    }
};

pub const CourseMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getSport(msg: CourseMessage) ?Sport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getName(msg: CourseMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getCapabilities(msg: CourseMessage) ?CourseCapabilities {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32z);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint32z);
    }

    pub fn getSubSport(msg: CourseMessage) ?SubSport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) CourseMessage {
        return CourseMessage{ .raw_fields = raw_fields };
    }
};

pub const CoursePointMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: CoursePointMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getTimestamp(msg: CoursePointMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getPositionLat(msg: CoursePointMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getPositionLong(msg: CoursePointMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getDistance(msg: CoursePointMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 100) - 0;
    }

    pub fn getType(msg: CoursePointMessage) ?CoursePoint {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getName(msg: CoursePointMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getFavorite(msg: CoursePointMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) CoursePointMessage {
        return CoursePointMessage{ .raw_fields = raw_fields };
    }
};

/// Unique Identification data for a segment file
pub const SegmentIdMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Friendly name assigned to segment
    pub fn getName(msg: SegmentIdMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    /// UUID of the segment
    pub fn getUuid(msg: SegmentIdMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    /// Sport associated with the segment
    pub fn getSport(msg: SegmentIdMessage) ?Sport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Segment enabled for evaluation
    pub fn getEnabled(msg: SegmentIdMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Primary key of the user that created the segment
    pub fn getUserProfilePrimaryKey(msg: SegmentIdMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// ID of the device that created the segment
    pub fn getDeviceId(msg: SegmentIdMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Index for the Leader Board entry selected as the default race participant
    pub fn getDefaultRaceLeader(msg: SegmentIdMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Indicates if any segments should be deleted
    pub fn getDeleteStatus(msg: SegmentIdMessage) ?SegmentDeleteStatus {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Indicates how the segment was selected to be sent to the device
    pub fn getSelectionType(msg: SegmentIdMessage) ?SegmentSelectionType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SegmentIdMessage {
        return SegmentIdMessage{ .raw_fields = raw_fields };
    }
};

/// Unique Identification data for an individual segment leader within a segment file
pub const SegmentLeaderboardEntryMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: SegmentLeaderboardEntryMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    /// Friendly name assigned to leader
    pub fn getName(msg: SegmentLeaderboardEntryMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    /// Leader classification
    pub fn getType(msg: SegmentLeaderboardEntryMessage) ?SegmentLeaderboardType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Primary user ID of this leader
    pub fn getGroupPrimaryKey(msg: SegmentLeaderboardEntryMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// ID of the activity associated with this leader time
    pub fn getActivityId(msg: SegmentLeaderboardEntryMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Segment Time (includes pauses)
    pub fn getSegmentTime(msg: SegmentLeaderboardEntryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// String version of the activity_id. 21 characters long, express in decimal
    pub fn getActivityIdString(msg: SegmentLeaderboardEntryMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SegmentLeaderboardEntryMessage {
        return SegmentLeaderboardEntryMessage{ .raw_fields = raw_fields };
    }
};

/// Navigation and race evaluation point for a segment decribing a point along the segment path and time it took each segment leader to reach that point
pub const SegmentPointMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: SegmentPointMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getPositionLat(msg: SegmentPointMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getPositionLong(msg: SegmentPointMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    /// Accumulated distance along the segment at the described point
    pub fn getDistance(msg: SegmentPointMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 100) - 0;
    }

    /// Accumulated altitude along the segment at the described point
    pub fn getAltitude(msg: SegmentPointMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 5) - 500;
    }

    /// Accumualted time each leader board member required to reach the described point. This value is zero for all leader board members at the starting point of the segment.
    pub fn getLeaderTime(msg: SegmentPointMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    /// Accumulated altitude along the segment at the described point
    pub fn getEnhancedAltitude(msg: SegmentPointMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 5) - 500;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SegmentPointMessage {
        return SegmentPointMessage{ .raw_fields = raw_fields };
    }
};

pub const SegmentLapMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: SegmentLapMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getTimestamp(msg: SegmentLapMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getEvent(msg: SegmentLapMessage) ?Event {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getEventType(msg: SegmentLapMessage) ?EventType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getStartTime(msg: SegmentLapMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getStartPositionLat(msg: SegmentLapMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getStartPositionLong(msg: SegmentLapMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getEndPositionLat(msg: SegmentLapMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getEndPositionLong(msg: SegmentLapMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    /// Time (includes pauses)
    pub fn getTotalElapsedTime(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Timer Time (excludes pauses)
    pub fn getTotalTimerTime(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getTotalDistance(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 100) - 0;
    }

    pub fn getTotalCalories(msg: SegmentLapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// If New Leaf
    pub fn getTotalFatCalories(msg: SegmentLapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getAvgSpeed(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    pub fn getMaxSpeed(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 14) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1000) - 0;
    }

    pub fn getAvgHeartRate(msg: SegmentLapMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 15) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getMaxHeartRate(msg: SegmentLapMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 16) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// total_cycles / total_timer_time if non_zero_avg_cadence otherwise total_cycles / total_elapsed_time
    pub fn getAvgCadence(msg: SegmentLapMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 17) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getMaxCadence(msg: SegmentLapMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 18) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// total_power / total_timer_time if non_zero_avg_power otherwise total_power / total_elapsed_time
    pub fn getAvgPower(msg: SegmentLapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 19) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getMaxPower(msg: SegmentLapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 20) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getTotalAscent(msg: SegmentLapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 21) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getTotalDescent(msg: SegmentLapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 22) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getSport(msg: SegmentLapMessage) ?Sport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 23) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getEventGroup(msg: SegmentLapMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 24) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// North east corner latitude.
    pub fn getNecLat(msg: SegmentLapMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 25) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    /// North east corner longitude.
    pub fn getNecLong(msg: SegmentLapMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 26) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    /// South west corner latitude.
    pub fn getSwcLat(msg: SegmentLapMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 27) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    /// South west corner latitude.
    pub fn getSwcLong(msg: SegmentLapMessage) ?i32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 28) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        return rf.?.*.raw_value.scalar.sint32;
    }

    pub fn getName(msg: SegmentLapMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 29) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getNormalizedPower(msg: SegmentLapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 30) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getLeftRightBalance(msg: SegmentLapMessage) ?LeftRightBalance100 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 31) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getSubSport(msg: SegmentLapMessage) ?SubSport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 32) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getTotalWork(msg: SegmentLapMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 33) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getAvgAltitude(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 34) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 5) - 500;
    }

    pub fn getMaxAltitude(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 35) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 5) - 500;
    }

    pub fn getGpsAccuracy(msg: SegmentLapMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 36) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getAvgGrade(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 37) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn getAvgPosGrade(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 38) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn getAvgNegGrade(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 39) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn getMaxPosGrade(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 40) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn getMaxNegGrade(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 41) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn getAvgTemperature(msg: SegmentLapMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 42) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    pub fn getMaxTemperature(msg: SegmentLapMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 43) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    pub fn getTotalMovingTime(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 44) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getAvgPosVerticalSpeed(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 45) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 1000) - 0;
    }

    pub fn getAvgNegVerticalSpeed(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 46) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 1000) - 0;
    }

    pub fn getMaxPosVerticalSpeed(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 47) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 1000) - 0;
    }

    pub fn getMaxNegVerticalSpeed(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 48) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 1000) - 0;
    }

    pub fn getTimeInHrZone(msg: SegmentLapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 49) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getTimeInSpeedZone(msg: SegmentLapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 50) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getTimeInCadenceZone(msg: SegmentLapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 51) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getTimeInPowerZone(msg: SegmentLapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 52) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn getRepetitionNum(msg: SegmentLapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 53) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getMinAltitude(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 54) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 5) - 500;
    }

    pub fn getMinHeartRate(msg: SegmentLapMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 55) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getActiveTime(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 56) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getWktStepIndex(msg: SegmentLapMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 57) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getSportEvent(msg: SegmentLapMessage) ?SportEvent {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 58) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getAvgLeftTorqueEffectiveness(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 59) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getAvgRightTorqueEffectiveness(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 60) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getAvgLeftPedalSmoothness(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 61) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getAvgRightPedalSmoothness(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 62) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getAvgCombinedPedalSmoothness(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 63) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 2) - 0;
    }

    pub fn getStatus(msg: SegmentLapMessage) ?SegmentLapStatus {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 64) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getUuid(msg: SegmentLapMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 65) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    /// fractional part of the avg_cadence
    pub fn getAvgFractionalCadence(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 66) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 128) - 0;
    }

    /// fractional part of the max_cadence
    pub fn getMaxFractionalCadence(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 67) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 128) - 0;
    }

    /// fractional part of the total_cycles
    pub fn getTotalFractionalCycles(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 68) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 128) - 0;
    }

    pub fn getFrontGearShiftCount(msg: SegmentLapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 69) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getRearGearShiftCount(msg: SegmentLapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 70) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Total time spent in the standing position
    pub fn getTimeStanding(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 71) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Number of transitions to the standing state
    pub fn getStandCount(msg: SegmentLapMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 72) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Average left platform center offset
    pub fn getAvgLeftPco(msg: SegmentLapMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 73) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    /// Average right platform center offset
    pub fn getAvgRightPco(msg: SegmentLapMessage) ?i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 74) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        return rf.?.*.raw_value.scalar.sint8;
    }

    /// Average left power phase angles. Data value indexes defined by power_phase_type.
    pub fn getAvgLeftPowerPhase(msg: SegmentLapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 75) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    /// Average left power phase peak angles. Data value indexes defined by power_phase_type.
    pub fn getAvgLeftPowerPhasePeak(msg: SegmentLapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 76) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    /// Average right power phase angles. Data value indexes defined by power_phase_type.
    pub fn getAvgRightPowerPhase(msg: SegmentLapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 77) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    /// Average right power phase peak angles. Data value indexes defined by power_phase_type.
    pub fn getAvgRightPowerPhasePeak(msg: SegmentLapMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 78) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 0.7111111) - 0;
        }
        return ret;
    }

    /// Average power by position. Data value indexes defined by rider_position_type.
    pub fn getAvgPowerPosition(msg: SegmentLapMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 79) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// Maximum power by position. Data value indexes defined by rider_position_type.
    pub fn getMaxPowerPosition(msg: SegmentLapMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 80) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// Average cadence by position. Data value indexes defined by rider_position_type.
    pub fn getAvgCadencePosition(msg: SegmentLapMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 81) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8;
        }
        return ret;
    }

    /// Maximum cadence by position. Data value indexes defined by rider_position_type.
    pub fn getMaxCadencePosition(msg: SegmentLapMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 82) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8;
        }
        return ret;
    }

    /// Manufacturer that produced the segment
    pub fn getManufacturer(msg: SegmentLapMessage) ?Manufacturer {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 83) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    /// The grit score estimates how challenging a route could be for a cyclist in terms of time spent going over sharp turns or large grade slopes.
    pub fn getTotalGrit(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 84) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// The flow score estimates how long distance wise a cyclist deaccelerates over intervals where deacceleration is unnecessary such as smooth turns or small grade angle intervals.
    pub fn getTotalFlow(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 85) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// The grit score estimates how challenging a route could be for a cyclist in terms of time spent going over sharp turns or large grade slopes.
    pub fn getAvgGrit(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 86) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// The flow score estimates how long distance wise a cyclist deaccelerates over intervals where deacceleration is unnecessary such as smooth turns or small grade angle intervals.
    pub fn getAvgFlow(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 87) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// fractional part of total_ascent
    pub fn getTotalFractionalAscent(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 89) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 100) - 0;
    }

    /// fractional part of total_descent
    pub fn getTotalFractionalDescent(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 90) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 100) - 0;
    }

    pub fn getEnhancedAvgAltitude(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 91) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 5) - 500;
    }

    pub fn getEnhancedMaxAltitude(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 92) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 5) - 500;
    }

    pub fn getEnhancedMinAltitude(msg: SegmentLapMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 93) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 5) - 500;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SegmentLapMessage {
        return SegmentLapMessage{ .raw_fields = raw_fields };
    }
};

/// Summary of the unique segment and leaderboard information associated with a segment file. This message is used to compile a segment list file describing all segment files on a device. The segment list file is used when refreshing the contents of a segment file with the latest available leaderboard information.
pub const SegmentFileMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: SegmentFileMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    /// UUID of the segment file
    pub fn getFileUuid(msg: SegmentFileMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    /// Enabled state of the segment file
    pub fn getEnabled(msg: SegmentFileMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Primary key of the user that created the segment file
    pub fn getUserProfilePrimaryKey(msg: SegmentFileMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Leader type of each leader in the segment file
    pub fn getLeaderType(msg: SegmentFileMessage, allocator: std.mem.Allocator) ?[]SegmentLeaderboardType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        var ret = try allocator.alloc(SegmentLeaderboardType, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.enumeration;
        }
        return ret;
    }

    /// Group primary key of each leader in the segment file
    pub fn getLeaderGroupPrimaryKey(msg: SegmentFileMessage, allocator: std.mem.Allocator) ?[]u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(u32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint32;
        }
        return ret;
    }

    /// Activity ID of each leader in the segment file
    pub fn getLeaderActivityId(msg: SegmentFileMessage, allocator: std.mem.Allocator) ?[]u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(u32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint32;
        }
        return ret;
    }

    /// String version of the activity ID of each leader in the segment file. 21 characters long for each ID, express in decimal
    pub fn getLeaderActivityIdString(msg: SegmentFileMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        var ret = try allocator.alloc([:0]u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.string;
        }
        return ret;
    }

    /// Index for the Leader Board entry selected as the default race participant
    pub fn getDefaultRaceLeader(msg: SegmentFileMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SegmentFileMessage {
        return SegmentFileMessage{ .raw_fields = raw_fields };
    }
};

pub const WorkoutMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: WorkoutMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getSport(msg: WorkoutMessage) ?Sport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getCapabilities(msg: WorkoutMessage) ?WorkoutCapabilities {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32z);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint32z);
    }

    /// number of valid steps
    pub fn getNumValidSteps(msg: WorkoutMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getWktName(msg: WorkoutMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getSubSport(msg: WorkoutMessage) ?SubSport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getPoolLength(msg: WorkoutMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 14) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getPoolLengthUnit(msg: WorkoutMessage) ?DisplayMeasure {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 15) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Description of the workout
    pub fn getWktDescription(msg: WorkoutMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 17) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) WorkoutMessage {
        return WorkoutMessage{ .raw_fields = raw_fields };
    }
};

pub const WorkoutSessionMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: WorkoutSessionMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getSport(msg: WorkoutSessionMessage) ?Sport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getSubSport(msg: WorkoutSessionMessage) ?SubSport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getNumValidSteps(msg: WorkoutSessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getFirstStepIndex(msg: WorkoutSessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getPoolLength(msg: WorkoutSessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getPoolLengthUnit(msg: WorkoutSessionMessage) ?DisplayMeasure {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) WorkoutSessionMessage {
        return WorkoutSessionMessage{ .raw_fields = raw_fields };
    }
};

pub const WorkoutStepMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: WorkoutStepMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getWktStepName(msg: WorkoutStepMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getDurationType(msg: WorkoutStepMessage) ?WktStepDuration {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getTargetType(msg: WorkoutStepMessage) ?WktStepTarget {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getIntensity(msg: WorkoutStepMessage) ?Intensity {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getNotes(msg: WorkoutStepMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        return try allocator.dupeSentinel(u8, rf.?.*.raw_value.scalar.string, 0);
    }

    pub fn getEquipment(msg: WorkoutStepMessage) ?WorkoutEquipment {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getExerciseCategory(msg: WorkoutStepMessage) ?ExerciseCategory {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getExerciseName(msg: WorkoutStepMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getExerciseWeight(msg: WorkoutStepMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getWeightDisplayUnit(msg: WorkoutStepMessage) ?FitBaseUnit {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getSecondaryTargetType(msg: WorkoutStepMessage) ?WktStepTarget {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 19) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) WorkoutStepMessage {
        return WorkoutStepMessage{ .raw_fields = raw_fields };
    }
};

pub const ExerciseTitleMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: ExerciseTitleMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getExerciseCategory(msg: ExerciseTitleMessage) ?ExerciseCategory {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getExerciseName(msg: ExerciseTitleMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getWktStepName(msg: ExerciseTitleMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        var ret = try allocator.alloc([:0]u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.string;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) ExerciseTitleMessage {
        return ExerciseTitleMessage{ .raw_fields = raw_fields };
    }
};

pub const ScheduleMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Corresponds to file_id of scheduled workout / course.
    pub fn getManufacturer(msg: ScheduleMessage) ?Manufacturer {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    /// Corresponds to file_id of scheduled workout / course.
    pub fn getSerialNumber(msg: ScheduleMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32z);
        return rf.?.*.raw_value.scalar.uint32z;
    }

    /// Corresponds to file_id of scheduled workout / course.
    pub fn getTimeCreated(msg: ScheduleMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// TRUE if this activity has been started
    pub fn getCompleted(msg: ScheduleMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getType(msg: ScheduleMessage) ?Schedule {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getScheduledTime(msg: ScheduleMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) ScheduleMessage {
        return ScheduleMessage{ .raw_fields = raw_fields };
    }
};

pub const TotalsMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: TotalsMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getTimestamp(msg: TotalsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Excludes pauses
    pub fn getTimerTime(msg: TotalsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getDistance(msg: TotalsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getCalories(msg: TotalsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getSport(msg: TotalsMessage) ?Sport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Includes pauses
    pub fn getElapsedTime(msg: TotalsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getSessions(msg: TotalsMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getActiveTime(msg: TotalsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getSportIndex(msg: TotalsMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) TotalsMessage {
        return TotalsMessage{ .raw_fields = raw_fields };
    }
};

pub const WeightScaleMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: WeightScaleMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getWeight(msg: WeightScaleMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getPercentFat(msg: WeightScaleMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getPercentHydration(msg: WeightScaleMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getVisceralFatMass(msg: WeightScaleMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getBoneMass(msg: WeightScaleMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getMuscleMass(msg: WeightScaleMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getBasalMet(msg: WeightScaleMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 4) - 0;
    }

    pub fn getPhysiqueRating(msg: WeightScaleMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// ~4kJ per kcal, 0.25 allows max 16384 kcal
    pub fn getActiveMet(msg: WeightScaleMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 4) - 0;
    }

    pub fn getMetabolicAge(msg: WeightScaleMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getVisceralFatRating(msg: WeightScaleMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Associates this weight scale message to a user. This corresponds to the index of the user profile message in the weight scale file.
    pub fn getUserProfileIndex(msg: WeightScaleMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getBmi(msg: WeightScaleMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) WeightScaleMessage {
        return WeightScaleMessage{ .raw_fields = raw_fields };
    }
};

pub const BloodPressureMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: BloodPressureMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getSystolicPressure(msg: BloodPressureMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getDiastolicPressure(msg: BloodPressureMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getMeanArterialPressure(msg: BloodPressureMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getMap3SampleMean(msg: BloodPressureMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getMapMorningValues(msg: BloodPressureMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getMapEveningValues(msg: BloodPressureMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getHeartRate(msg: BloodPressureMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getHeartRateType(msg: BloodPressureMessage) ?HrType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getStatus(msg: BloodPressureMessage) ?BpStatus {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Associates this blood pressure message to a user. This corresponds to the index of the user profile message in the blood pressure file.
    pub fn getUserProfileIndex(msg: BloodPressureMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) BloodPressureMessage {
        return BloodPressureMessage{ .raw_fields = raw_fields };
    }
};

pub const MonitoringInfoMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: MonitoringInfoMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Use to convert activity timestamps to local time if device does not support time zone and daylight savings time correction.
    pub fn getLocalTimestamp(msg: MonitoringInfoMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getActivityType(msg: MonitoringInfoMessage, allocator: std.mem.Allocator) ?[]ActivityType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        var ret = try allocator.alloc(ActivityType, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.enumeration;
        }
        return ret;
    }

    /// Indexed by activity_type
    pub fn getCyclesToDistance(msg: MonitoringInfoMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 5000) - 0;
        }
        return ret;
    }

    /// Indexed by activity_type
    pub fn getCyclesToCalories(msg: MonitoringInfoMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 5000) - 0;
        }
        return ret;
    }

    pub fn getRestingMetabolicRate(msg: MonitoringInfoMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) MonitoringInfoMessage {
        return MonitoringInfoMessage{ .raw_fields = raw_fields };
    }
};

pub const MonitoringMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Must align to logging interval, for example, time must be 00:00:00 for daily log.
    pub fn getTimestamp(msg: MonitoringMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Associates this data to device_info message. Not required for file with single device (sensor).
    pub fn getDeviceIndex(msg: MonitoringMessage) ?DeviceIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint8);
    }

    /// Accumulated total calories. Maintained by MonitoringReader for each activity_type. See SDK documentation
    pub fn getCalories(msg: MonitoringMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Accumulated distance. Maintained by MonitoringReader for each activity_type. See SDK documentation.
    pub fn getDistance(msg: MonitoringMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 100) - 0;
    }

    pub fn getActiveTime(msg: MonitoringMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getActivityType(msg: MonitoringMessage) ?ActivityType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getActivitySubtype(msg: MonitoringMessage) ?ActivitySubtype {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getActivityLevel(msg: MonitoringMessage) ?ActivityLevel {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getDistance16(msg: MonitoringMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getCycles16(msg: MonitoringMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getActiveTime16(msg: MonitoringMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Must align to logging interval, for example, time must be 00:00:00 for daily log.
    pub fn getLocalTimestamp(msg: MonitoringMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Avg temperature during the logging interval ended at timestamp
    pub fn getTemperature(msg: MonitoringMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    /// Min temperature during the logging interval ended at timestamp
    pub fn getTemperatureMin(msg: MonitoringMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 14) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    /// Max temperature during the logging interval ended at timestamp
    pub fn getTemperatureMax(msg: MonitoringMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 15) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    /// Indexed using minute_activity_level enum
    pub fn getActivityTime(msg: MonitoringMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 16) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    pub fn getActiveCalories(msg: MonitoringMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 19) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Indicates single type / intensity for duration since last monitoring message.
    pub fn getCurrentActivityTypeIntensity(msg: MonitoringMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 24) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        return rf.?.*.raw_value.scalar.byte;
    }

    pub fn getTimestampMin8(msg: MonitoringMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 25) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getTimestamp16(msg: MonitoringMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 26) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getHeartRate(msg: MonitoringMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 27) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getIntensity(msg: MonitoringMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 28) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 10) - 0;
    }

    pub fn getDurationMin(msg: MonitoringMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 29) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getDuration(msg: MonitoringMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 30) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getAscent(msg: MonitoringMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 31) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getDescent(msg: MonitoringMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 32) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getModerateActivityMinutes(msg: MonitoringMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 33) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getVigorousActivityMinutes(msg: MonitoringMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 34) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) MonitoringMessage {
        return MonitoringMessage{ .raw_fields = raw_fields };
    }
};

pub const MonitoringHrDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Must align to logging interval, for example, time must be 00:00:00 for daily log.
    pub fn getTimestamp(msg: MonitoringHrDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// 7-day rolling average
    pub fn getRestingHeartRate(msg: MonitoringHrDataMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// RHR for today only. (Feeds into 7-day average)
    pub fn getCurrentDayRestingHeartRate(msg: MonitoringHrDataMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) MonitoringHrDataMessage {
        return MonitoringHrDataMessage{ .raw_fields = raw_fields };
    }
};

pub const Spo2DataMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: Spo2DataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getReadingSpo2(msg: Spo2DataMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 1) - 0;
    }

    pub fn getReadingConfidence(msg: Spo2DataMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 1) - 0;
    }

    /// Mode when data was captured
    pub fn getMode(msg: Spo2DataMessage) ?Spo2MeasurementType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) Spo2DataMessage {
        return Spo2DataMessage{ .raw_fields = raw_fields };
    }
};

pub const HrMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: HrMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getFractionalTimestamp(msg: HrMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 32768) - 0;
    }

    pub fn getTime256(msg: HrMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 256) - 0;
    }

    pub fn getFilteredBpm(msg: HrMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8;
        }
        return ret;
    }

    pub fn getEventTimestamp(msg: HrMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1024) - 0;
        }
        return ret;
    }

    pub fn getEventTimestamp12(msg: HrMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.byte;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HrMessage {
        return HrMessage{ .raw_fields = raw_fields };
    }
};

/// Value from 1 to 100 calculated by FirstBeat
pub const StressLevelMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getStressLevelValue(msg: StressLevelMessage) ?i16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        return rf.?.*.raw_value.scalar.sint16;
    }

    /// Time stress score was calculated
    pub fn getStressLevelTime(msg: StressLevelMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) StressLevelMessage {
        return StressLevelMessage{ .raw_fields = raw_fields };
    }
};

pub const MaxMetDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Time maxMET and vo2 were calculated
    pub fn getUpdateTime(msg: MaxMetDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getVo2Max(msg: MaxMetDataMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 10) - 0;
    }

    pub fn getSport(msg: MaxMetDataMessage) ?Sport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getSubSport(msg: MaxMetDataMessage) ?SubSport {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getMaxMetCategory(msg: MaxMetDataMessage) ?MaxMetCategory {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Indicates if calibrated data was used in the calculation
    pub fn getCalibratedData(msg: MaxMetDataMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    /// Indicates if the estimate was obtained using a chest strap or wrist heart rate
    pub fn getHrSource(msg: MaxMetDataMessage) ?MaxMetHeartRateSource {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// Indidcates if the estimate was obtained using onboard GPS or connected GPS
    pub fn getSpeedSource(msg: MaxMetDataMessage) ?MaxMetSpeedSource {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) MaxMetDataMessage {
        return MaxMetDataMessage{ .raw_fields = raw_fields };
    }
};

/// Body battery data used for HSA custom data logging
pub const HsaBodyBatteryDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: HsaBodyBatteryDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Processing interval length in seconds
    pub fn getProcessingInterval(msg: HsaBodyBatteryDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Body battery level: [0,100] Blank: -16
    pub fn getLevel(msg: HsaBodyBatteryDataMessage, allocator: std.mem.Allocator) ?[]i8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        var ret = try allocator.alloc(i8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.sint8;
        }
        return ret;
    }

    /// Body battery charged value
    pub fn getCharged(msg: HsaBodyBatteryDataMessage, allocator: std.mem.Allocator) ?[]i16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(i16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.sint16;
        }
        return ret;
    }

    /// Body battery uncharged value
    pub fn getUncharged(msg: HsaBodyBatteryDataMessage, allocator: std.mem.Allocator) ?[]i16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(i16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.sint16;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HsaBodyBatteryDataMessage {
        return HsaBodyBatteryDataMessage{ .raw_fields = raw_fields };
    }
};

/// HSA events
pub const HsaEventMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: HsaEventMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Event ID. Health SDK use only
    pub fn getEventId(msg: HsaEventMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HsaEventMessage {
        return HsaEventMessage{ .raw_fields = raw_fields };
    }
};

/// Raw accelerometer data used for HSA custom data logging
pub const HsaAccelerometerDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: HsaAccelerometerDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Millisecond resolution of the timestamp
    pub fn getTimestampMs(msg: HsaAccelerometerDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Sampling Interval in Milliseconds
    pub fn getSamplingInterval(msg: HsaAccelerometerDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// X-Axis Measurement
    pub fn getAccelX(msg: HsaAccelerometerDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint16);
            ret[i] = (f / 1.024) - 0;
        }
        return ret;
    }

    /// Y-Axis Measurement
    pub fn getAccelY(msg: HsaAccelerometerDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint16);
            ret[i] = (f / 1.024) - 0;
        }
        return ret;
    }

    /// Z-Axis Measurement
    pub fn getAccelZ(msg: HsaAccelerometerDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint16);
            ret[i] = (f / 1.024) - 0;
        }
        return ret;
    }

    /// 32 kHz timestamp
    pub fn getTimestamp32k(msg: HsaAccelerometerDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HsaAccelerometerDataMessage {
        return HsaAccelerometerDataMessage{ .raw_fields = raw_fields };
    }
};

pub const HsaGyroscopeDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: HsaGyroscopeDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Millisecond resolution of the timestamp
    pub fn getTimestampMs(msg: HsaGyroscopeDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Sampling Interval in 32 kHz timescale
    pub fn getSamplingInterval(msg: HsaGyroscopeDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// X-Axis Measurement
    pub fn getGyroX(msg: HsaGyroscopeDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint16);
            ret[i] = (f / 28.57143) - 0;
        }
        return ret;
    }

    /// Y-Axis Measurement
    pub fn getGyroY(msg: HsaGyroscopeDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint16);
            ret[i] = (f / 28.57143) - 0;
        }
        return ret;
    }

    /// Z-Axis Measurement
    pub fn getGyroZ(msg: HsaGyroscopeDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint16);
            ret[i] = (f / 28.57143) - 0;
        }
        return ret;
    }

    /// 32 kHz timestamp
    pub fn getTimestamp32k(msg: HsaGyroscopeDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HsaGyroscopeDataMessage {
        return HsaGyroscopeDataMessage{ .raw_fields = raw_fields };
    }
};

/// User's current daily step data used for HSA custom data logging
pub const HsaStepDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: HsaStepDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Processing interval length in seconds. File start: 0xFFFFFFEF File stop: 0xFFFFFFEE
    pub fn getProcessingInterval(msg: HsaStepDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Total step sum
    pub fn getSteps(msg: HsaStepDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint32);
            ret[i] = (f / 1) - 0;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HsaStepDataMessage {
        return HsaStepDataMessage{ .raw_fields = raw_fields };
    }
};

/// User's current SpO2 data used for HSA custom data logging
pub const HsaSpo2DataMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: HsaSpo2DataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Processing interval length in seconds
    pub fn getProcessingInterval(msg: HsaSpo2DataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// SpO2 Reading: [70,100] Blank: 240
    pub fn getReadingSpo2(msg: HsaSpo2DataMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8;
        }
        return ret;
    }

    /// SpO2 Confidence: [0,254]
    pub fn getConfidence(msg: HsaSpo2DataMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HsaSpo2DataMessage {
        return HsaSpo2DataMessage{ .raw_fields = raw_fields };
    }
};

/// User's current stress data used for HSA custom data logging
pub const HsaStressDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: HsaStressDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Processing interval length in seconds
    pub fn getProcessingInterval(msg: HsaStressDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Stress Level: [0,100] Off wrist: -1 Excess motion: -2 Not enough data: -3 Recovering from exercise: -4 Unidentified: -5 Blank: -16
    pub fn getStressLevel(msg: HsaStressDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint8);
            ret[i] = (f / 1) - 0;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HsaStressDataMessage {
        return HsaStressDataMessage{ .raw_fields = raw_fields };
    }
};

/// User's current respiration data used for HSA custom data logging
pub const HsaRespirationDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: HsaRespirationDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Processing interval length in seconds
    pub fn getProcessingInterval(msg: HsaRespirationDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Breaths / min: [1,100] Invalid: 255 Excess motion: 254 Off wrist: 253 Not available: 252 Blank: 2.4
    pub fn getRespirationRate(msg: HsaRespirationDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.sint16);
            ret[i] = (f / 100) - 0;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HsaRespirationDataMessage {
        return HsaRespirationDataMessage{ .raw_fields = raw_fields };
    }
};

/// User's current heart rate data used for HSA custom data logging
pub const HsaHeartRateDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: HsaHeartRateDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Processing interval length in seconds
    pub fn getProcessingInterval(msg: HsaHeartRateDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Status of measurements in buffer - 0 indicates SEARCHING 1 indicates LOCKED
    pub fn getStatus(msg: HsaHeartRateDataMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Beats / min. Blank: 0
    pub fn getHeartRate(msg: HsaHeartRateDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint8);
            ret[i] = (f / 1) - 0;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HsaHeartRateDataMessage {
        return HsaHeartRateDataMessage{ .raw_fields = raw_fields };
    }
};

/// Configuration data for HSA custom data logging
pub const HsaConfigurationDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Encoded configuration data
    pub fn getTimestamp(msg: HsaConfigurationDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Encoded configuration data. Health SDK use only
    pub fn getData(msg: HsaConfigurationDataMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.byte;
        }
        return ret;
    }

    /// Size in bytes of data field
    pub fn getDataSize(msg: HsaConfigurationDataMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HsaConfigurationDataMessage {
        return HsaConfigurationDataMessage{ .raw_fields = raw_fields };
    }
};

/// Wrist temperature data used for HSA custom data logging
pub const HsaWristTemperatureDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: HsaWristTemperatureDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Processing interval length in seconds
    pub fn getProcessingInterval(msg: HsaWristTemperatureDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Wrist temperature reading
    pub fn getValue(msg: HsaWristTemperatureDataMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HsaWristTemperatureDataMessage {
        return HsaWristTemperatureDataMessage{ .raw_fields = raw_fields };
    }
};

pub const MemoGlobMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Sequence number of memo blocks
    pub fn getPartIndex(msg: MemoGlobMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 250) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Deprecated. Use data field.
    pub fn getMemo(msg: MemoGlobMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.byte;
        }
        return ret;
    }

    /// Message Number of the parent message
    pub fn getMesgNum(msg: MemoGlobMessage) ?MesgNum {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    /// Index of mesg that this glob is associated with.
    pub fn getParentIndex(msg: MemoGlobMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    /// Field within the parent that this glob is associated with
    pub fn getFieldNum(msg: MemoGlobMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Block of utf8 bytes. Note, mutltibyte characters may be split across adjoining memo_glob messages.
    pub fn getData(msg: MemoGlobMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8z;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) MemoGlobMessage {
        return MemoGlobMessage{ .raw_fields = raw_fields };
    }
};

pub const SleepLevelMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: SleepLevelMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getSleepLevel(msg: SleepLevelMessage) ?SleepLevel {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SleepLevelMessage {
        return SleepLevelMessage{ .raw_fields = raw_fields };
    }
};

pub const AntChannelIdMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getChannelNumber(msg: AntChannelIdMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getDeviceType(msg: AntChannelIdMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return rf.?.*.raw_value.scalar.uint8z;
    }

    pub fn getDeviceNumber(msg: AntChannelIdMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16z);
        return rf.?.*.raw_value.scalar.uint16z;
    }

    pub fn getTransmissionType(msg: AntChannelIdMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8z);
        return rf.?.*.raw_value.scalar.uint8z;
    }

    pub fn getDeviceIndex(msg: AntChannelIdMessage) ?DeviceIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint8);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) AntChannelIdMessage {
        return AntChannelIdMessage{ .raw_fields = raw_fields };
    }
};

pub const AntRxMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: AntRxMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getFractionalTimestamp(msg: AntRxMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 32768) - 0;
    }

    pub fn getMesgId(msg: AntRxMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        return rf.?.*.raw_value.scalar.byte;
    }

    pub fn getMesgData(msg: AntRxMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.byte;
        }
        return ret;
    }

    pub fn getChannelNumber(msg: AntRxMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getData(msg: AntRxMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.byte;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) AntRxMessage {
        return AntRxMessage{ .raw_fields = raw_fields };
    }
};

pub const AntTxMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: AntTxMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getFractionalTimestamp(msg: AntTxMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 32768) - 0;
    }

    pub fn getMesgId(msg: AntTxMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        return rf.?.*.raw_value.scalar.byte;
    }

    pub fn getMesgData(msg: AntTxMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.byte;
        }
        return ret;
    }

    pub fn getChannelNumber(msg: AntTxMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getData(msg: AntTxMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.byte;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) AntTxMessage {
        return AntTxMessage{ .raw_fields = raw_fields };
    }
};

pub const ExdScreenConfigurationMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getScreenIndex(msg: ExdScreenConfigurationMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// number of fields in screen
    pub fn getFieldCount(msg: ExdScreenConfigurationMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getLayout(msg: ExdScreenConfigurationMessage) ?ExdLayout {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getScreenEnabled(msg: ExdScreenConfigurationMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) ExdScreenConfigurationMessage {
        return ExdScreenConfigurationMessage{ .raw_fields = raw_fields };
    }
};

pub const ExdDataFieldConfigurationMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getScreenIndex(msg: ExdDataFieldConfigurationMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getConceptField(msg: ExdDataFieldConfigurationMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        return rf.?.*.raw_value.scalar.byte;
    }

    pub fn getFieldId(msg: ExdDataFieldConfigurationMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getConceptCount(msg: ExdDataFieldConfigurationMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getDisplayType(msg: ExdDataFieldConfigurationMessage) ?ExdDisplayType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getTitle(msg: ExdDataFieldConfigurationMessage, allocator: std.mem.Allocator) ?[:0]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .string);
        var ret = try allocator.alloc([:0]u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.string;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) ExdDataFieldConfigurationMessage {
        return ExdDataFieldConfigurationMessage{ .raw_fields = raw_fields };
    }
};

pub const ExdDataConceptConfigurationMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getScreenIndex(msg: ExdDataConceptConfigurationMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getConceptField(msg: ExdDataConceptConfigurationMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .byte);
        return rf.?.*.raw_value.scalar.byte;
    }

    pub fn getFieldId(msg: ExdDataConceptConfigurationMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getConceptIndex(msg: ExdDataConceptConfigurationMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getDataPage(msg: ExdDataConceptConfigurationMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getConceptKey(msg: ExdDataConceptConfigurationMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getScaling(msg: ExdDataConceptConfigurationMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    pub fn getDataUnits(msg: ExdDataConceptConfigurationMessage) ?ExdDataUnits {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getQualifier(msg: ExdDataConceptConfigurationMessage) ?ExdQualifiers {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getDescriptor(msg: ExdDataConceptConfigurationMessage) ?ExdDescriptors {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getIsSigned(msg: ExdDataConceptConfigurationMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) ExdDataConceptConfigurationMessage {
        return ExdDataConceptConfigurationMessage{ .raw_fields = raw_fields };
    }
};

pub const DiveSummaryMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: DiveSummaryMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getReferenceMesg(msg: DiveSummaryMessage) ?MesgNum {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getReferenceIndex(msg: DiveSummaryMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    /// 0 if above water
    pub fn getAvgDepth(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// 0 if above water
    pub fn getMaxDepth(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Time since end of last dive
    pub fn getSurfaceInterval(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1) - 0;
    }

    pub fn getStartCns(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 1) - 0;
    }

    pub fn getEndCns(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint8);
        return (f / 1) - 0;
    }

    pub fn getStartN2(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1) - 0;
    }

    pub fn getEndN2(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 1) - 0;
    }

    pub fn getO2Toxicity(msg: DiveSummaryMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getDiveNumber(msg: DiveSummaryMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getBottomTime(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Average pressure-based surface air consumption
    pub fn getAvgPressureSac(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 12) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    /// Average volumetric surface air consumption
    pub fn getAvgVolumeSac(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 13) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    /// Average respiratory minute volume
    pub fn getAvgRmv(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 14) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    /// Time to reach deepest level stop
    pub fn getDescentTime(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 15) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Time after leaving bottom until reaching surface
    pub fn getAscentTime(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 16) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Average ascent rate, not including descents or stops
    pub fn getAvgAscentRate(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 17) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint32);
        return (f / 1000) - 0;
    }

    /// Average descent rate, not including ascents or stops
    pub fn getAvgDescentRate(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 22) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Maximum ascent rate
    pub fn getMaxAscentRate(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 23) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Maximum descent rate
    pub fn getMaxDescentRate(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 24) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    /// Time spent neither ascending nor descending
    pub fn getHangTime(msg: DiveSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 25) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) DiveSummaryMessage {
        return DiveSummaryMessage{ .raw_fields = raw_fields };
    }
};

/// Number of acclerometer zero crossings summed over the specified time interval
pub const AadAccelFeaturesMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: AadAccelFeaturesMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Time interval length in seconds
    pub fn getTime(msg: AadAccelFeaturesMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Total accelerometer energy in the interval
    pub fn getEnergyTotal(msg: AadAccelFeaturesMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Count of zero crossings
    pub fn getZeroCrossCnt(msg: AadAccelFeaturesMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Instance ID of zero crossing algorithm
    pub fn getInstance(msg: AadAccelFeaturesMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Total accelerometer time above threshold in the interval
    pub fn getTimeAboveThreshold(msg: AadAccelFeaturesMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 25) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) AadAccelFeaturesMessage {
        return AadAccelFeaturesMessage{ .raw_fields = raw_fields };
    }
};

/// Heart rate variability
pub const HrvMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Time between beats
    pub fn getTime(msg: HrvMessage, allocator: std.mem.Allocator) ?[]f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(f32, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            const f: f32 = @floatFromInt(raw_value.uint16);
            ret[i] = (f / 1000) - 0;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HrvMessage {
        return HrvMessage{ .raw_fields = raw_fields };
    }
};

/// Array of heart beat intervals
pub const BeatIntervalsMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: BeatIntervalsMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Milliseconds past date_time
    pub fn getTimestampMs(msg: BeatIntervalsMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// Array of millisecond times between beats
    pub fn getTime(msg: BeatIntervalsMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) BeatIntervalsMessage {
        return BeatIntervalsMessage{ .raw_fields = raw_fields };
    }
};

pub const HrvStatusSummaryMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: HrvStatusSummaryMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// 7 day RMSSD average over sleep
    pub fn getWeeklyAverage(msg: HrvStatusSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 128) - 0;
    }

    /// Last night RMSSD average over sleep
    pub fn getLastNightAverage(msg: HrvStatusSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 128) - 0;
    }

    /// 5 minute high RMSSD value over sleep
    pub fn getLastNight5MinHigh(msg: HrvStatusSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 128) - 0;
    }

    /// 3 week baseline, upper boundary of low HRV status
    pub fn getBaselineLowUpper(msg: HrvStatusSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 128) - 0;
    }

    /// 3 week baseline, lower boundary of balanced HRV status
    pub fn getBaselineBalancedLower(msg: HrvStatusSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 128) - 0;
    }

    /// 3 week baseline, upper boundary of balanced HRV status
    pub fn getBaselineBalancedUpper(msg: HrvStatusSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 128) - 0;
    }

    pub fn getStatus(msg: HrvStatusSummaryMessage) ?HrvStatus {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HrvStatusSummaryMessage {
        return HrvStatusSummaryMessage{ .raw_fields = raw_fields };
    }
};

pub const HrvValueMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: HrvValueMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// 5 minute RMSSD
    pub fn getValue(msg: HrvValueMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 128) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) HrvValueMessage {
        return HrvValueMessage{ .raw_fields = raw_fields };
    }
};

/// Raw Beat-to-Beat Interval values
pub const RawBbiMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: RawBbiMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Millisecond resolution of the timestamp
    pub fn getTimestampMs(msg: RawBbiMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    /// 1 bit for gap indicator, 1 bit for quality indicator, and 14 bits for Beat-to-Beat interval values in whole-integer millisecond resolution
    pub fn getData(msg: RawBbiMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// Array of millisecond times between beats
    pub fn getTime(msg: RawBbiMessage, allocator: std.mem.Allocator) ?[]u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        var ret = try allocator.alloc(u16, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint16;
        }
        return ret;
    }

    /// 1 = high confidence. 0 = low confidence. N/A when gap = 1
    pub fn getQuality(msg: RawBbiMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8;
        }
        return ret;
    }

    /// 1 = gap (time represents ms gap length). 0 = BBI data
    pub fn getGap(msg: RawBbiMessage, allocator: std.mem.Allocator) ?[]u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        var ret = try allocator.alloc(u8, rf.?.*.raw_value.array.len);
        for (rf.?.*.raw_value.array, 0..) |raw_value, i| {
            ret[i] = raw_value.uint8;
        }
        return ret;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) RawBbiMessage {
        return RawBbiMessage{ .raw_fields = raw_fields };
    }
};

pub const RespirationRateMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: RespirationRateMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// Breaths * 100 /min, -300 indicates invalid, -200 indicates large motion, -100 indicates off wrist
    pub fn getRespirationRate(msg: RespirationRateMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.sint16);
        return (f / 100) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) RespirationRateMessage {
        return RespirationRateMessage{ .raw_fields = raw_fields };
    }
};

/// Specifically used for XERO products.
pub const ChronoShotSessionMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: ChronoShotSessionMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getMinSpeed(msg: ChronoShotSessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getMaxSpeed(msg: ChronoShotSessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getAvgSpeed(msg: ChronoShotSessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getShotCount(msg: ChronoShotSessionMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn getProjectileType(msg: ChronoShotSessionMessage) ?ProjectileType {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getGrainWeight(msg: ChronoShotSessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 10) - 0;
    }

    pub fn getStandardDeviation(msg: ChronoShotSessionMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) ChronoShotSessionMessage {
        return ChronoShotSessionMessage{ .raw_fields = raw_fields };
    }
};

/// Specifically used for XERO products.
pub const ChronoShotDataMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: ChronoShotDataMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getShotSpeed(msg: ChronoShotDataMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 1000) - 0;
    }

    pub fn getShotNum(msg: ChronoShotDataMessage) ?u16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return rf.?.*.raw_value.scalar.uint16;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) ChronoShotDataMessage {
        return ChronoShotDataMessage{ .raw_fields = raw_fields };
    }
};

pub const TankUpdateMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: TankUpdateMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getSensor(msg: TankUpdateMessage) ?AntChannelId {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32z);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint32z);
    }

    pub fn getPressure(msg: TankUpdateMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) TankUpdateMessage {
        return TankUpdateMessage{ .raw_fields = raw_fields };
    }
};

pub const TankSummaryMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: TankSummaryMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getSensor(msg: TankSummaryMessage) ?AntChannelId {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32z);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint32z);
    }

    pub fn getStartPressure(msg: TankSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getEndPressure(msg: TankSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn getVolumeUsed(msg: TankSummaryMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint32);
        return (f / 100) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) TankSummaryMessage {
        return TankSummaryMessage{ .raw_fields = raw_fields };
    }
};

pub const SleepAssessmentMessage = struct {
    raw_fields: []const decoder.FieldData,

    /// Average of awake_time_score and awakenings_count_score. If valid: 0 (worst) to 100 (best). If unknown: FIT_UINT8_INVALID.
    pub fn getCombinedAwakeScore(msg: SleepAssessmentMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Score that evaluates the total time spent awake between sleep. If valid: 0 (worst) to 100 (best). If unknown: FIT_UINT8_INVALID.
    pub fn getAwakeTimeScore(msg: SleepAssessmentMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Score that evaluates the number of awakenings that interrupt sleep. If valid: 0 (worst) to 100 (best). If unknown: FIT_UINT8_INVALID.
    pub fn getAwakeningsCountScore(msg: SleepAssessmentMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Score that evaluates the amount of deep sleep. If valid: 0 (worst) to 100 (best). If unknown: FIT_UINT8_INVALID.
    pub fn getDeepSleepScore(msg: SleepAssessmentMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Score that evaluates the quality of sleep based on sleep stages, heart-rate variability and possible awakenings during the night. If valid: 0 (worst) to 100 (best). If unknown: FIT_UINT8_INVALID.
    pub fn getSleepDurationScore(msg: SleepAssessmentMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Score that evaluates the amount of light sleep. If valid: 0 (worst) to 100 (best). If unknown: FIT_UINT8_INVALID.
    pub fn getLightSleepScore(msg: SleepAssessmentMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Total score that summarizes the overall quality of sleep, combining sleep duration and quality. If valid: 0 (worst) to 100 (best). If unknown: FIT_UINT8_INVALID.
    pub fn getOverallSleepScore(msg: SleepAssessmentMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Score that evaluates the quality of sleep based on sleep stages, heart-rate variability and possible awakenings during the night. If valid: 0 (worst) to 100 (best). If unknown: FIT_UINT8_INVALID.
    pub fn getSleepQualityScore(msg: SleepAssessmentMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Score that evaluates stress and recovery during sleep. If valid: 0 (worst) to 100 (best). If unknown: FIT_UINT8_INVALID.
    pub fn getSleepRecoveryScore(msg: SleepAssessmentMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 8) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Score that evaluates the amount of REM sleep. If valid: 0 (worst) to 100 (best). If unknown: FIT_UINT8_INVALID.
    pub fn getRemSleepScore(msg: SleepAssessmentMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 9) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Score that evaluates the amount of restlessness during sleep. If valid: 0 (worst) to 100 (best). If unknown: FIT_UINT8_INVALID.
    pub fn getSleepRestlessnessScore(msg: SleepAssessmentMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 10) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// The number of awakenings during sleep.
    pub fn getAwakeningsCount(msg: SleepAssessmentMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 11) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Score that evaluates the sleep interruptions. If valid: 0 (worst) to 100 (best). If unknown: FIT_UINT8_INVALID.
    pub fn getInterruptionsScore(msg: SleepAssessmentMessage) ?u8 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 14) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint8);
        return rf.?.*.raw_value.scalar.uint8;
    }

    /// Excludes stress during awake periods in the sleep window
    pub fn getAverageStressDuringSleep(msg: SleepAssessmentMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 15) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        const f: f32 = @floatFromInt(rf.?.*.raw_value.scalar.uint16);
        return (f / 100) - 0;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SleepAssessmentMessage {
        return SleepAssessmentMessage{ .raw_fields = raw_fields };
    }
};

pub const SleepDisruptionSeverityPeriodMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: SleepDisruptionSeverityPeriodMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getTimestamp(msg: SleepDisruptionSeverityPeriodMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getSeverity(msg: SleepDisruptionSeverityPeriodMessage) ?SleepDisruptionSeverity {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SleepDisruptionSeverityPeriodMessage {
        return SleepDisruptionSeverityPeriodMessage{ .raw_fields = raw_fields };
    }
};

pub const SleepDisruptionOvernightSeverityMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: SleepDisruptionOvernightSeverityMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getSeverity(msg: SleepDisruptionOvernightSeverityMessage) ?SleepDisruptionSeverity {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SleepDisruptionOvernightSeverityMessage {
        return SleepDisruptionOvernightSeverityMessage{ .raw_fields = raw_fields };
    }
};

pub const NapEventMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getMessageIndex(msg: NapEventMessage) ?MessageIndex {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 254) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint16);
        return @enumFromInt(rf.?.*.raw_value.scalar.uint16);
    }

    pub fn getTimestamp(msg: NapEventMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getStartTime(msg: NapEventMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getStartTimezoneOffset(msg: NapEventMessage) ?i16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        return rf.?.*.raw_value.scalar.sint16;
    }

    pub fn getEndTime(msg: NapEventMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getEndTimezoneOffset(msg: NapEventMessage) ?i16 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 3) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .sint16);
        return rf.?.*.raw_value.scalar.sint16;
    }

    pub fn getFeedback(msg: NapEventMessage) ?NapPeriodFeedback {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    pub fn getIsDeleted(msg: NapEventMessage) ?bool {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 5) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        return null;
    }

    pub fn getSource(msg: NapEventMessage) ?NapSource {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 6) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .enumeration);
        return @enumFromInt(rf.?.*.raw_value.scalar.enumeration);
    }

    /// The timestamp representing when this nap event was last updated
    pub fn getUpdateTimestamp(msg: NapEventMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 7) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) NapEventMessage {
        return NapEventMessage{ .raw_fields = raw_fields };
    }
};

pub const SkinTempOvernightMessage = struct {
    raw_fields: []const decoder.FieldData,

    pub fn getTimestamp(msg: SkinTempOvernightMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 253) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    pub fn getLocalTimestamp(msg: SkinTempOvernightMessage) ?u32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 0) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .uint32);
        return rf.?.*.raw_value.scalar.uint32;
    }

    /// The average overnight deviation from baseline temperature in degrees C
    pub fn getAverageDeviation(msg: SkinTempOvernightMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 1) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// The average 7 day overnight deviation from baseline temperature in degrees C
    pub fn getAverage7DayDeviation(msg: SkinTempOvernightMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 2) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    /// Final overnight temperature value
    pub fn getNightlyValue(msg: SkinTempOvernightMessage) ?f32 {
        var rf: ?*const decoder.FieldData = null;
        for (msg.raw_fields) |raw_field| {
            if (raw_field.id == 4) {
                rf = &raw_field;
                break;
            }
        }
        if (rf == null) {
            return null;
        }
        assert(@as(decoder.FieldType, rf.?.*.raw_value.scalar) == .float32);
        return rf.?.*.raw_value.scalar.float32;
    }

    pub fn fromRawFields(raw_fields: []const decoder.FieldData) SkinTempOvernightMessage {
        return SkinTempOvernightMessage{ .raw_fields = raw_fields };
    }
};

const Message = union(MesgNum) {
    file_id: FileIdMessage,
    file_creator: FileCreatorMessage,
    timestamp_correlation: TimestampCorrelationMessage,
    software: SoftwareMessage,
    slave_device: SlaveDeviceMessage,
    capabilities: CapabilitiesMessage,
    file_capabilities: FileCapabilitiesMessage,
    mesg_capabilities: MesgCapabilitiesMessage,
    field_capabilities: FieldCapabilitiesMessage,
    device_settings: DeviceSettingsMessage,
    user_profile: UserProfileMessage,
    hrm_profile: HrmProfileMessage,
    sdm_profile: SdmProfileMessage,
    bike_profile: BikeProfileMessage,
    connectivity: ConnectivityMessage,
    watchface_settings: WatchfaceSettingsMessage,
    ohr_settings: OhrSettingsMessage,
    time_in_zone: TimeInZoneMessage,
    zones_target: ZonesTargetMessage,
    sport: SportMessage,
    hr_zone: HrZoneMessage,
    speed_zone: SpeedZoneMessage,
    cadence_zone: CadenceZoneMessage,
    power_zone: PowerZoneMessage,
    met_zone: MetZoneMessage,
    training_settings: TrainingSettingsMessage,
    dive_settings: DiveSettingsMessage,
    dive_alarm: DiveAlarmMessage,
    dive_apnea_alarm: DiveApneaAlarmMessage,
    dive_gas: DiveGasMessage,
    goal: GoalMessage,
    activity: ActivityMessage,
    session: SessionMessage,
    lap: LapMessage,
    length: LengthMessage,
    record: RecordMessage,
    event: EventMessage,
    device_info: DeviceInfoMessage,
    device_aux_battery_info: DeviceAuxBatteryInfoMessage,
    training_file: TrainingFileMessage,
    weather_conditions: WeatherConditionsMessage,
    weather_alert: WeatherAlertMessage,
    gps_metadata: GpsMetadataMessage,
    camera_event: CameraEventMessage,
    gyroscope_data: GyroscopeDataMessage,
    accelerometer_data: AccelerometerDataMessage,
    magnetometer_data: MagnetometerDataMessage,
    barometer_data: BarometerDataMessage,
    three_d_sensor_calibration: ThreeDSensorCalibrationMessage,
    one_d_sensor_calibration: OneDSensorCalibrationMessage,
    video_frame: VideoFrameMessage,
    obdii_data: ObdiiDataMessage,
    nmea_sentence: NmeaSentenceMessage,
    aviation_attitude: AviationAttitudeMessage,
    video: VideoMessage,
    video_title: VideoTitleMessage,
    video_description: VideoDescriptionMessage,
    video_clip: VideoClipMessage,
    set: SetMessage,
    jump: JumpMessage,
    split: SplitMessage,
    split_summary: SplitSummaryMessage,
    climb_pro: ClimbProMessage,
    field_description: FieldDescriptionMessage,
    developer_data_id: DeveloperDataIdMessage,
    course: CourseMessage,
    course_point: CoursePointMessage,
    segment_id: SegmentIdMessage,
    segment_leaderboard_entry: SegmentLeaderboardEntryMessage,
    segment_point: SegmentPointMessage,
    segment_lap: SegmentLapMessage,
    segment_file: SegmentFileMessage,
    workout: WorkoutMessage,
    workout_session: WorkoutSessionMessage,
    workout_step: WorkoutStepMessage,
    exercise_title: ExerciseTitleMessage,
    schedule: ScheduleMessage,
    totals: TotalsMessage,
    weight_scale: WeightScaleMessage,
    blood_pressure: BloodPressureMessage,
    monitoring_info: MonitoringInfoMessage,
    monitoring: MonitoringMessage,
    monitoring_hr_data: MonitoringHrDataMessage,
    spo2_data: Spo2DataMessage,
    hr: HrMessage,
    stress_level: StressLevelMessage,
    max_met_data: MaxMetDataMessage,
    hsa_body_battery_data: HsaBodyBatteryDataMessage,
    hsa_event: HsaEventMessage,
    hsa_accelerometer_data: HsaAccelerometerDataMessage,
    hsa_gyroscope_data: HsaGyroscopeDataMessage,
    hsa_step_data: HsaStepDataMessage,
    hsa_spo2_data: HsaSpo2DataMessage,
    hsa_stress_data: HsaStressDataMessage,
    hsa_respiration_data: HsaRespirationDataMessage,
    hsa_heart_rate_data: HsaHeartRateDataMessage,
    hsa_configuration_data: HsaConfigurationDataMessage,
    hsa_wrist_temperature_data: HsaWristTemperatureDataMessage,
    memo_glob: MemoGlobMessage,
    sleep_level: SleepLevelMessage,
    ant_channel_id: AntChannelIdMessage,
    ant_rx: AntRxMessage,
    ant_tx: AntTxMessage,
    exd_screen_configuration: ExdScreenConfigurationMessage,
    exd_data_field_configuration: ExdDataFieldConfigurationMessage,
    exd_data_concept_configuration: ExdDataConceptConfigurationMessage,
    dive_summary: DiveSummaryMessage,
    aad_accel_features: AadAccelFeaturesMessage,
    hrv: HrvMessage,
    beat_intervals: BeatIntervalsMessage,
    hrv_status_summary: HrvStatusSummaryMessage,
    hrv_value: HrvValueMessage,
    raw_bbi: RawBbiMessage,
    respiration_rate: RespirationRateMessage,
    chrono_shot_session: ChronoShotSessionMessage,
    chrono_shot_data: ChronoShotDataMessage,
    tank_update: TankUpdateMessage,
    tank_summary: TankSummaryMessage,
    sleep_assessment: SleepAssessmentMessage,
    sleep_disruption_severity_period: SleepDisruptionSeverityPeriodMessage,
    sleep_disruption_overnight_severity: SleepDisruptionOvernightSeverityMessage,
    nap_event: NapEventMessage,
    skin_temp_overnight: SkinTempOvernightMessage,
};
